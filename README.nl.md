...
Met één `npx`-opdracht worden ze geïnstalleerd. Geen externe backend of cloudhosting: uw codeeragent voert alles lokaal uit.
...
Agent-Vorcl-Flow verandert een ondersteunde codeeragent in een **gestructureerd technisch team**. In plaats van één algemene assistent krijgt u **22 gerichte subagenten** (architect, backend, frontend, Expo mobiele ingenieur, DB ingenieur, architectuurcartograaf, liveboard-operator en meer), elk met zijn eigen **vaardigheden** op het domein, snelle **slash-opdrachten** en de **MCP tools** die hij nodig heeft. Elke niet-triviale taak doorloopt een gedisciplineerde **Task Master**-lus – *doel → taken → implementeren → verifiëren → klaar* – zodat het werk wordt gepland, bijgehouden en onderbrekingen overleeft.

- 🧩 **22 subagenten**, 44 vaardigheden, 135 slash-opdrachten
...
- 🔑 **Eén `.env`-bestand voor alle runtimes** — sleutels gelezen door een opstartprogramma, niet `~/.zshrc`, dus ze werken zelfs vanuit GUI/IDE-starts; geen externe AVF-service; liveboard is alleen localhost en kortstondig
...
> Het installatieprogramma vult nooit uw geheimen in; het maakt alleen een lege `.env` op basis van de sjabloon. Daar voeg je sleutels toe (zie [Configuration](#configuratie-mcp--toetsen)).
...
### Alternatieve installaties (Claude Code)
...
Na de installatie **herstart Claude Code** (of voer `/reload-plugins` uit in een open sessie) om de agents te laden.
...
### 1. Universeel toegangspunt – noem gewoon een doel
```text
/vorcl add a shopping cart to checkout
/audit .
```
`/vorcl` zoekt uit welke subagent eigenaar moet zijn van het werk en regelt de volledige Task Master-cyclus. `/audit` detecteert automatisch backend, frontend, mobiel, data en infrastructuur en schrijft een op bewijs gebaseerde `PROJECT_AUDIT.md` met behulp van alle relevante rollen.

### 2. Praat met een specifieke subagent
```text
@agent-vorcl-flow:architect  design billing for a SaaS
@agent-vorcl-flow:backend    add a POST /invoices endpoint
```

### 3. Voer een specifiek slash-commando uit
```text
...
Elke agent heeft ook zijn eigen `/<agent>:vorcl`-ingangspunt dat de Task Master-lus uitvoert die naar die agent is gericht.
...
## De agenten| Agent | Rol | Hoogtepunten |
| --- | --- | --- |
| 🔵 **architect** | Systeem- en oplossingsarchitect | Analyse van vereisten, systeem-/DB/API-ontwerp, architectuurbeoordelingen |
| 🟢 **backend** | Backend-ontwikkelaar | Knooppunt/TS, Postgres, Redis; modulaire architectuur; elke route die volledig wordt gedekt door OpenAPI |
...
| ⚪ **veerkracht** | Betrouwbaarheid: fouten + loggen | proberen/vangen op de juiste grenzen, getypte fouten, nieuwe pogingen/time-outs, gestructureerde logs |
...
| 🧜 **zeemeermin** | Mermaid diagrammen (+ echte weergave) | stroomdiagram, volgorde, klasse, staat, ER, gantt, gitGraph, mindmap…; gevalideerd via mcp-zeemeermin/`mmdc`; overhandigt u het bestand (`.mmd` + SVG/PNG/PDF) |
...
| 🛡️ **beveiliging** | Beveiligingsauditor (alleen-lezen) | Geheimen in de boom- en gitgeschiedenis, OWASP Top 10, afhankelijkheids-CVE's, PII; bevindingen worden taken – oplossingen worden gedelegeerd |
| 📝 **documenten** | Documentatie-ingenieur | README (meertalige pariteit), API documenten van OpenAPI, ARCHITECTUUR, BIJDRAGEN, release-opmerkingen; elk voorbeeld geverifieerd aan de hand van de code |
| 🐳 **ontwikkelaars** | Containers & CI/CD | Multistage Dockerfiles, docker-compose voor lokale ontwikkelaar, GitHub Actiepijplijnen, omgevings-/geheimenhygiëne, monitoring |
| 📡 **liveboard** | Lokaal activiteitenbord | Live Git werkbomen, agentprocessen en Task Master taken op een kortstondig localhost-dashboard |**Een paar dingen die de moeite waard zijn om te weten:**
- **Frontend praat altijd met een echte API.** De OpenAPI-specificatie van de backend is de enige bron van waarheid; er worden typen gegenereerd (`openapi-typescript` + `openapi-fetch`). Geen schijnvertoningen in het productietraject.
...
Elke onderstaande opdracht is een slash-opdracht. `<…>` markeert uw invoer.

### `/vorcl` — universele router
...
### 📱 expo-mobiel — React Native / Expo| Commando | Wat het doet |
| --- | --- |
| `/expo-mobile:vorcl <goal>` | Doel → Task Master fietsen voor Expo mobiel werken. |
...
| `/expo-mobile:test <scope>` | Voer domeineenheid, React Native Testbibliotheek en Maestro controles uit. |

### 🟠 analyser — code-audit (alleen-lezen)
| Commando | Wat het doet |
| --- | --- |
| `/analyzer:vorcl <goal>` | Controleer een doel via Task Master — bevindingen worden taken. |
| `/analyzer:audit` | Volledige audit: bugs, typen, DB, frontend-spots, backend-geuren. |
| `/analyzer:bugs` | Ga op zoek naar bugs: onverwerkte fouten, raceomstandigheden, randgevallen. |
| `/analyzer:types` | Typecontrole — `tsc`, `any`, onveilige casts, zod↔types drift. |
| `/analyzer:db` | Audit DB structuur — schema, indexen, FK's, N+1, migraties. |
| `/analyzer:mocks` | Vind mockup/nepgegevens op de frontend. |
| `/analyzer:backend` | Vind "slechte" backend-code - architectuurschendingen, logica in controllers. |

### 🟡 branie — OpenAPI/Swagger dekking (elke stapel)
| Commando | Wat het doet |
| --- | --- |
| `/swagger:vorcl <goal>` | Doel voor volledige dekking via Task Master — audit → taken → dekking → verifiëren. |
| `/swagger:audit` | Alleen-lezen: vind routes die niet volledig onder de specificatie vallen. |
| `/swagger:cover <route>` | Behandel een route/module – parameters, antwoorden, beschrijvingen, beveiliging + verificatie. |

### 🔴 firecrawl — webonderzoek
| Commando | Wat het doet |
| --- | --- |
...
| `/firecrawl:parse <file>` | Parseer een lokaal/privédocument in markdown of JSON. |
...
### 🟤 render — hosten/implementeren (Render)
| Commando | Wat het doet |
| --- | --- |
| `/render:vorcl <goal>` | Infradoel via Task Master — implementeren/diagnostiseren/configureren naar klaar. |
...
### 🟦 database — DB ingenieur / DBA (Postgres / MongoDB / Redis)
| Commando | Wat het doet |
| --- | --- |
| `/database:vorcl <goal>` | Gegevensdoel via Task Master — schema/query's/migraties/cache naar gedaan. |
...
### ⚪ veerkracht — foutafhandeling + loggen
| Commando | Wat het doet |
| --- | --- |
...
### 🔎 visueel onderzoek — screenshot → geverifieerd webantwoord
| Commando | Wat het doet |
| --- | --- |
...
### 🎯 lokaliseren — screenshot → in een bestaand project plaatsen (alleen-lezen)
| Commando | Wat het doet |
| --- | --- |
...
| `/pinpoint:trace <target>` | Traceer de logica achter een element: handler → state → data-fetch → API. |
| `/pinpoint:handoff <change>` | Bouw een nauwkeurig bewerkingsverzoek op basis van bestaande code en delegeer dit aan `frontend`/`backend`. |

### 📊 drawio — diagrammen (draw.io / diagrams.net)
| Commando | Wat het doet |
| --- | --- |
...
| `/drawio:convert <source> [type]` | Een bron naar een diagram converteren — DB schema → ERD, mappen → boom, code → UML, zeemeermin/CSV/JSON. |
| `/drawio:refine <file>` | Verfijn een bestaande `.drawio` — lay-out, thema, knooppunten toevoegen/verwijderen, uitlijnen op raster. |

### 🗺️ archmap — architectuurkaart uit code| Commando | Wat het doet |
| --- | --- |
...
| `/archmap:extract [repo]` | Alleen extractie: machinaal leesbare `architecture.json` met `source:{file,line}` op elk knooppunt. |
| `/archmap:annotate [json]` | LLM-verrijking van een bestaande `architecture.json` (agentgeheugen, dataflow-semantiek); onbewezen feiten automatisch gedegradeerd naar `inferred`. |
...
### 🧜 zeemeermin — Mermaid diagrammen (+ echte weergave)
| Commando | Wat het doet |
| --- | --- |
...
| `/testing:integration <endpoint\|module>` | Integratietesten (Supertest/inject, real DB of testcontainers). |
...
### 🌿 gitflow — git-workflow en releases
| Commando | Wat het doet |
| --- | --- |
...
### 🛡️ beveiliging — beveiligingsaudit (alleen-lezen)
| Commando | Wat het doet |
| --- | --- |
...
| `/docs:readme [path]` | README maken/bijwerken — what/quickstart/usage/config/troubleshooting; voorbeelden geverifieerd; taalversies gesynchroniseerd. || `/docs:api [spec]` | API documenten gegenereerd op basis van de OpenAPI-specificatie (eindpunten, params, curl-voorbeelden); suggereert `/swagger:audit` als er geen specificatie is. |
| `/docs:architecture` | ARCHITECTURE.md — modules, grenzen, gegevensstroom; diagrammen gedelegeerd aan `mermaid`/`drawio`. |
| `/docs:contributing` | CONTRIBUTING.md - opzet, structuur, tests, commit-conventies (uitgelijnd met `gitflow`), PR-proces. |
...
| `/devops:ci [type]` | GitHub Acties — PR-workflow (lint+typecheck+test, npm cache), workflow implementeren, minimale machtigingen. |
...
### 📡 liveboard — kortstondig lokaal operatiebord
| Commando | Wat het doet |
| --- | --- |
| `/liveboard:start [path] [--port N] [--interval ms]` | Start een gepolijst dashboard in 43 talen op een gratis localhost-poort; Task Master verandert de stroom via SSE en stemt elke 5 minuten af. |
| `/liveboard:vorcl <goal>` | Ontwikkel of wijzig liveboard zelf via de benodigde Task Master workflow. |
...
- `--interval`: volledig afstemmingsinterval in milliseconden; bestand bekijken stilstaande streams Task Master verandert onmiddellijk.
...
> **Waarom een launcher in plaats van `~/.zshrc`?** Env-var-uitbreiding verschilt per runtime (`${VAR:-}` in Claude, `${env:VAR}` in Cursor, letterlijke waarden in Codex/Kimi) en elke runtime leest alleen de omgeving waarin **deze** werd gelanceerd. GUI/IDE-lanceringen op macOS genereren geen bron `~/.zshrc`, dus geëxporteerde sleutels zijn onzichtbaar en de servers maken met niets verbinding - de klassieke "MCP env not set"-fout. Als u uit één `.env`-bestand leest, worden beide problemen in één keer opgelost.**Voorrang** (later wint): de gedeelde `~/.config/agent-vorcl-flow/.env` → een `./.env` in de projectroot → een echte `export` in je shell. Bewaar de globale sleutels in het gedeelde bestand, overschrijf per project (bijvoorbeeld een andere `MONGODB_URI`) met een project `.env`, en een echte shell-export wint nog steeds voor CLI runs. U kunt het opstartprogramma naar een ander bestand richten met `AGENT_VORCL_ENV_FILE=/path/.env`.

Een server waarvan de vereiste sleutel ontbreekt **start eenvoudigweg niet**: u ziet een regel `[agent-vorcl-flow] MCP «…» is not configured: …` in het MCP-logboek van de runtime, en elke andere server blijft werken. Voeg de sleutel toe aan `.env` en start opnieuw op. (Je mag de `GITHUB_TOKEN`/`MONGODB_URI`-namen behouden; het opstartprogramma wijst ze toe aan de `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` die de servers verwachten.)

> ⚠️ **Vereist voor AI-aangedreven Task Master-opdrachten:** configureer ten minste één geselecteerde provider — `ANTHROPIC_API_KEY` voor Claude, `OPENAI_API_KEY` voor GPT of Codex CLI OAuth. Zonder inloggegevens voor het model dat is geselecteerd in `.taskmaster/config.json`, kan `/vorcl` geen taken genereren of uitbreiden.

Kies welke Task Master provider daadwerkelijk de opwekking uitvoert; toetsen alleen selecteren het model niet:
...
Codex heeft geen "plug-ins", dus dezelfde mogelijkheden worden uitgedrukt als **vaardigheden**, **profielen** en een `AGENTS.md`-router:

| Claude Code | Codex gelijkwaardig |
| --- | --- |
| subagent `@agent-vorcl-flow:frontend` | vaardigheidspersoon `$frontend` + `codex --profile frontend` |
...
| Agent-Vorcl-Flow-concept | Cursor gelijkwaardig |
| --- | --- |
...
Het installatieprogramma converteert roldefinities naar Cursor frontmatter, geeft subagenten het voorvoegsel `avf-` om botsingen tussen vaardigheidsnamen te voorkomen, gebruikt `model: inherit` en markeert agenten die alleen audits uitvoeren als `readonly: true`. Bestaande MCP-servergegevens met dezelfde naam blijven behouden. Zie [`cursor/README.md`](./cursor/README.md).
...
| Expo PostToolUse guard | samengevoegd tot `~/.kimi/config.toml` |
...
Kimi CLI heeft geen `${VAR}`-uitbreiding in `mcp.json`, dus sleutels komen van de gedeelde `.env` via het opstartprogramma - precies zoals de andere runtimes. Zie [`kimi/README.md`](./kimi/README.md).
