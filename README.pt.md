...
**Uma equipe de subagentes de IA especializados para [Claude Code](https://claude.com/claude-code), [GPT Codex](https://developers.openai.com/codex/cli/), [Cursor](https://cursor.com/) e [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) — com habilidades, comandos e MCP ferramentas.**
...
| **Kimi CLI** | Instala habilidades em `~/.kimi/skills`, o agente personalizado Expo nativo em `~/.kimi/agents`, ambos Expo arquitetura/UI conectam-se em `~/.kimi/config.toml` e mescla servidores MCP. |

> O instalador nunca preenche seus segredos — ele apenas cria um `.env` vazio a partir do modelo. Você adiciona chaves lá (veja [Configuration](#configuração-mcp--teclas)).
...
| 🟡 **arrogância** | Cobertura OpenAPI/Swagger (qualquer stack) | Encontra rotas não totalmente documentadas e as cobre, com verificação |
...
| 🧪 **testes** | Engenheiro de teste e verificação | Unidade (Vitest/Jest), integração (Supertest), E2E (Playwright), cobertura, caça a testes esquisitos; executa `testStrategy` de cada tarefa — nada é "feito" sem uma execução verde |
...
| 📝 **documentos** | Engenheiro de documentação | README (paridade multilíngue), API documentos de OpenAPI, ARCHITECTURE, CONTRIBUTING, notas de lançamento; cada exemplo verificado em relação ao código |
| 🐳 **devops** | Recipientes e CI/CD | Dockerfiles de vários estágios, docker-compose para desenvolvimento local, GitHub Pipelines de ações, higiene de ambiente/segredos, monitoramento |
| 📡 **quadro ao vivo** | Conselho de operações locais | Árvores de trabalho Git, processos de agentes e tarefas Task Master em tempo real em um painel localhost efêmero |**Algumas coisas que vale a pena saber:**
...
Cada comando abaixo é um comando de barra. `<…>` marca sua entrada.
...
| `/audit [path] [focus]` | Auditoria multifuncional somente leitura profunda → sistemas detectados, descobertas de segurança/CVE/resiliência, arquitetura de destino e `PROJECT_AUDIT.md` em fases. |
...
| | Adicione chaves de esquema/DTO/mapeador/query e integração TanStack Query. |
...
| `/expo-mobile:compatibility [app] [change]` | Auditoria de compatibilidade de Expo/RN/Node/package/native-runtime somente leitura em tempo real contra fontes oficiais versionadas. |
...
| `/analyzer:db` | Estrutura DB de auditoria — esquema, índices, FKs, N+1, migrações. |
...
| Comando | O que faz |
| --- | --- |
| `/database:vorcl <goal>` | Meta de dados via Task Master — esquema/consultas/migrações/cache concluídas. |
...
| Comando | O que faz |
| --- | --- |
| `/gitflow:vorcl <goal>` | Uma meta git/release via Task Master (preparar um lançamento, limpar histórico, ramificação de recursos). |
...
| `/devops:env` | Inventário de variáveis ​​ambientais: onde ler, o que é necessário, `.env.example` modelo; segredos nunca em imagens. |
...
- `--port 0`: seleciona automaticamente uma porta livre.
...
> **Por que um iniciador em vez de `~/.zshrc`?** A expansão Env-var difere de acordo com o tempo de execução (`${VAR:-}` em Claude, `${env:VAR}` em Cursor, literais em Codex/Kimi) e cada tempo de execução lê apenas o ambiente **em que** foi iniciado. Inicializações de GUI/IDE no macOS não originam `~/.zshrc`, portanto, as chaves exportadas são invisíveis e os servidores não se conectam a nada — a clássica falha "MCP env not set". A leitura de um arquivo `.env` elimina os dois problemas de uma só vez.**Precedência** (vence mais tarde): o `~/.config/agent-vorcl-flow/.env` → um `./.env` compartilhado na raiz do projeto → um `export` real no seu shell. Mantenha as chaves globais no arquivo compartilhado, substitua por projeto (por exemplo, um `MONGODB_URI` diferente) por um projeto `.env`, e uma exportação de shell genuína ainda vence para execuções de CLI. Você pode apontar o inicializador para um arquivo diferente com `AGENT_VORCL_ENV_FILE=/path/.env`.
...
Codex não tem "plugins", portanto os mesmos recursos são expressos como **habilidades**, **perfis** e um roteador `AGENTS.md`:
...
| --- | --- |
| função `backend` | subagente personalizado `/avf-backend` em `~/.cursor/agents` |
...
[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) carrega nativamente habilidades de agente, arquivos de agente personalizados e ganchos de ciclo de vida; AVF também mescla os mesmos MCP servidores usados por Claude e Cursor:
...
**Como tudo se encaixa:** `agents/*.md` declara uma função e, no início `skills:`, anexa habilidades → habilidades em `skills/*/SKILL.md` são carregadas automaticamente por descrição → `commands/<agent>/*.md` fornece atalhos `/agent:command` rápidos que delegam ao subagente → `.mcp.json` fornece aos agentes suas ferramentas, cada uma iniciada por `bin/mcp-env.mjs` que carrega segredos do `.env` compartilhado. Um gancho `SessionStart` informa a Claude que os agentes estão disponíveis.
