...
Agent-Vorcl-Flow превръща поддържан кодиращ агент в **структуриран инженерен екип**. Вместо един общ асистент, вие получавате **22 фокусирани под-агента** (архитект, бекенд, фронтенд, Expo мобилен инженер, DB инженер, архитектурен картограф, оператор на liveboard и други), всеки със собствен домейн **умения**, бързи **команди с наклонена черта** и **MCP инструменти**, от които се нуждае. Всяка нетривиална задача преминава през дисциплиниран **Task Master** цикъл — *цел → задачи → изпълнение → проверка → готово* — така че работата се планира, проследява и оцелява при прекъсвания.

- 🧩 **22 подагента**, 44 умения, 135 команди с наклонена черта
...
- 🤝 **Работи на Claude Code, GPT Codex, Cursor и Kimi CLI** от един и същи източник
...
| **Claude Code** | Регистрира това репо като **пазар** на приставка и активира приставката (чрез `claude plugin …`, с директен `~/.claude/settings.json` резервен вариант). |
...
| **Kimi CLI** | Инсталира умения в `~/.kimi/skills`, нативния Expo персонализиран агент в `~/.kimi/agents`, и двете Expo архитектура/UI се свързват в `~/.kimi/config.toml` и обединяват MCP сървъри. |

> Инсталаторът никога не попълва вашите тайни — той само създава празен `.env` от шаблона. Добавяте ключове там (вижте [Configuration](#конфигурация-mcp-и-ключове)).

### Актуализирайте до най-новата версия

Стартирайте инсталатора отново с етикета npm `latest`:
...
След инсталиране **рестартирайте Claude Code** (или стартирайте `/reload-plugins` в отворена сесия), за да заредите агентите.
...
### 1. Универсална входна точка — просто заявете цел
```text
/vorcl add a shopping cart to checkout
/audit .
```
`/vorcl` определя кой подагент трябва да притежава работата и задвижва пълния Task Master цикъл. `/audit` автоматично открива бекенд, фронтенд, мобилни устройства, данни и инфраструктура и пише базиран на доказателства `PROJECT_AUDIT.md`, използвайки всички съответни роли.

### 2. Говорете с конкретен подагент
```text
@agent-vorcl-flow:architect  design billing for a SaaS
@agent-vorcl-flow:backend    add a POST /invoices endpoint
```

### 3. Изпълнете конкретна команда за наклонена черта
```text
...
Всеки агент също има своя собствена `/<agent>:vorcl` входна точка, която изпълнява Task Master цикъла, обхванат от този агент.
...
## Агентите| Агент | Роля | Акценти |
| --- | --- | --- |
| 🔵 **архитект** | Архитект на системи и решения | Анализ на изискванията, системен/DB/API дизайн, прегледи на архитектурата |
...
| 🟠 **анализатор** | Одитор на код (само за четене) | Бъгове, безопасност на типа, DB структура, подигравки на фронтенда, миризми на задния край |
| 🟡 **перчене** | OpenAPI/Swagger покритие (всеки стек) | Намира маршрути, които не са напълно документирани и ги покрива с проверка |
| 🔴 **firecrawl** | Уеб изследовател | Live CLI/MCP/REST, интегриране на приложения и готови работни потоци с уеб данни |
...
| 🧜 **русалка** | Mermaid диаграми (+ реално изобразяване) | блок-схема, последователност, клас, състояние, ER, gantt, gitGraph, mindmap…; потвърдено чрез mcp-mermaid/`mmdc`; предава ви файла (`.mmd` + SVG/PNG/PDF) |
...
| 🌿 **gitflow** | Git работен процес и издания | Конвенционални ангажименти, ангажименти по име (никога `git add .`), PRs, Keep-a-Changelog, издания на semver; натискане само с изрично потвърждение |
| 🛡️ **сигурност** | Одитор на сигурността (само за четене) | Тайни в историята на дървото и git, Топ 10 на OWASP, CVE на зависимости, PII; констатациите стават задачи — поправките се делегират |
| 📝 **документи** | Инженер по документация | README (многоезичен паритет), API документи от OpenAPI, АРХИТЕКТУРА, ПРИНОС, бележки по изданието; всеки пример, проверен спрямо кода |
...
| 📡 **liveboard** | Местен оперативен съвет | Живи Git работни дървета, процеси на агенти и Task Master задачи на ефимерно табло за локален хост |**Няколко неща, които си струва да знаете:**
...
- **`visual-research` проверява, вместо да предполага.** Той третира екранна снимка като доказателство, потвърждава официалния домейн и документи, проверява текущите данни на сайта и маркира възможни фишинг или остарели стойности.
- **`i18n` налага „нулево езиково твърдо кодиране.“** Агентите първо откриват дали даден проект е многоезичен и се адаптират — обърнатите към потребителя низове преминават през слой за превод (next-intl / react-i18next / i18next), никога вграден.

---

##  Справка за команди

Всяка команда по-долу е команда с наклонена черта. `<…>` маркира въведеното от вас.

### `/vorcl` — универсален рутер
| Команда | Какво прави |
| --- | --- |
...
### 🟢 бекенд — сървър (възел/TS, Postgres, Redis)
| Команда | Какво прави |
| --- | --- |
...
### 📱 expo-mobile — React Native / Expo| Команда | Какво прави |
| --- | --- |
...
| `/expo-mobile:add-api <contract>` | Добавяне на схема/DTO/mapper/ключове за заявка и TanStack Query интеграция. |
...
| `/expo-mobile:compatibility [app] [change]` | Одит на съвместимост само за четене на живо Expo/RN/Node/package/native-runtime спрямо официални източници с версии. |
| `/expo-mobile:test <scope>` | Изпълнете единица на домейн, React Native Библиотека за тестване и Maestro проверки. |

### 🟠 анализатор — одит на код (само за четене)
| Команда | Какво прави |
| --- | --- |
...
| `/analyzer:types` | Проверка на типа — `tsc`, `any`, несигурни кастинги, отклонение на zod↔типове. |
| `/analyzer:db` | Одит DB структура — схема, индекси, FK, N+1, миграции. |
| `/analyzer:mocks` | Намерете макет / фалшиви данни в интерфейса. |
| `/analyzer:backend` | Намерете „лош“ бекенд код — нарушения на архитектурата, логика в контролерите. |

### 🟡 размах — OpenAPI/Swagger покритие (всеки стек)
| Команда | Какво прави |
| --- | --- |
...
### 🔴 firecrawl — уеб проучване
| Команда | Какво прави |
| --- | --- |
| `/firecrawl:vorcl <goal>` | Изследователска цел чрез Task Master — събиране на уеб данни до завършен резултат. |
...
| `/firecrawl:extract <url>` | Структурирано извличане чрез JSON схема. |
| `/firecrawl:setup` | Инсталирайте/потвърдете CLI плюс официални умения за изграждане и работен процес (с потвърждение). |
...
| `/firecrawl:monitor <action>` | Списък на проверките или управление на повтарящи се монитори за смяна на страници. |
| `/firecrawl:agent <goal>` | Изпълнете ограничена дълго изпълняваща се Firecrawl задача на агент. |
...
### 🟤 изобразяване — хостинг / внедряване (Визуализация)
| Команда | Какво прави |
| --- | --- |
| `/render:vorcl <goal>` | Инфра цел чрез Task Master — внедряване/диагностициране/конфигуриране до готово. |
...
### 🟦 база данни — DB инженер / DBA (Postgres / MongoDB / Redis)
| Команда | Какво прави |
| --- | --- |
...
### ⚪ устойчивост — обработка на грешки + регистриране
| Команда | Какво прави |
| --- | --- |
...
### 🖼️ екранна снимка — екранна снимка UI → код
| Команда | Какво прави |
| --- | --- |
...
### 🔎 визуално изследване — екранна снимка → потвърден уеб отговор
| Команда | Какво прави |
| --- | --- |
...
### 🎯 pinpoint — екранна снимка → място в съществуващ проект (само за четене)
| Команда | Какво прави |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Намерете/разберете/променете съществуващ UI от екранна снимка чрез Task Master — карта → задачи → делегат. |
| `/pinpoint:locate <image>` | Намерете съществуващия компонент/файл(ове) от екранна снимка — `file:line`, няма нов код. |
...
| `/pinpoint:trace <target>` | Проследете логиката зад елемент — манипулатор → състояние → извличане на данни → API. |
| `/pinpoint:handoff <change>` | Създайте прецизна заявка за редактиране спрямо съществуващ код и делегирайте на `frontend`/`backend`. |

### 📊 drawio — диаграми (draw.io / diagrams.net)
| Команда | Какво прави |
| --- | --- |
...
### 🗺️ archmap — архитектурна карта от код| Команда | Какво прави |
| --- | --- |
...
### 🧜 русалка — Mermaid диаграми (+ реално изобразяване)
| Команда | Какво прави |
| --- | --- |
| `/mermaid:vorcl <goal>` | Набор от диаграми чрез Task Master — компилиране до готово (рендиране потвърдено). |
...
| `/testing:verify <task\|testStrategy>` | Изпълнява `testStrategy` на задача и връща присъда ГОТОВ / НЕ ГОТОВ с реален резултат. |
| `/testing:coverage [path]` | Доклад за покритие с констатации — кой критичен код не е тестван; създава задачи. |
| `/testing:flaky <test>` | Диагностицира нестабилен тест (състезание, времена, споделено състояние, подигравки) и го коригира завинаги. |

### 🌿 gitflow — git работен процес и издания
| Команда | Какво прави |
| --- | --- |
...
| `/gitflow:release <version\|auto>` | Semver от ангажименти → синхронизирайте версии на манифест → етикет → GitHub издание. Push само след изрично потвърждение. |
...
### 🛡️ сигурност — одит на сигурността (само за четене)
| Команда | Какво прави |
| --- | --- |
...
| `/docs:readme [path]` | Създаване/актуализиране на README — what/quickstart/usage/config/troubleshooting; проверени примери; синхронизирани езикови версии. || `/docs:api [spec]` | API документи, генерирани от OpenAPI спецификацията (крайни точки, параметри, curl примери); предлага `/swagger:audit` ако няма спец. |
| `/docs:architecture` | ARCHITECTURE.md — модули, граници, поток от данни; диаграми, делегирани на `mermaid`/`drawio`. |
| `/docs:contributing` | CONTRIBUTING.md — настройка, структура, тестове, конвенции за ангажиране (подравнени с `gitflow`), PR процес. |
...
### 🐳 devops — контейнери и CI/CD
| Команда | Какво прави |
| --- | --- |
| `/devops:vorcl <goal>` | Инфраструктурна цел чрез Task Master. |
| `/devops:dockerfile [app-type]` | Напишете/прегледайте Dockerfile — многостепенна, тънка основа, не-root, HEALTHCHECK; потвърдено от реален `docker build`. |
| `/devops:compose` | docker-compose.yml за локални разработчици (приложение + DB); env промените се нуждаят от `--force-recreate`, изчаква здрави. |
| `/devops:ci [type]` | GitHub Действия — PR работен поток (lint+typecheck+test, npm кеш), работен процес за внедряване, минимални разрешения. |
...
### 📡 liveboard — ефимерен местен оперативен съвет
| Команда | Какво прави |
| --- | --- |
...
| `/liveboard:vorcl <goal>` | Разработете или променете самата liveboard чрез необходимия Task Master работен процес. |
...
> **Защо стартер вместо `~/.zshrc`?** Разширяването на Env-var се различава за време на изпълнение (`${VAR:-}` в Claude, `${env:VAR}` в Cursor, литерали в Codex/Kimi) и всяко време на изпълнение чете само средата, в която **то** е стартирано. GUI / IDE стартира на macOS не източник `~/.zshrc`, така че експортираните ключове са невидими и сървърите не се свързват с нищо – класическото Грешка „MCP env не е зададена“. Четенето от един `.env` файл премахва и двата проблема наведнъж.**Предимство** (по-късно печели): споделеният `~/.config/agent-vorcl-flow/.env` → a `./.env` в корена на проекта → истински `export` във вашата обвивка. Запазете глобалните ключове в споделения файл, заменете за всеки проект (напр. различен `MONGODB_URI`) с проект `.env` и истинският експорт на обвивка все още печели за CLI изпълнения. Можете да насочите стартовия панел към друг файл с `AGENT_VORCL_ENV_FILE=/path/.env`.

Сървър, чийто изискван ключ липсва, просто **не стартира** — ще видите едноредов `[agent-vorcl-flow] MCP «…» is not configured: …` в дневника MCP на средата за изпълнение и всеки друг сървър продължава да работи. Добавете ключа към `.env` и рестартирайте. (Можете да запазите `GITHUB_TOKEN`/`MONGODB_URI` имена — програмата за стартиране ги съпоставя с `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING`, които сървърите очакват.)

> ⚠️ **Изисква се за управлявани от изкуствен интелект Task Master команди:** конфигурирайте поне един избран доставчик — `ANTHROPIC_API_KEY` за Claude, `OPENAI_API_KEY` за GPT или Codex CLI OAuth. Без идентификационни данни за модела, избран в `.taskmaster/config.json`, `/vorcl` не може да генерира или разширява задачи.
...
Codex няма „плъгини“, така че същите възможности се изразяват като **умения**, **профили** и `AGENTS.md` рутер:
...
| `SessionStart` кука | маршрутизиране на роли в `AGENTS.md` |
...
Kimi CLI няма разширение `${VAR}` в `mcp.json`, така че ключовете идват от споделеното `.env` през стартовия панел — точно както другите изпълнения. Вижте [`kimi/README.md`](./kimi/README.md).
...
**Как се вписва:** `agents/*.md` декларирайте роля и, в front-matter `skills:`, прикачете умения → уменията в `skills/*/SKILL.md` се зареждат автоматично чрез описание → `commands/<agent>/*.md` предоставят бързи `/agent:command` преки пътища, които делегират на подагента → `.mcp.json` дава на агентите техните инструменти, всеки стартиран чрез `bin/mcp-env.mjs`, който зарежда тайни от споделеното `.env`. Кука `SessionStart` казва Claude агентите са налични.
