...
Agent-Vorcl-Flow trasforma un agente di codifica supportato in un **team di ingegneri strutturato**. Invece di un assistente generale, avrai a disposizione **22 sub-agenti specializzati** (architetto, backend, frontend, Expo ingegnere mobile, DB ingegnere, cartografo di architettura, operatore liveboard e altro), ciascuno con le proprie **competenze** di dominio, rapidi **comandi barra** e gli **MCP strumenti** di cui ha bisogno. Ogni attività non banale viene eseguita attraverso un ciclo disciplinato **Task Master**: *obiettivo → attività → implementazione → verifica → fatto* – in modo che il lavoro venga pianificato, monitorato e sopravviva alle interruzioni.

- 🧩 **22 sub-agenti**, 44 abilità, 135 comandi barra
- ⚡ **Installazione con un solo comando** per Claude Code, Codex, Cursor e/o Kimi CLI — `npx`
- 🔌 **11 MCP server** collegati (GitHub, Postgres, MongoDB, Redis, Docker, Firecrawl, Vercel, Render, filesystem, Task Master, Mermaid)
...
> Il programma di installazione non inserisce mai i tuoi segreti: crea solo una `.env` vuota dal modello. Qui si aggiungono le chiavi (vedere [Configuration](#configurazione-mcp--tasti)).
...
| 🧪 **test** | Ingegnere di test e verifica | Unità (Vitest/Jest), integrazione (Supertest), E2E (Playwright), copertura, ricerca di test instabile; esegue il `testStrategy` di ogni attività: nulla viene "fatto" senza una corsa verde |
| 🌿 **gitflow** | Git flusso di lavoro e rilasci | Commit convenzionali, commit per nome (mai `git add .`), PR, Keep-a-Changelog, rilasci semestrali; push solo con conferma esplicita |
...
| 📝 **documenti** | Ingegnere della documentazione | README (parità multilingue), API documenti da OpenAPI, ARCHITETTURA, CONTRIBUTI, note di rilascio; ogni esempio verificato rispetto al codice |
...
| 📡 **tavola live** | Consiglio operativo locale | Git alberi di lavoro, processi agente e Task Master attività in tempo reale su una dashboard effimera localhost |**Alcune cose che vale la pena sapere:**
...
Ogni comando seguente è un comando barra. `<…>` contrassegna il tuo input.
...
| 🌹 | Aggiungi schema/DTO/mapper/chiavi di query e integrazione TanStack Query. |
...
| `/expo-mobile:test <scope>` | Eseguire l'unità di dominio, React Native Test della libreria e Maestro controlli. |
...
| Comando | Cosa fa |
| --- | --- |
| `/firecrawl:vorcl <goal>` | Obiettivo della ricerca tramite Task Master: raccogli dati web per ottenere un risultato finale. |
...
| `/firecrawl:setup` | Installa/verifica CLI oltre alle competenze ufficiali sulla build e sul flusso di lavoro (con conferma). |
...
| `/screenshot:convert <image> [framework]` | Genera codice eseguibile completo da uno screenshot (impostazione predefinita React + Tailwind v4). |
...
| `/pinpoint:locate <image>` | Individua i componenti/file esistenti da uno screenshot: `file:line`, nessun nuovo codice. |
...
| `/archmap:annotate [json]` | Arricchimento LLM di un `architecture.json` esistente (memoria dell'agente, semantica del flusso di dati); i fatti non dimostrati vengono retrocessi automaticamente a `inferred`. |
...
### 🧜 sirena — Mermaid diagrammi (+ render reale)
| Comando | Cosa fa |
| --- | --- |
...
| `/mermaid:convert <source> [type]` | Converte una sorgente in Mermaid — DB schema → ER, codice → classe/sequenza, cartelle → diagramma di flusso, `.drawio`/CSV/JSON. |
...
| Comando | Cosa fa |
| --- | --- |
| `/gitflow:vorcl <goal>` | Un obiettivo git/release tramite Task Master (preparare un rilascio, ripulire la cronologia, ramo funzionalità). |
...
| `/gitflow:release <version\|auto>` | Semver da commit → sincronizza le versioni manifest → tag → versione GitHub. Push solo dopo esplicita conferma. |
...
| `/docs:contributing` | CONTRIBUTING.md: configurazione, struttura, test, convenzioni di commit (allineate con `gitflow`), processo PR. |
...
| `/devops:compose` | docker-compose.yml per sviluppo locale (app + DB); I cambiamenti di env richiedono `--force-recreate`, aspettano sano. |
...
> **Perché un launcher invece di `~/.zshrc`?** L'espansione Env-var differisce in base al runtime (`${VAR:-}` in Claude, `${env:VAR}` in Cursor, valori letterali in Codex/Kimi) e ogni runtime legge solo l'ambiente **in** è stato avviato. I lanci GUI/IDE su macOS non generano `~/.zshrc`, quindi le chiavi esportate sono invisibili e i server non si connettono a nulla: il classico errore "MCP env non impostato". La lettura da un file `.env` rimuove entrambi i problemi contemporaneamente.**Precedenza** (successivamente vince): la `~/.config/agent-vorcl-flow/.env` condivisa → una `./.env` nella radice del progetto → una `export` reale nella tua shell. Mantieni le chiavi globali nel file condiviso, sovrascrivi per progetto (ad esempio un diverso `MONGODB_URI`) con un progetto `.env` e un'autentica esportazione della shell vince comunque per CLI esecuzioni. È possibile puntare il programma di avvio su un file diverso con `AGENT_VORCL_ENV_FILE=/path/.env`.

Un server a cui manca la chiave richiesta semplicemente **non si avvia**: vedrai una riga `[agent-vorcl-flow] MCP «…» is not configured: …` nel registro MCP del runtime e tutti gli altri server continuano a funzionare. Aggiungere la chiave a `.env` e riavviare. (Puoi mantenere i nomi `GITHUB_TOKEN`/`MONGODB_URI`: il programma di avvio li mappa sui `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` previsti dai server.)
...
Scegli quale fornitore Task Master esegue effettivamente la generazione; i tasti da soli non selezionano il modello:
...
Codex non ha "plugin", quindi le stesse funzionalità sono espresse come **competenze**, **profili** e un router `AGENTS.md`:
...
[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) carica in modo nativo le competenze dell'agente, i file dell'agente personalizzati e gli hook del ciclo di vita; AVF unisce anche gli stessi server MCP utilizzati da Claude e Cursor:
...
Kimi CLI non ha l'espansione `${VAR}` in `mcp.json`, quindi le chiavi provengono dalla `.env` condivisa tramite il launcher, esattamente come gli altri runtime. Vedi [`kimi/README.md`](./kimi/README.md).
...
**Come si combina:** `agents/*.md` dichiara un ruolo e, in fase iniziale `skills:`, allega competenze → le competenze in `skills/*/SKILL.md` vengono caricate automaticamente in base alla descrizione → `commands/<agent>/*.md` fornisce scorciatoie rapide `/agent:command` che delegano al sub-agente → `.mcp.json` fornisce agli agenti i propri strumenti, ciascuno avviato tramite `bin/mcp-env.mjs` che carica i segreti dalla condivisione `.env`. Un gancio `SessionStart` indica Claude che gli agenti sono disponibili.
