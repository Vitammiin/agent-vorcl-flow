...
**فريق من وكلاء الذكاء الاصطناعي الفرعيين المتخصصين في [Claude Code](https://claude.com/claude-code) و[GPT Codex](https://developers.openai.com/codex/cli/) و[Cursor](https://cursor.com/) و[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) - بالمهارات والأوامر والأدوات.**
...
Agent-Vorcl-Flow يحول وكيل الترميز المدعوم إلى **فريق هندسي منظم**. بدلاً من مساعد عام واحد، تحصل على **24 وكيلًا فرعيًا مركّزًا** (مهندس معماري، ومهندس رئيسي قائم على التعليمات البرمجية، وواجهة خلفية، وواجهة أمامية، ومهندس متنقل، ومهندس منتج وتصميم مرئي، ومهندس، ورسام خرائط معماري، ومشغل لوحة حية، والمزيد)، ولكل منهم مجاله الخاص **المهارات**، وأوامر الشرطة المائلة السريعة**، و**MCPالأدوات** التي يحتاجها. يتم تشغيل كل مهمة غير تافهة من خلال حلقة **Task Master** منضبطة — *الهدف ← المهام ← تنفيذ ← التحقق ← إنجاز* - بحيث يتم تخطيط العمل وتتبعه والتغلب على الانقطاعات.

- 🧩 **24 وكيلًا فرعيًا**، 46 مهارة، 150 أمر شرطة مائلة
...
| **Kimi CLI** | يقوم بتثبيت المهارات في `~/.kimi/skills`، والوكيل المخصص Expo الأصلي في `~/.kimi/agents`، وكلاً من Expo البنية/UI الخطافات في `~/.kimi/config.toml`، ويدمج MCP الخوادم. |
...
### عمليات التثبيت البديلة (Claude Code)
...
/audit .
```
`/vorcl` يحدد الوكيل الفرعي الذي يجب أن يمتلك العمل ويقود الدورة الكاملة Task Master. `/audit` يكتشف تلقائيًا الواجهة الخلفية والواجهة الأمامية والجوال والبيانات والبنية التحتية ويكتب مستندًا على الأدلة باستخدام جميع الأدوار ذات الصلة.
...
| 🏛️ **مهندس معماري رئيسي** | البرمجيات الرئيسية / البنية التحتية / مهندس الذكاء الاصطناعي | يقوم بمسح الكود الحقيقي بـ 11 لغة وينشئ MD وJSON وHTML وPDF وdraw.io وMermaid مدعومًا بالأدلة؛ تحديثات إعادة الفحص الكامل تحافظ على التعليقات التوضيحية |
| 🟢 **الواجهة الخلفية** | مطور الواجهة الخلفية | عقدة/TS، Postgres، Redis؛ العمارة المعيارية كل طريق مغطى بالكامل بـ OpenAPI |
...
| 🟤 **رندر** | الاستضافة والنشر (رندر) | عمليات النشر، والتشخيصات المستندة إلى السجل، والمقاييس، ومتغيرات البيئة، والعرض Postgres |
...
| 🎨 **استديو التصميم** | استوديو المنتجات والتصميم المرئي | HTML المصنوعات اليدوية المحلية، والنماذج الأولية، والإطارات السلكية، والطوابق/PPTX، والمستندات، والرسوم المتحركة، وثلاثية الأبعاد، وأنظمة التصميم واستيراد Figma/GitHub/HTML؛ مقتبس من معهد ماساتشوستس للتكنولوجيا `JimLiu/baoyu-design` |
...
| 🌿 **جيت فلو** | Git سير العمل والإصدارات | الالتزامات التقليدية، الالتزامات بالاسم (أبدًا)، العلاقات العامة، الاحتفاظ بسجل التغيير، الإصدارات المنتظمة؛ ادفع فقط بتأكيد صريح |
...
| 📝 **مستندات** | مهندس توثيق | README (تكافؤ متعدد اللغات)، API مستندات من OpenAPI، الهندسة المعمارية، المساهمة، ملاحظات الإصدار؛ تم التحقق من كل مثال مقابل الكود || 🐳 **ديفوبس** | الحاويات و CI/CD | ملفات Dockerfiles متعددة المراحل، إنشاء عامل إرساء للتطوير المحلي، GitHub خطوط أنابيب الإجراءات، نظافة البيئة/أسرار النظافة، المراقبة |
...
- **`i18n` يفرض "ترميزًا ثابتًا للغة صفر."** يكتشف الوكلاء أولاً ما إذا كان المشروع متعدد اللغات ويتكيف - تمر السلاسل التي تواجه المستخدم عبر طبقة ترجمة (next-intl / React-i18next / i18next)، ولا تكون مضمنة أبدًا.

---

##  مرجع الأمركل أمر أدناه هو أمر شرطة مائلة. `<…>` يحدد المدخلات الخاصة بك.
...
### 🏛️ المهندس المعماري الرئيسي - حزمة الهندسة المعمارية القائمة على الكود
| الأمر | ماذا يفعل |
| --- | --- |
| `/principal-architect:vorcl <goal>` | يدير هدفًا معماريًا كبيرًا من خلال Task Master والتحف التي تم التحقق منها. |
| `/principal-architect:create [options]` | يقوم بمسح المستودع الحالي وإنشاء MD، JSON، HTML، PDF، draw.io وMermaid من دليل التعليمات البرمجية. |
| `/principal-architect:update [options]` | إعادة فحص حزمة موجودة بالكامل، وكتابة دليل متباين، وتحديث العناصر التي تم إنشاؤها ذريًا. |

### 🟢 الواجهة الخلفية - الخادم (Node/TS، Postgres، Redis)
| الأمر | ماذا يفعل |
| --- | --- |
...
| | تحسين العرض / الحزمة / مؤشرات أداء الويب الأساسية. |
...
| `/expo-mobile:add-api <contract>` | أضف مفاتيح المخطط/DTO/مخطط/الاستعلام والتكامل TanStack Query. |
...
| `/firecrawl:docs-search <question>` | ابحث في الوثائق الرسمية الحالية Firecrawl. |
| `/firecrawl:integrate <feature>` | أضف Firecrawl إلى كود التطبيق عبر مهارات البناء الأولية. |
| `/firecrawl:deliverable <artifact>` | قم بإنتاج ملخص أو تدقيق أو قائمة رئيسية أو أي عناصر أخرى لسير العمل. |`/firecrawl:setup` يقوم بتشغيل التدفق الرسمي `firecrawl-cli init --all` فقط بعد التأكيد. المهارات الرسمية الموجودة لها الأولوية ويتم الحفاظ عليها بواسطة برنامج التثبيت Codex/Cursor؛ يوفر AVF إجراءات احتياطية متوافقة للمهارات المفقودة. مسار العمليات المباشرة من خلال CLI → MCP → REST/بدون مفتاح.
...
| `/screenshot:convert <image> [framework]` | قم بإنشاء كود قابل للتشغيل كاملاً من لقطة الشاشة (الافتراضي React + Tailwind v4). |
| `/screenshot:tokens <image>` | قم باستخراج رموز التصميم (ألوان OKLCH، والطباعة، والتباعد) إلى Tailwind `@theme`. |
| `/screenshot:responsive <target>` | اجعل UI مستجيبة — نقاط التوقف، السائل، `clamp()`، استعلامات الحاوية. |

### 🎨 استوديو التصميم – تصميم المنتجات والمرئيات
| الأمر | ماذا يفعل |
| --- | --- |
...
| `/design-studio:import <type> <source>` | قم باستيراد Figma `.fig`، GitHub أو HTML/CSS مع المصدر. |
| `/design-studio:deck <brief>` | أنشئ مجموعة HTML تحتوي على ملاحظات المتحدث والرسوم المتحركة وPPTX الاختيارية القابلة للتحرير. |
...
| الأمر | ماذا يفعل |
| --- | --- |
| `/pinpoint:vorcl <goal>` | ابحث عن/فهم/تغيير الموجود UI من لقطة الشاشة عبر Task Master — خريطة ← المهام ← التفويض. || `/pinpoint:locate <image>` | حدد موقع المكون/الملف (الملفات) الموجود من لقطة الشاشة — `file:line`، لا يوجد رمز جديد. |
...
| `/archmap:extract [repo]` | الاستخراج فقط — يمكن قراءته بواسطة الآلة `architecture.json` مع `source:{file,line}` على كل عقدة. |
...
| `/archmap:html [json]` | خريطة HTML تفاعلية قائمة بذاتها — تبديل الطبقة، وحزم التتبع، والعقدة → لوحة `file:line`، والبحث، وطباعة CSS. |
...
| `/docs:contributing` | CONTRIBUTING.md — الإعداد، والهيكل، والاختبارات، واتفاقيات الالتزام (المتوافقة مع `gitflow`)، وعملية العلاقات العامة. |
...
- `--root`: المشروع الذي تم فحص Git أشجار العمل والملفات Task Master الخاصة به.
...
> **لماذا المشغل بدلاً من `~/.zshrc`؟** يختلف توسيع Env-var حسب وقت التشغيل (`${VAR:-}` في Claude، `${env:VAR}` في Cursor، الأحرف الحرفية في Codex/Kimi) وكل وقت تشغيل يقرأ فقط البيئة **التي تم إطلاقها فيها**. لا يتم تشغيل GUI / IDE على نظام التشغيل macOS `~/.zshrc`، لذا تكون المفاتيح المصدرة غير مرئية ولا تتصل الخوادم بأي شيء - "MCP env not set" الكلاسيكي الفشل. تؤدي القراءة من ملف `.env` واحد إلى إزالة المشكلتين في وقت واحد.**الأسبقية** (تفوز لاحقًا): `~/.config/agent-vorcl-flow/.env` → a `./.env` المشترك في جذر المشروع → `export` الحقيقي في الصدفة الخاصة بك. احتفظ بالمفاتيح العامة في الملف المشترك، وتجاوز كل مشروع (على سبيل المثال، `MONGODB_URI` مختلف) بمشروع `.env`، وسيظل تصدير Shell الأصلي يفوز بعمليات التشغيل CLI. يمكنك توجيه المشغل إلى ملف مختلف باستخدام `AGENT_VORCL_ENV_FILE=/path/.env`.

الخادم الذي يكون مفتاحه المطلوب مفقودًا **لا يبدأ** — سترى سطرًا واحدًا `[agent-vorcl-flow] MCP «…» is not configured: …` في سجل MCP وقت التشغيل، ويستمر كل خادم آخر في العمل. أضف المفتاح إلى `.env` وأعد التشغيل. (يمكنك الاحتفاظ بأسماء `GITHUB_TOKEN`/`MONGODB_URI` — يقوم المشغل بتعيينها إلى `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` التي تتوقعها الخوادم.)

> ⚠️ **مطلوب للأوامر Task Master المدعومة بالذكاء الاصطناعي:** قم بتكوين موفر واحد محدد على الأقل — `ANTHROPIC_API_KEY` لـ Claude، أو `OPENAI_API_KEY` لـ GPT، أو Codex CLI OAuth. بدون بيانات اعتماد النموذج المحدد في `.taskmaster/config.json`، لا يمكن لـ `/vorcl` إنشاء المهام أو توسيعها.

اختر أي مزود Task Master يقوم بالفعل بتشغيل التوليد؛ المفاتيح وحدها لا تحدد النموذج:
...
Cursor يستخدم نفس التنسيق `SKILL.md` المفتوح مثل المحول Codex، بالإضافة إلى العوامل الفرعية المخصصة الأصلية والتكوين MCP العام:

| Agent-Vorcl-Flow مفهوم | Cursor يعادل |
| --- | --- |
| الدور `backend` | الوكيل الفرعي المخصص `/avf-backend` في `~/.cursor/agents` |
| أمر المهمة `/backend:create-api` | المهارة `/backend-create-api` |
...
**كيف تتناسب معًا:** `agents/*.md` أعلن دورًا، وفي المقدمة `skills:`، قم بإرفاق المهارات ← يتم تحميل المهارات الموجودة في `skills/*/SKILL.md` تلقائيًا بواسطة الوصف → `commands/<agent>/*.md` توفير اختصارات `/agent:command` سريعة تفوض إلى الوكيل الفرعي → `.mcp.json` تمنح الوكلاء أدواتهم، يبدأ كل منهم من خلال `bin/mcp-env.mjs` الذي يقوم بتحميل الأسرار من `.env` المشتركة. يشير الخطاف إلى أن الوكلاء متاحون.
