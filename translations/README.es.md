...
Agent-Vorcl-Flow convierte a un agente de codificación compatible en un **equipo de ingeniería estructurado**. En lugar de un asistente general, obtienes **24 subagentes enfocados** (arquitecto, arquitecto principal basado en código, backend, frontend, Expo ingeniero móvil, ingeniero de diseño visual y de producto, DB ingeniero, cartógrafo de arquitectura, operador de liveboard y más), cada uno con su propio dominio **habilidades**, **comandos de barra diagonal** rápidos y las **MCP herramientas** que necesita. Cada tarea no trivial se ejecuta a través de un ciclo disciplinado **Task Master** (*objetivo → tareas → implementar → verificar → hecho*) para que el trabajo se planifique, se realice un seguimiento y sobreviva a las interrupciones.
...
- 🔑 **Un archivo `.env` para todos los tiempos de ejecución**: claves leídas por un iniciador, no `~/.zshrc`, por lo que funcionan incluso desde inicios de GUI/IDE; sin servicio AVF remoto; liveboard es solo localhost y efímero
...
| **Kimi CLI** | Instala habilidades en `~/.kimi/skills`, el Expo agente personalizado nativo en `~/.kimi/agents`, ambos Expo arquitectura/UI ganchos en `~/.kimi/config.toml` y fusiona MCP servidores. |

> El instalador nunca completa sus secretos; solo crea un `.env` vacío a partir de la plantilla. Allí se añaden claves (ver [Configuration](#configuración-mcp-y-teclas)).
...
### 1. Punto de entrada universal: simplemente establezca un objetivo
```text
/vorcl add a shopping cart to checkout
/audit .
```
`/vorcl` determina qué subagente debería ser el propietario del trabajo y conduce el ciclo Task Master completo. `/audit` detecta automáticamente backend, frontend, dispositivos móviles, datos e infraestructura y escribe un `PROJECT_AUDIT.md` basado en evidencia utilizando todos los roles relevantes.

### 2. Habla con un subagente específico
```text
@agent-vorcl-flow:architect  design billing for a SaaS
@agent-vorcl-flow:backend    add a POST /invoices endpoint
```

### 3. Ejecute un comando de barra diagonal específico
```text
...
## Los agentes| Agente | Rol | Aspectos destacados |
| --- | --- | --- |
...
| 🟠 **analizador** | Auditor de código (solo lectura) | Errores, seguridad de tipos, DB estructura, simulacros de frontend, olores de backend |
| 🟡 **arrogancia** | Cobertura OpenAPI/Swagger (cualquier pila) | Encuentra rutas no totalmente documentadas y las cubre, con verificación |
...
| ⚪ **resiliencia** | Fiabilidad: errores + registro | try/catch en los límites correctos, errores escritos, reintentos/tiempos de espera, registros estructurados |
...
| 🎨 **estudio-de-diseño** | Estudio de diseño visual y de producto | Artefactos HTML locales, prototipos, wireframes, decks/PPTX, documentos, animación, 3D, sistemas de diseño e importación Figma/GitHub/HTML; adaptado del MIT `JimLiu/baoyu-design` |
...
| 🛡️ **seguridad** | Auditor de seguridad (solo lectura) | Secretos en el historial de árbol y git, OWASP Top 10, CVE de dependencia, PII; los hallazgos se convierten en tareas: las correcciones se delegan |
| 📝 **docs** | Ingeniero de documentación | README (paridad multilingüe), API documentos de OpenAPI, ARQUITECTURA, CONTRIBUCIÓN, notas de la versión; cada ejemplo verificado con el código || 🐳 **devops** | Contenedores y CI/CD | Dockerfiles de varias etapas, docker-compose para desarrollo local, GitHub Canalizaciones de acciones, higiene de entorno/secretos, monitoreo |
...
- **`archmap` nunca se basa en la imaginación.** La extracción y la representación están estrictamente separadas: los scripts de dependencia cero llevan el repositorio a `architecture.json` (bases de datos con cardinalidad FK real, API rutas, agentes de IA con sus modelos/herramientas/memoria, gráfico de importación, env), y cada diagrama se representa desde ese JSON únicamente. Todo lo que el LLM agregue sin una `file:line` verificable se marca con fuerza `inferred:true` y se dibuja con guiones.
- **`principal-architect` es el flujo de trabajo de publicación de arquitectura completo.** Funciona en cualquier repositorio que inicie el agente, ignora las afirmaciones de Markdown como evidencia de topología, utiliza el paquete WASM Tree-sitter fuera de línea para TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin y Swift, escribe `ARCHITECTURE.md` primero, luego produce el modelo JSON compartido, HTML, PDF autónomo, draw.io nativo y Mermaid copiable. L0–L4. `update` realiza una nueva exploración completa y conserva las anotaciones y los archivos no administrados.
...
## Referencia de comandoCada comando a continuación es un comando de barra diagonal. `<…>` marca tu entrada.

### `/vorcl` — enrutador universal
| Comando | Qué hace |
| --- | --- |
...
| `/audit [path] [focus]` | Auditoría profunda de funciones múltiples de solo lectura → sistemas detectados, hallazgos de seguridad/CVE/resiliencia, arquitectura de destino y `PROJECT_AUDIT.md` por fases. |
...
### 🏛️ principal-architect: paquete de arquitectura basada en código
| Comando | Qué hace |
| --- | --- |
...
### 🟢 backend — servidor (Nodo/TS, Postgres, Redis)
| Comando | Qué hace |
| --- | --- |
| `/backend:vorcl <goal>` | Objetivo → tareas → ciclo para el trabajo backend. |
| `/backend:create-api <endpoint>` | Genera un punto final API en la arquitectura modular, completamente cubierto por OpenAPI. |
...
| `/frontend:refactor <target>` | Refactor UI / ganchos sin cambiar el comportamiento. |
| | Optimice el renderizado/paquete/Core Web Vitals. |
| `/frontend:test <target>` | Generar pruebas de componentes. |

### 📱 expo-móvil — React Native / Expo| Comando | Qué hace |
| --- | --- |
...
| `/expo-mobile:create-screen <flow>` | Cree una ruta Expo Router delgada más una pantalla y estados propiedad del módulo. |
...
| `/expo-mobile:compatibility [app] [change]` | Auditoría de compatibilidad en vivo de solo lectura Expo/RN/Node/package/native-runtime contra fuentes oficiales versionadas. |
| `/expo-mobile:test <scope>` | Ejecute la unidad de dominio, React Native Biblioteca de pruebas y Maestro comprobaciones. |

### 🟠 analizador: auditoría de código (solo lectura)
| Comando | Qué hace |
| --- | --- |
...
| `/analyzer:mocks` | Encuentre datos simulados/falsos en la interfaz. |
| `/analyzer:backend` | Encuentre código backend "incorrecto": violaciones de arquitectura, lógica en los controladores. |

### 🟡 arrogancia - cobertura OpenAPI/Swagger (cualquier pila)
| Comando | Qué hace |
| --- | --- |
...
| `/swagger:cover <route>` | Cubra una ruta/módulo: parámetros, respuestas, descripciones, seguridad + verificación. |

### 🔴 firecrawl — investigación web
| Comando | Qué hace |
| --- | --- |
| `/firecrawl:vorcl <goal>` | Objetivo de investigación a través de Task Master: recopilar datos web para obtener un resultado final. |
...
| `/firecrawl:setup` | Instale/verifique CLI además de habilidades oficiales de compilación y flujo de trabajo (con confirmación). |
...
| `/firecrawl:agent <goal>` | Ejecute una tarea de Agente Firecrawl de larga duración limitada. |
...
| `/firecrawl:docs-search <question>` | Busque documentación oficial Firecrawl actual. |
| `/firecrawl:integrate <feature>` | Agregue Firecrawl al código de la aplicación mediante habilidades de compilación ascendentes. |
| `/firecrawl:deliverable <artifact>` | Produzca un resumen, una auditoría, una lista de clientes potenciales u otro artefacto del flujo de trabajo. |`/firecrawl:setup` ejecuta el flujo `firecrawl-cli init --all` oficial solo después de la confirmación. Las habilidades oficiales existentes de `firecrawl-*` tienen prioridad y las conserva el instalador de Codex/Cursor; AVF proporciona alternativas compatibles para las habilidades faltantes. Ruta de operaciones en vivo a través de CLI → MCP → REST/sin llave.

### 🟤 render — alojamiento/implementación (Renderizado)
| Comando | Qué hace |
| --- | --- |
...
| `/render:query <sql>` | SQL de solo lectura contra renderizado Postgres. |

### 🟦 base de datos — DB ingeniero / DBA (Postgres / MongoDB / Redis)
| Comando | Qué hace |
| --- | --- |
...
### ⚪ resiliencia: manejo de errores + registro
| Comando | Qué hace |
| --- | --- |
...
### 🖼️ captura de pantalla — captura de pantalla UI → código
| Comando | Qué hace |
| --- | --- |
...
### 🎨 estudio de diseño — diseño visual y de producto
| Comando | Qué hace |
| --- | --- |
...
### 🔎 investigación visual — captura de pantalla → respuesta web verificada
| Comando | Qué hace |
| --- | --- |
...
### 🎯 localizar — captura de pantalla → colocar en un proyecto existente (solo lectura)
| Comando | Qué hace |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Encuentre/comprenda/cambie UI existente desde una captura de pantalla a través de Task Master — mapa → tareas → delegar. || `/pinpoint:locate <image>` | Localice los componentes/archivos existentes en una captura de pantalla: `file:line`, sin código nuevo. |
...
| `/pinpoint:trace <target>` | Rastree la lógica detrás de un elemento: controlador → estado → obtención de datos → API. |
| `/pinpoint:handoff <change>` | Cree una solicitud de edición precisa con el código existente y delega en `frontend`/`backend`. |

### 📊 drawio — diagramas (draw.io / diagramas.net)
| Comando | Qué hace |
| --- | --- |
...
### 🗺️ archmap: mapa de arquitectura a partir del código| Comando | Qué hace |
| --- | --- |
| `/archmap:vorcl <goal>` | Un objetivo de mapeo a través de Task Master: construir en un conjunto de artefactos verificados. |
...
| `/archmap:annotate [json]` | Enriquecimiento LLM de un `architecture.json` existente (memoria del agente, semántica del flujo de datos); hechos no probados rebajados automáticamente a `inferred`. |
...
### 🧜 sirena — Mermaid diagramas (+ render real)
| Comando | Qué hace |
| --- | --- |
...
### 🧪 pruebas: pruebas y verificación
| Comando | Qué hace |
| --- | --- |
...
### 🌿 gitflow — flujo de trabajo y lanzamientos de git
| Comando | Qué hace |
| --- | --- |
...
| `/gitflow:commit <files\|scope>` | Un compromiso por nombre (nunca `git add .`) con un mensaje de compromisos convencionales; se detiene en WIP desconocido. |
...
### 🛡️ seguridad — auditoría de seguridad (solo lectura)
| Comando | Qué hace |
| --- | --- |
...
| `/docs:readme [path]` | Crear/actualizar README: what/quickstart/usage/config/troubleshooting; ejemplos verificados; Versiones de idiomas sincronizadas. || `/docs:api [spec]` | API documentos generados a partir de la especificación OpenAPI (puntos finales, parámetros, ejemplos de curl); sugiere `/swagger:audit` si no hay especificaciones. |
| `/docs:architecture` | ARCHITECTURE.md — módulos, límites, flujo de datos; diagramas delegados a `mermaid`/`drawio`. |
| `/docs:contributing` | CONTRIBUTING.md: configuración, estructura, pruebas, convenciones de confirmación (alineadas con `gitflow`), proceso de relaciones públicas. |
...
| `/devops:compose` | docker-compose.yml para desarrollo local (aplicación + bases de datos); Los cambios de entorno necesitan `--force-recreate`, espera a que esté saludable. |
...
### 📡 liveboard — tablero de operaciones locales efímeras
| Comando | Qué hace |
| --- | --- |
...
- `--interval`: intervalo de conciliación completo en milisegundos; La visualización de archivos sigue transmitiéndose y cambia inmediatamente.
...
> **¿Por qué un iniciador en lugar de `~/.zshrc`?** La expansión de Env-var difiere según el tiempo de ejecución (`${VAR:-}` en Claude, `${env:VAR}` en Cursor, literales en Codex/Kimi) y cada tiempo de ejecución lee solo el entorno en el que **fue iniciado. Los lanzamientos de GUI/IDE en macOS no generan `~/.zshrc`, por lo que las claves exportadas son invisibles y los servidores no se conectan a nada: el clásico error "MCP entorno no configurado". Leer desde un archivo `.env` elimina ambos problemas a la vez.**Precedencia** (luego gana): el `~/.config/agent-vorcl-flow/.env` compartido → un `./.env` en la raíz del proyecto → un `export` real en su shell. Mantenga las claves globales en el archivo compartido, anule por proyecto (por ejemplo, un `MONGODB_URI` diferente) con un proyecto `.env`, y una exportación de shell genuina seguirá ganando para CLI ejecuciones. Puede apuntar el iniciador a un archivo diferente con `AGENT_VORCL_ENV_FILE=/path/.env`.

Un servidor al que le falta la clave requerida simplemente **no se inicia**; verá una línea `[agent-vorcl-flow] MCP «…» is not configured: …` en el registro MCP del tiempo de ejecución y todos los demás servidores seguirán funcionando. Agregue la clave a `.env` y reinicie. (Puede conservar los nombres `GITHUB_TOKEN`/`MONGODB_URI`; el iniciador los asigna a los `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` que esperan los servidores).

> ⚠️ **Requerido para comandos Task Master impulsados por IA:** configure al menos un proveedor seleccionado: `ANTHROPIC_API_KEY` para Claude, `OPENAI_API_KEY` para GPT o Codex CLI OAuth. Sin credenciales para el modelo seleccionado en `.taskmaster/config.json`, `/vorcl` no puede generar ni expandir tareas.
...
El comando utiliza el flujo `task-master models` oficial y almacena solo la selección de modelo en `.taskmaster/config.json`. `PERPLEXITY_API_KEY` es opcional y solo es necesario cuando se selecciona Perplejidad como modelo de investigación.
...
Codex no tiene "complementos", por lo que las mismas capacidades se expresan como **habilidades**, **perfiles** y un enrutador `AGENTS.md`:
...
Cursor utiliza el mismo formato `SKILL.md` abierto que el adaptador Codex, además de subagentes personalizados nativos y configuración MCP global:
...
El instalador convierte las definiciones de funciones a Cursor frontmatter, antepone a los subagentes `avf-` para evitar colisiones entre nombres de habilidades, utiliza `model: inherit` y marca los agentes de solo auditoría como `readonly: true`. Se conservan las entradas del servidor MCP existentes con los mismos nombres. Véase [`cursor/README.md`](../cursor/README.md).
...
| archivo de claves por tiempo de ejecución | el compartido `~/.config/agent-vorcl-flow/.env` (a través del lanzador) |
