...
Agent-Vorcl-Flow biến tác nhân mã hóa được hỗ trợ thành **nhóm kỹ thuật có cấu trúc**. Thay vì một trợ lý chung, bạn có **24 đại lý phụ tập trung** (kiến trúc sư, kiến ​​trúc sư chính dựa trên mã, phụ trợ, giao diện người dùng, Expo kỹ sư di động, kỹ sư thiết kế sản phẩm và hình ảnh, DB kỹ sư, người vẽ bản đồ kiến ​​trúc, người vận hành liveboard, v.v.), mỗi người có **kỹ năng** miền riêng, **lệnh gạch chéo** nhanh và **MCP công cụ** cần thiết. Mọi nhiệm vụ không hề tầm thường đều chạy qua một vòng lặp **Task Master** có kỷ luật — *mục tiêu → nhiệm vụ → triển khai → xác minh → hoàn thành* — vì vậy công việc được lên kế hoạch, theo dõi và không bị gián đoạn.

- 🧩 **24 đặc vụ phụ**, 46 kỹ năng, 150 lệnh gạch chéo
- ⚡ **Cài đặt bằng một lệnh** cho Claude Code, Codex, Cursor, và/hoặc Kimi CLI — `npx`
- 🔌 **11 MCP máy chủ** được kết nối với (GitHub, Postgres, MongoDB, Redis, Docker, Firecrawl, Vercel, Kết xuất, hệ thống tập tin, Task Master, Mermaid)
- 🔑 **Một `.env` tệp cho tất cả thời gian chạy** — các khóa được đọc bởi trình khởi chạy chứ không phải `~/.zshrc`, vì vậy chúng hoạt động ngay cả khi khởi chạy GUI/IDE; không có dịch vụ AVF từ xa; liveboard chỉ dành cho localhost và phù du
...
| **Claude Code** | Đăng ký repo này như một plugin **marketplace** và kích hoạt plugin (thông qua `claude plugin …`, với `~/.claude/settings.json` dự phòng trực tiếp). |
...
| **Cursor** | Cài đặt các kỹ năng vào `~/.cursor/skills`, các tác nhân phụ tùy chỉnh gốc vào `~/.cursor/agents` và hợp nhất các máy chủ bị thiếu vào `~/.cursor/mcp.json`. |
...
> Trình cài đặt không bao giờ điền các bí mật của bạn — nó chỉ tạo một `.env` trống từ mẫu. Bạn thêm khóa vào đó (xem [Configuration](#cấu-hình-mcp--phím)).
...
### 1. Điểm vào chung - chỉ cần nêu mục tiêu
```text
/vorcl add a shopping cart to checkout
/audit .
```
`/vorcl` tìm ra tác nhân phụ nào sẽ sở hữu công việc và điều khiển toàn bộ chu trình Task Master. `/audit` tự động phát hiện phần phụ trợ, giao diện người dùng, thiết bị di động, dữ liệu và cơ sở hạ tầng và viết một `PROJECT_AUDIT.md` dựa trên bằng chứng bằng cách sử dụng tất cả các vai trò liên quan.

### 2. Nói chuyện với một đại lý phụ cụ thể
```text
@agent-vorcl-flow:architect  design billing for a SaaS
@agent-vorcl-flow:backend    add a POST /invoices endpoint
```

### 3. Chạy lệnh gạch chéo cụ thể
```text
...
### Vòng lặp Task Master
Mọi nhiệm vụ không hề tầm thường đều được thực hiện thông qua **Task Master** (`task-master-ai`):
...
## Các đại lý| Đại lý | Vai trò | Điểm nổi bật |
| --- | --- | --- |
...
| 🟢 **phụ trợ** | Nhà phát triển phụ trợ | Nút/TS, Postgres, Redis; kiến trúc mô-đun; mọi tuyến đường đều được bao phủ hoàn toàn bởi OpenAPI |
...
| 🟡 **vênh vang** | phạm vi bảo hiểm OpenAPI/Swagger (bất kỳ ngăn xếp nào) | Tìm các tuyến đường không được ghi lại đầy đủ và bao gồm chúng, với xác minh |
| 🔴 **bắn pháo** | Nhà nghiên cứu web | CLI/MCP/REST trực tiếp, tích hợp ứng dụng và quy trình làm việc dữ liệu web đã hoàn thiện |
| 🟤 **kết xuất** | Lưu trữ & triển khai (Render) | Triển khai, chẩn đoán dựa trên nhật ký, số liệu, biến env, Kết xuất Postgres |
...
| 🎨 **studio-thiết kế** | Studio thiết kế sản phẩm và hình ảnh | Các tạo phẩm HTML cục bộ, nguyên mẫu, wireframe, deck/PPTX, tài liệu, hoạt hình, 3D, hệ thống thiết kế và nhập Figma/GitHub/HTML; chuyển thể từ MIT `JimLiu/baoyu-design` |
...
| 🎯 **chính xác** | Ảnh chụp màn hình → đặt vào dự án hiện có (chỉ đọc) | Tạo ảnh chụp màn hình ứng dụng đang chạy trong cơ sở mã thực — thành phần, `file:line`, tuyến đường/trang, điều khiển chính xác và logic đằng sau nó; không tạo ra gì, ủy quyền chỉnh sửa |
...
| 🧜 **nàng tiên cá** | Mermaid sơ đồ (+ kết xuất thực) | sơ đồ, trình tự, lớp, trạng thái, ER, gantt, gitGraph, mindmap…; xác thực thông qua mcp-mermaid/`mmdc`; đưa cho bạn tập tin (`.mmd` + SVG/PNG/PDF) |
| 🧪 **thử nghiệm** | Kỹ sư kiểm tra và xác minh | Đơn vị (Vitest/Jest), tích hợp (Supertest), E2E (Playwright), phạm vi bao phủ, săn tìm thử nghiệm không ổn định; thực thi `testStrategy` của mỗi nhiệm vụ — không có gì được "hoàn thành" nếu không có lệnh chạy màu xanh lục |
...
| 🛡️ **bảo mật** | Kiểm toán viên bảo mật (chỉ đọc) | Bí mật trong lịch sử cây & git, Top 10 OWASP, CVE phụ thuộc, PII; những phát hiện trở thành nhiệm vụ — việc sửa lỗi được ủy quyền |
| 📝 **tài liệu** | Kỹ sư tài liệu | README (tương đương đa ngôn ngữ), API tài liệu từ OpenAPI, KIẾN TRÚC, ĐÓNG GÓP, ghi chú phát hành; mọi ví dụ được xác minh theo mã || 🐳 **devops** | Thùng chứa & CI/CD | Dockerfiles nhiều tầng, docker-compose dành cho nhà phát triển cục bộ, GitHub Quy trình hành động, vệ sinh môi trường/bí mật, giám sát |
| 📡 **bảng trực tiếp** | Ban điều hành địa phương | Git sơ đồ công việc trực tiếp, quy trình tác nhân và Task Master nhiệm vụ trên bảng điều khiển localhost nhất thời |

**Một số điều đáng biết:**
- **Giao diện người dùng luôn nói chuyện với một API thực.** Thông số kỹ thuật OpenAPI của phần phụ trợ là nguồn thông tin chính xác duy nhất; các loại được tạo ra từ nó (`openapi-typescript` + `openapi-fetch`). Không có mô hình nào trong quá trình sản xuất.
...
- **`resilience` gửi một móc an toàn.** Một móc `PostToolUse` không chặn (`catch-guard.js`) nhẹ nhàng gắn cờ các khối `catch {}` trống trong các tập tin bạn vừa chỉnh sửa.
- **`archmap` không bao giờ dựa trên trí tưởng tượng.** Quá trình trích xuất và kết xuất được tách biệt hoàn toàn: các tập lệnh không phụ thuộc đưa kho lưu trữ vào `architecture.json` (cơ sở dữ liệu với số lượng FK thực, API tuyến đường, tác nhân AI với mô hình/công cụ/bộ nhớ của chúng, biểu đồ nhập, env) và mọi sơ đồ chỉ được kết xuất từ ​​JSON đó. Bất cứ điều gì LLM thêm vào mà không có `file:line` có thể kiểm chứng đều được đánh dấu bắt buộc `inferred:true` và được vẽ nét đứt.
- **`principal-architect` là quy trình xuất bản kiến trúc đầy đủ.** Nó hoạt động trong bất kỳ kho lưu trữ nào khởi chạy tác nhân, bỏ qua các tuyên bố Markdown làm bằng chứng cấu trúc liên kết, sử dụng WASM Tree-sitter ngoại tuyến đi kèm cho TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin và Swift, viết `ARCHITECTURE.md` trước, sau đó tạo mô hình JSON được chia sẻ, HTML độc lập, PDF, draw.io gốc và có thể sao chép Mermaid L0–L4. `update` thực hiện quét lại toàn bộ và giữ lại các chú thích cũng như các tệp không được quản lý.
- **`pinpoint` tìm thấy, không bao giờ tạo.** Đưa ra ảnh chụp màn hình của một ứng dụng đang chạy, nó ánh xạ màn hình tới mã thực — thành phần, tuyến đường, điều khiển chính xác và logic đằng sau nó — và chuyển phần chỉnh sửa cho `frontend`/`backend`. Nó hoạt động trên những gì đã tồn tại (nghịch đảo của `screenshot`).
...
## Tham chiếu lệnhMọi lệnh dưới đây đều là lệnh gạch chéo. `<…>` đánh dấu đầu vào của bạn.

### `/vorcl` — bộ định tuyến đa năng
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🏛️ kiến trúc sư chính — gói kiến trúc dựa trên mã nguồn
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🟢 phụ trợ — máy chủ (Node/TS, Postgres, Redis)
| Lệnh | Nó làm gì |
| --- | --- |
...
| `/expo-mobile:compatibility [app] [change]` | Kiểm tra trực tiếp khả năng tương thích Expo/RN/Node/gói/thời gian chạy gốc chỉ đọc trực tiếp với các nguồn chính thức đã được phiên bản. |
| `/expo-mobile:test <scope>` | Chạy đơn vị miền, React Native Thư viện kiểm tra và Maestro kiểm tra. |

### 🟠 máy phân tích — kiểm tra mã (chỉ đọc)
| Lệnh | Nó làm gì |
| --- | --- |
| `/analyzer:vorcl <goal>` | Kiểm tra mục tiêu thông qua Task Master — các phát hiện trở thành nhiệm vụ. |
| `/analyzer:audit` | Kiểm tra đầy đủ: lỗi, loại, DB, mô hình giao diện người dùng, mùi phụ trợ. |
| `/analyzer:bugs` | Săn lỗi - lỗi chưa được xử lý, điều kiện chạy đua, trường hợp khó khăn. |
| `/analyzer:types` | Kiểm tra kiểu — `tsc`, `any`, chuyển hướng không an toàn, trôi dạt kiểu zod↔. |
| `/analyzer:db` | Kiểm tra DB cấu trúc — lược đồ, chỉ mục, FK, N+1, di chuyển. |
| `/analyzer:mocks` | Tìm dữ liệu mô phỏng/giả mạo trên giao diện người dùng. |
| `/analyzer:backend` | Tìm mã phụ trợ "xấu" — vi phạm kiến ​​trúc, logic trong bộ điều khiển. |

### 🟡 vênh vang — phạm vi phủ sóng OpenAPI/Swagger (bất kỳ ngăn xếp nào)
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🟦 cơ sở dữ liệu — DB kỹ sư / DBA (Postgres / MongoDB / Redis)
| Lệnh | Nó làm gì |
| --- | --- |
| `/database:vorcl <goal>` | Mục tiêu dữ liệu thông qua Task Master — lược đồ/truy vấn/di chuyển/bộ nhớ đệm cần hoàn tất. |
...
### ⚪ khả năng phục hồi — xử lý lỗi + ghi nhật ký
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🖼️ ảnh chụp màn hình — ảnh chụp màn hình UI → mã
| Lệnh | Nó làm gì |
| --- | --- |
...
| `/screenshot:responsive <target>` | Làm cho UI được tạo có tính phản hồi — điểm dừng, linh hoạt, `clamp()`, truy vấn vùng chứa. |

### 🎨 studio thiết kế — thiết kế sản phẩm và hình ảnh
| Lệnh | Nó làm gì |
| --- | --- |
...
| `/design-studio:create <brief>` | Tạo một tạo phẩm trực quan khép kín được đánh bóng hoặc hi-fi UI. |
...
| `/design-studio:deck <brief>` | Xây dựng một bộ HTML với ghi chú của người thuyết trình, hoạt ảnh và PPTX có thể chỉnh sửa tùy chọn. |
...
### 🔎 nghiên cứu trực quan — ảnh chụp màn hình → câu trả lời trên web đã được xác minh
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🎯 xác định chính xác — ảnh chụp màn hình → đặt vào dự án hiện có (chỉ đọc)
| Lệnh | Nó làm gì |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Tìm/hiểu/thay đổi UI hiện có từ ảnh chụp màn hình thông qua Task Master — bản đồ → nhiệm vụ → đại biểu. || `/pinpoint:locate <image>` | Xác định vị trí (các) thành phần/tệp hiện có từ ảnh chụp màn hình — `file:line`, không có mã mới. |
| `/pinpoint:route <image>` | Xác định tuyến/trang mà màn hình đang bật (Next.js Bộ định tuyến ứng dụng/trang, React Bộ định tuyến). |
| `/pinpoint:control <image>` | Xác định chính xác điều khiển (nút/trường) và trình xử lý của nó trong mã. |
| `/pinpoint:trace <target>` | Theo dõi logic đằng sau một phần tử — trình xử lý → trạng thái → tìm nạp dữ liệu → API. |
| `/pinpoint:handoff <change>` | Xây dựng yêu cầu chỉnh sửa chính xác dựa trên mã hiện có và ủy quyền cho `frontend`/`backend`. |

### 📊 drawio — sơ đồ (draw.io / sơ đồ.net)
| Lệnh | Nó làm gì |
| --- | --- |
...
### 🗺️ Archmap — bản đồ kiến ​​trúc từ mã| Lệnh | Nó làm gì |
| --- | --- |
...
| `/archmap:html [json]` | Bản đồ HTML tương tác khép kín — chuyển đổi lớp, theo dõi chùm, nút → bảng điều khiển, tìm kiếm, in CSS. |
| `/archmap:diagram [json] [drawio\|mermaid]` | draw.io (nhiều trang: Tổng quan / ERD / API / Đại lý) và/hoặc Mermaid lượt xem, đã được xác thực. |

### 🧜 nàng tiên cá — Mermaid sơ đồ (+ kết xuất thực)
| Lệnh | Nó làm gì |
| --- | --- |
...
| `/mermaid:convert <source> [type]` | Chuyển đổi nguồn thành Mermaid — DB lược đồ → ER, mã → lớp/trình tự, thư mục → sơ đồ, `.drawio`/CSV/JSON. |
...
| `/testing:integration <endpoint\|module>` | Kiểm tra tích hợp (Supertest/inject, real DB hoặc testcontainer). |
| `/testing:e2e <scenario>` | Playwright E2E dành cho đường dẫn người dùng quan trọng — bộ chọn vai trò, cố định, theo dõi lỗi. |
| `/testing:verify <task\|testStrategy>` | Thực thi `testStrategy` của một nhiệm vụ và trả về kết quả SẴN SÀNG / KHÔNG SẴN SÀNG với đầu ra thực. |
...
### 🌿 gitflow — quy trình làm việc và bản phát hành git
| Lệnh | Nó làm gì |
| --- | --- |
...
| `/gitflow:commit <files\|scope>` | Một cam kết theo tên (không bao giờ `git add .`) với thông báo Cam kết thông thường; dừng trên WIP không xác định. |
...
| `/gitflow:release <version\|auto>` | Phân kỳ từ các lần xác nhận → đồng bộ hóa các phiên bản bảng kê khai → thẻ → GitHub phát hành. Chỉ đẩy sau khi xác nhận rõ ràng. |
| `/gitflow:audit [branch]` | Kiểm tra lịch sử chỉ đọc: vi phạm quy ước, cam kết kết xuất, các đốm màu lớn, các nhánh mồ côi. |

### 🛡️ bảo mật — kiểm tra bảo mật (chỉ đọc)
| Lệnh | Nó làm gì |
| --- | --- |
| `/security:vorcl <goal>` | Mục tiêu bảo mật thông qua Task Master — kiểm tra → phát hiện → nhiệm vụ → các bản sửa lỗi được ủy quyền. |
...
| `/docs:readme [path]` | Tạo/cập nhật README — what/quickstart/usage/config/khắc phục sự cố; ví dụ đã được xác minh; phiên bản ngôn ngữ được đồng bộ hóa. || `/docs:api [spec]` | API tài liệu được tạo từ thông số OpenAPI (điểm cuối, thông số, ví dụ về đường cong); gợi ý `/swagger:audit` nếu không có thông số kỹ thuật. |
| `/docs:architecture` | ARCHITECTURE.md — mô-đun, ranh giới, luồng dữ liệu; sơ đồ được ủy quyền cho `mermaid`/`drawio`. |
| `/docs:contributing` | CONTRIBUTING.md — thiết lập, cấu trúc, kiểm tra, quy ước cam kết (liên kết với `gitflow`), quy trình PR. |
...
| `/devops:compose` | docker-compose.yml dành cho nhà phát triển cục bộ (ứng dụng + DB); Thay đổi env cần `--force-recreate`, chờ khỏe mạnh. |
...
### 📡 liveboard — ban điều hành địa phương phù du
| Lệnh | Nó làm gì |
| --- | --- |
| `/liveboard:start [path] [--port N] [--interval ms]` | Bắt đầu bảng điều khiển 43 ngôn ngữ tinh tế trên cổng localhost miễn phí; Task Master thay đổi luồng thông qua SSE và điều chỉnh cứ sau 5 phút. |
| `/liveboard:vorcl <goal>` | Tự phát triển hoặc thay đổi liveboard thông qua quy trình làm việc Task Master bắt buộc. |
...
- `--root`: dự án có Git cây công việc và Task Master tệp được quét.
...
> **Tại sao trình khởi chạy thay vì `~/.zshrc`?** Bản mở rộng Env-var khác nhau trong mỗi thời gian chạy (`${VAR:-}` trong Claude, `${env:VAR}` trong Cursor, chữ trong Codex/Kimi) và mỗi thời gian chạy chỉ đọc môi trường **it** được khởi chạy trong đó. GUI / IDE khởi chạy trên macOS không lấy nguồn `~/.zshrc`, do đó, các khóa đã xuất sẽ ẩn và máy chủ không kết nối với gì — lỗi "MCP env not set" cổ điển. Việc đọc từ một tệp `.env` sẽ loại bỏ cả hai vấn đề cùng một lúc.**Ưu tiên** (thắng sau): `~/.config/agent-vorcl-flow/.env` → a `./.env` được chia sẻ trong thư mục gốc của dự án → `export` thực trong shell của bạn. Giữ các khóa chung trong tệp được chia sẻ, ghi đè lên từng dự án (ví dụ: một `MONGODB_URI` khác) bằng một dự án `.env` và việc xuất shell chính hãng vẫn thắng trong CLI lần chạy. Bạn có thể trỏ trình khởi chạy tới một tệp khác bằng `AGENT_VORCL_ENV_FILE=/path/.env`.

Một máy chủ bị thiếu khóa yêu cầu chỉ **không khởi động** — bạn sẽ thấy một dòng `[agent-vorcl-flow] MCP «…» is not configured: …` trong nhật ký MCP của thời gian chạy và mọi máy chủ khác vẫn tiếp tục hoạt động. Thêm khóa vào `.env` và khởi động lại. (Bạn có thể giữ lại tên `GITHUB_TOKEN`/`MONGODB_URI` — trình khởi chạy sẽ ánh xạ chúng tới `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` mà máy chủ mong đợi.)
...
Chọn nhà cung cấp Task Master nào thực sự chạy thế hệ; riêng các phím không chọn kiểu máy:
...
Codex không có "plugin", do đó các khả năng tương tự được thể hiện dưới dạng **kỹ năng**, **hồ sơ** và bộ định tuyến `AGENTS.md`:
...
Cursor sử dụng cùng định dạng `SKILL.md` mở như bộ chuyển đổi Codex, cộng với các tác nhân phụ tùy chỉnh gốc và cấu hình MCP toàn cầu:
...
Trình cài đặt chuyển đổi các định nghĩa vai trò thành Cursor frontmatter, đặt tiền tố cho các tác nhân phụ bằng `avf-` để tránh xung đột tên kỹ năng, sử dụng `model: inherit` và đánh dấu các tác nhân chỉ kiểm tra là `readonly: true`. Các mục nhập máy chủ MCP hiện có có cùng tên sẽ được giữ nguyên. Xem [`cursor/README.md`](../cursor/README.md).

---

## Kimi CLI

[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) tải các Kỹ năng đặc vụ, các tệp tác nhân tùy chỉnh và móc vòng đời; AVF cũng hợp nhất các máy chủ MCP tương tự được sử dụng bởi Claude và Cursor:
...
| tệp khóa mỗi lần chạy | `~/.config/agent-vorcl-flow/.env` được chia sẻ (thông qua trình khởi chạy) |
...
Kimi CLI không có phần mở rộng `${VAR}` trong `mcp.json`, do đó, các khóa đến từ `.env` được chia sẻ thông qua trình khởi chạy — giống hệt như các thời gian chạy khác. Xem [`kimi/README.md`](../kimi/README.md).
