...
Agent-Vorcl-Flow превръща поддържан кодиращ агент в **структуриран инженерен екип**. Вместо един общ асистент, вие получавате **24 фокусирани под-агента** (архитект, базиран на код главен архитект, бекенд, фронтенд, Expo мобилен инженер, инженер по продуктов и визуален дизайн, DB инженер, архитектурен картограф, оператор на liveboard и други), всеки със собствени **умения** на домейн, бързи **команди с наклонена черта** и **MCP инструменти**, от които се нуждае. Всяка нетривиална задача преминава през дисциплиниран **Task Master** цикъл — *цел → задачи → изпълнение → проверка → готово* — така че работата се планира, проследява и оцелява при прекъсвания.

- 🧩 **24 подагента**, 46 умения, 150 команди с наклонена черта
...
> Инсталаторът никога не попълва вашите тайни — той само създава празен `.env` от шаблона. Там добавяте ключове (вижте [Configuration](#конфигурация-mcp-и-ключове)).
...
/audit .
```
`/vorcl` определя кой подагент трябва да притежава работата и управлява пълния Task Master цикъл. `/audit` автоматично открива бекенд, фронтенд, мобилни устройства, данни и инфраструктура и пише базиран на доказателства `PROJECT_AUDIT.md`, използвайки всички съответни роли.
...
| 🟠 **анализатор** | Одитор на код (само за четене) | Бъгове, безопасност на типа, DB структура, подигравки на интерфейса, миризми на задния край |
| 🟡 **перчене** | OpenAPI/Swagger покритие (всеки стек) | Намира маршрути, които не са напълно документирани и ги покрива с проверка |
| 🔴 **firecrawl** | Уеб изследовател | Живи CLI/MCP/REST, интеграция на приложения и завършени работни процеси за уеб данни |
...
| 🎨 **дизайн-студио** | Студио за продуктов и визуален дизайн | Местни HTML артефакти, прототипи, телени рамки, палуби/PPTX, документи, анимация, 3D, системи за проектиране и импортиране на Figma/GitHub/HTML; адаптирано от MIT `JimLiu/baoyu-design` |
...
| 📝 **документи** | Инженер по документация | README (многоезичен паритет), API документи от OpenAPI, АРХИТЕКТУРА, ПРИНОС, бележки по изданието; всеки пример, проверен спрямо кода || 🐳 **devops** | Контейнери и CI/CD | Многоетапни Dockerfiles, docker-compose за локални разработчици, GitHub Actions тръбопроводи, env/secrets хигиена, мониторинг |
...
##  Справка за командиВсяка команда по-долу е команда с наклонена черта. `<…>` маркира въведеното от вас.
...
### 🏛️ principal-architect — базиран на код архитектурен пакет
| Команда | Какво прави |
| --- | --- |
...
| `/firecrawl:agent <goal>` | Изпълнете ограничена дългосрочна задача Firecrawl Agent. |
| `/firecrawl:research <query>` | Търсете документи и GitHub изследователски контекст. |
| `/firecrawl:ask <jobId>` | Диагностицирайте неуспешна Firecrawl задача. |
| `/firecrawl:docs-search <question>` | Търсене в текущата официална Firecrawl документация. |
| `/firecrawl:integrate <feature>` | Добавете Firecrawl към кода на приложението чрез умения за изграждане нагоре по веригата. |
| `/firecrawl:deliverable <artifact>` | Създайте кратка информация, одит, списък с потенциални клиенти или друг артефакт на работния процес. |`/firecrawl:setup` изпълнява официалния поток `firecrawl-cli init --all` само след потвърждение. Съществуващите официални `firecrawl-*` умения имат предимство и се запазват от Codex/Cursor инсталатора; AVF предоставя съвместими резервни варианти за липсващи умения. Маршрут за операции на живо през CLI → MCP → REST/без ключ.

### 🟤 изобразяване — хостинг / разгръщане (изобразяване)
| Команда | Какво прави |
| --- | --- |
...
### 🎨 design-studio — продуктов и визуален дизайн
| Команда | Какво прави |
| --- | --- |
| `/design-studio:vorcl <goal>` | Пълна цел на дизайна чрез Task Master — контекст → варианти → HTML → преглед → проверка → експорт. |
...
| Команда | Какво прави |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Намерете/разберете/променете съществуващо UI от екранна снимка чрез Task Master — карта → задачи → делегат. || `/pinpoint:locate <image>` | Намерете съществуващия компонент/файл(ове) от екранна снимка — `file:line`, без нов код. |
...
| `/drawio:refine <file>` | Прецизиране на съществуващ `.drawio` — оформление, тема, добавяне/премахване на възли, подравняване към мрежата. |
...
| `/archmap:annotate [json]` | LLM обогатяване на съществуващ `architecture.json` (памет на агент, семантика на поток от данни); недоказани факти, автоматично понижени до `inferred`. |
...
| Команда | Какво прави |
| --- | --- |
| `/gitflow:vorcl <goal>` | Целта на git/release чрез Task Master (подготвяне на издание, почистване на хронологията, клон на функциите). |
...
| `/gitflow:release <version\|auto>` | Semver от ангажименти → синхронизиране на версии на манифест → етикет → GitHub издание. Push само след изрично потвърждение. |
...
| `/docs:readme [path]` | Създаване/актуализиране на README — what/quickstart/usage/config/troubleshooting; проверени примери; синхронизирани езикови версии. || `/docs:api [spec]` | API документи, генерирани от OpenAPI спецификация (крайни точки, параметри, curl примери); предлага `/swagger:audit` ако няма спец. |
...
| `/devops:compose` | docker-compose.yml за локални разработчици (приложение + DB); env промените се нуждаят от `--force-recreate`, чакат здрави. |
...
Codex няма „плъгини“, така че същите възможности са изразени като **умения**, **профили** и `AGENTS.md` рутер:
...
Инсталаторът преобразува дефинициите на роли в Cursor frontmatter, добавя подагенти с `avf-`, за да избегне сблъсъци между имена на умения, използва `model: inherit` и маркира агенти само за одит като `readonly: true`. Съществуващите MCP сървърни записи със същите имена се запазват. Вижте [`cursor/README.md`](../cursor/README.md).
...
Kimi CLI няма `${VAR}` разширение в `mcp.json`, така че ключовете идват от споделеното `.env` чрез стартовия панел — точно както другите изпълнения. Вижте [`kimi/README.md`](../kimi/README.md).
...
**Как се вписва:** `agents/*.md` декларирайте роля и, в преден план `skills:`, прикачете умения → уменията в `skills/*/SKILL.md` се зареждат автоматично чрез описание → `commands/<agent>/*.md` предоставят бързи `/agent:command` преки пътища, които делегират на подагента → `.mcp.json` дава на агентите техните инструменти, всеки стартиран чрез `bin/mcp-env.mjs`, който зарежда тайни от споделеното `.env`. Кука `SessionStart` казва Claude, че агентите са налични.
