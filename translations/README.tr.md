...
**Becerilere, komutlara ve MCP araçlara sahip [Claude Code](https://claude.com/claude-code), [GPT Codex](https://developers.openai.com/codex/cli/), [Cursor](https://cursor.com/) ve [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) için uzmanlaşmış yapay zeka alt aracılarından oluşan bir ekip.**
...
Agent-Vorcl-Flow, desteklenen bir kodlama aracısını **yapılandırılmış bir mühendislik ekibine** dönüştürür. Bir genel asistan yerine, her biri kendi etki alanı **becerilerine**, hızlı **eğik çizgi komutlarına** ve ihtiyaç duyduğu **MCP araçlara** sahip **24 odaklanmış alt aracı** (mimar, kod tabanlı baş mimar, arka uç, ön uç, Expo mobil mühendisi, ürün ve görsel tasarım mühendisi, DB mühendis, mimari haritacı, liveboard operatörü ve daha fazlası) alırsınız. Önemsiz olmayan her görev disiplinli bir **Task Master** döngüden geçer — *hedef → görevler → uygulama → doğrulama → yapıldı* — böylece iş planlanır, izlenir ve kesintilere karşı dayanıklı olur.

- 🧩 **24 alt ajan**, 46 beceri, 150 eğik çizgi komutu
...
| **Kimi CLI** | Becerileri `~/.kimi/skills`'ye, yerel Expo özel aracıyı `~/.kimi/agents`'ye yükler, her iki Expo mimarisi/UI `~/.kimi/config.toml`'ye bağlanır ve MCP sunucularını birleştirir. |
...
Ayrıca her aracının, o aracıya yönelik Task Master döngüsünü çalıştıran kendi `/<agent>:vorcl` giriş noktası vardır.
...
| 🟢 **arka uç** | Arka uç geliştiricisi | Düğüm/TS, Postgres, Redis; modüler mimari; tüm güzergahlar OpenAPI |
...
| 🎨 **tasarım stüdyosu** | Ürün ve görsel tasarım stüdyosu | Yerel HTML eserler, prototipler, tel çerçeveler, desteler/PPTX, belgeler, animasyon, 3D, tasarım sistemleri ve Figma/GitHub/HTML içe aktarımı; MIT'den uyarlanmıştır `JimLiu/baoyu-design` |
...
| 📝 **belgeler** | Dokümantasyon mühendisi | README (çoklu dil eşitliği), API OpenAPI'den dokümanlar, MİMARLIK, KATKIDA BULUNMA, sürüm notları; her örnek koda göre doğrulandı || 🐳 **devops** | Kaplar ve CI/CD | Çok Aşamalı Dockerfiles, yerel geliştirici için docker-compose, GitHub Eylem ardışık düzenleri, env/secret hijyeni, izleme |
| 📡 **canlı tahtası** | Yerel operasyonlar kurulu | Geçici bir localhost kontrol panelinde canlı Git çalışma ağaçları, aracı süreçleri ve Task Master görevler |
...
- **`archmap` hiçbir zaman hayal gücünden faydalanmaz.** Çıkarma ve oluşturma kesinlikle birbirinden ayrılmıştır: sıfır bağımlılık komut dosyaları, repo'yu `architecture.json`'ye (gerçek FK kardinalitesine sahip veritabanları, API rotalar, modelleri/araçları/belleği, içe aktarma grafiği, env'si ile AI aracıları) yürütür ve her diyagram yalnızca JSON'den oluşturulur. LLM'nin doğrulanabilir bir `file:line` olmadan eklediği her şey zorunlu olarak `inferred:true` olarak işaretlenir ve kesikli çizgiyle çizilir.
- **`principal-architect` tam mimari yayın iş akışıdır.** Aracıyı başlatan depoda çalışır, topoloji kanıtı olarak Markdown iddialarını göz ardı eder, TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin ve Swift için paketlenmiş çevrimdışı Ağaç bakıcısı WASM'yi kullanır, önce `ARCHITECTURE.md`'yi yazar, ardından paylaşılan JSON modelini üretir, bağımsız HTML, PDF, yerel çizim.io ve kopyalanabilir Mermaid L0–L4. `update` tam bir yeniden tarama gerçekleştirir ve ek açıklamaları ve yönetilmeyen dosyaları korur.
...
## Komut referansıAşağıdaki her komut bir eğik çizgi komutudur. `<…>` girişinizi işaretler.
...
### 🏛️ müdür-mimar — kod temelli mimari paketi
| Komut | Ne işe yarar |
| --- | --- |
| `/principal-architect:vorcl <goal>` | Task Master ve doğrulanmış yapılar aracılığıyla büyük bir mimari hedefini çalıştırır. |
| `/principal-architect:create [options]` | Geçerli depoyu tarar ve kod kanıtlarından MD, JSON, HTML, PDF, Draw.io ve Mermaid oluşturur. |
...
| `/firecrawl:scrape <url>` | Bir URL'yi işaretleme/JSON'ye kazıyın. |
...
| `/firecrawl:agent <goal>` | Sınırlı, uzun süreli bir Firecrawl Aracı görevi çalıştırın. |
| `/firecrawl:research <query>` | Makaleleri ve GitHub araştırma içeriğini arayın. |
| `/firecrawl:ask <jobId>` | Başarısız bir Firecrawl işi tanılayın. |
...
| `/firecrawl:deliverable <artifact>` | Özet, denetim, müşteri listesi veya başka bir iş akışı yapısı oluşturun. |`/firecrawl:setup` yalnızca onaylandıktan sonra resmi `firecrawl-cli init --all` akışını çalıştırır. Mevcut resmi `firecrawl-*` becerileri önceliklidir ve Codex/Cursor yükleyicisi tarafından korunur; AVF, eksik beceriler için uyumlu yedekler sağlar. Canlı işlemler CLI → MCP → REST/anahtarsız üzerinden yönlendirilir.
...
| `/screenshot:responsive <target>` | Oluşturulan UI'yi duyarlı hale getirin — kesme noktaları, değişken, `clamp()`, kapsayıcı sorguları. |

### 🎨 tasarım stüdyosu — ürün ve görsel tasarım
| Komut | Ne işe yarar |
| --- | --- |
...
| Komut | Ne işe yarar |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Task Master — harita → görevler → yetki verme yoluyla ekran görüntüsünden mevcut UI'yi bulun/anlayın/değiştirin. || `/pinpoint:locate <image>` | Mevcut bileşeni/dosyaları ekran görüntüsünden bulun — `file:line`, yeni kod yok. |
...
| `/archmap:extract [repo]` | Yalnızca çıkarma — her düğümde `source:{file,line}` ile makine tarafından okunabilir `architecture.json`. |
| `/archmap:annotate [json]` | Mevcut bir `architecture.json`'nin LLM zenginleştirmesi (aracı belleği, veri akışı semantiği); kanıtlanmamış gerçekler otomatik olarak `inferred`'a düşürülür. |
| `/archmap:html [json]` | Etkileşimli bağımsız HTML haritası — katman geçişleri, izleme ışınları, düğüm → `file:line` paneli, arama, CSS yazdırma. |
...
| Komut | Ne işe yarar |
| --- | --- |
| `/mermaid:vorcl <goal>` | Task Master — inşa edilecek (oluşturma doğrulanmış) aracılığıyla bir dizi diyagram. |
...
| `/mermaid:convert <source> [type]` | Kaynağı Mermaid — DB şema → ER, kod → sınıf/sıra, klasörler → akış şeması, `.drawio`/CSV/JSON'ye dönüştürün. |
...
| Komut | Ne işe yarar |
| --- | --- |
| `/gitflow:vorcl <goal>` | Task Master aracılığıyla bir git/yayın hedefi (bir sürüm hazırlama, geçmişi temizleme, özellik dalı). |
...
| `/gitflow:release <version\|auto>` | Taahhütlerden Semver → bildirim sürümlerini senkronize et → etiket → GitHub yayın. Yalnızca açıkça onaylandıktan sonra basın. |
...
| `/docs:readme [path]` | README'yi oluştur/güncelle — ne/hızlı başlangıç/kullanım/yapılandırma/sorun giderme; örnekler doğrulandı; dil sürümleri senkronize edildi. || `/docs:api [spec]` | OpenAPI spesifikasyonundan oluşturulan API dokümanlar (uç noktalar, parametreler, rotasyonel örnekler); özellik yoksa `/swagger:audit` önerilir. |
| `/docs:architecture` | ARCHITECTURE.md — modüller, sınırlar, veri akışı; `mermaid`/`drawio`'ye devredilen diyagramlar. |
...
| `/devops:compose` | yerel geliştirici için docker-compose.yml (app + DB'ler); env değişiklikleri `--force-recreate`'ye ihtiyaç duyar, sağlıklı olanı bekler. |
...
> **Neden `~/.zshrc` yerine başlatıcı?** Env-var genişletmesi çalışma zamanına göre farklılık gösterir (`${VAR:-}` Claude, `${env:VAR}` Cursor, değişmez değerler Codex/Kimi) ve her çalışma zamanı yalnızca **it**'in başlatıldığı ortamı okur. macOS'ta GUI / IDE başlatılırken `~/.zshrc` kaynaklanmaz, dolayısıyla dışa aktarılan anahtarlar görünmez ve sunucular hiçbir şeye bağlanmaz; klasik "MCP env ayarlanmadı" hatası. Tek bir `.env` dosyasından okumak her iki sorunu da aynı anda ortadan kaldırır.**Öncelik** (sonradan kazanır): paylaşılan `~/.config/agent-vorcl-flow/.env` → proje kökünde bir `./.env` → kabuğunuzda gerçek bir `export`. Genel anahtarları paylaşılan dosyada tutun, proje başına (örneğin farklı bir `MONGODB_URI`) bir proje `.env` ile geçersiz kılın; gerçek bir kabuk dışa aktarımı CLI çalıştırmalar için yine de kazanır. Başlatıcıyı `AGENT_VORCL_ENV_FILE=/path/.env` ile farklı bir dosyaya yönlendirebilirsiniz.
...
Tam haritalama için [`codex/README.md`](../codex/README.md)'ye bakın.
...
| --- | --- |
| rolü `backend` | `~/.cursor/agents`'de özel alt temsilci `/avf-backend` |
...
[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI), Aracı Becerilerini, özel aracı dosyalarını ve yaşam döngüsü kancalarını yerel olarak yükler; AVF ayrıca Claude ve Cursor tarafından kullanılan aynı MCP sunucularını birleştirir:
...
| Expo PostToolUse koruması | `~/.kimi/config.toml` |
...
Kimi CLI'de `mcp.json`'de `${VAR}` genişletmesi yoktur, bu nedenle anahtarlar, tıpkı diğer çalışma zamanlarında olduğu gibi, başlatıcı aracılığıyla paylaşılan `.env`'den gelir. [`kimi/README.md`](../kimi/README.md)'ye bakın.
...
**Birbirine nasıl uyuyor:** `agents/*.md` bir rol beyan edin ve en başta `skills:`, becerileri ekleyin → `skills/*/SKILL.md`'deki beceriler açıklamaya göre otomatik olarak yüklenir → `commands/<agent>/*.md` alt aracıya yetki veren hızlı `/agent:command` kısayolları sağlar → `.mcp.json` aracılara, her biri paylaşılan `.env`'den sırları yükleyen `bin/mcp-env.mjs` ile başlayan araçlarını verir. `SessionStart` kancası Claude'ye aracıların uygun olduğunu bildirir.
