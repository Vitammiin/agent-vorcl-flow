...
| **Kimi CLI** | Installe les compétences dans `~/.kimi/skills`, l'agent personnalisé Expo natif dans `~/.kimi/agents`, l'architecture Expo/les hooks UI dans `~/.kimi/config.toml` et fusionne les serveurs MCP. |

> L'installateur ne remplit jamais vos secrets — il crée uniquement un `.env` vide à partir du modèle. Vous y ajoutez des clés (voir [Configuration](#configuration-mcp-et-touches)).
...
| 🎨 **studio-design** | Studio de conception de produits et visuels | Artefacts HTML locaux, prototypes, wireframes, decks/PPTX, documents, animation, 3D, systèmes de conception et importation Figma/GitHub/HTML ; adapté du MIT `JimLiu/baoyu-design` |
...
| 📝 **documents** | Ingénieur documentation | README (parité multilingue), API docs de OpenAPI, ARCHITECTURE, CONTRIBUTING, notes de version ; chaque exemple vérifié par rapport au code || 🐳 **dévops** | Conteneurs & CI/CD | Dockerfiles multi-étapes, docker-compose pour le développement local, GitHub pipelines d'actions, hygiène env/secrets, surveillance |
...
## Référence de commandeChaque commande ci-dessous est une commande slash. `<…>` marque votre saisie.
...
### 🏛️ architecte principal — package d'architecture basé sur le code
| Commande | Ce qu'il fait |
| --- | --- |
...
| 🟠 | Optimiser le rendu/bundle/Core Web Vitals. |
| `/frontend:test <target>` | Générez des tests de composants. |

### 📱 expo-mobile — React Native / Expo| Commande | Ce qu'il fait |
| --- | --- |
| `/expo-mobile:vorcl <goal>` | Objectif → cycle Task Master pour Expo travail mobile. |
...
| `/expo-mobile:create-screen <flow>` | Créez un itinéraire mince plus un écran et des états appartenant au module. |
...
| `/expo-mobile:compatibility [app] [change]` | Audit de compatibilité Expo/RN/Node/package/native-runtime en direct en lecture seule par rapport aux sources officielles versionnées. |
| `/expo-mobile:test <scope>` | Exécutez l'unité de domaine, React Native Bibliothèque de tests et Maestro vérifications. |
...
| `/analyzer:db` | Structure d'audit DB — schéma, index, FK, N+1, migrations. |
...
| `/firecrawl:research <query>` | Documents de recherche et contexte de recherche. |
...
### 🎨 design-studio — design produit et visuel
| Commande | Ce qu'il fait |
| --- | --- |
...
| Commande | Ce qu'il fait |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Rechercher/comprendre/modifier les UI existants à partir d'une capture d'écran via Task Master — carte → tâches → délégué. || `/pinpoint:locate <image>` | Localisez le(s) composant(s) existant(s) à partir d'une capture d'écran — `file:line`, pas de nouveau code. |
...
| `/testing:e2e <scenario>` | Playwright E2E pour un chemin utilisateur critique — sélecteurs de rôles, dispositifs, trace en cas de panne. |
...
| Commande | Ce qu'il fait |
| --- | --- |
| `/gitflow:vorcl <goal>` | Un objectif git/release via Task Master (préparer une release, nettoyer l'historique, brancher des fonctionnalités). |
...
| `/docs:architecture` | ARCHITECTURE.md — modules, limites, flux de données ; diagrammes délégués à `mermaid`/`drawio`. |
...
| `/devops:monitoring` | Journaux structurés (pino/JSON), point final de santé, sur quoi alerter ; Rendu des métriques via l'agent `render`. |

### 📡 liveboard — tableau des opérations locales éphémère
| Commande | Ce qu'il fait |
| --- | --- |
| `/liveboard:start [path] [--port N] [--interval ms]` | Démarrez un tableau de bord raffiné en 43 langues sur un port localhost gratuit ; Task Master change de flux via SSE et se réconcilie toutes les 5 minutes. |
...
- `--root` : projet dont les Git worktrees et Task Master fichiers sont scannés.
- `--port 0` : sélectionne automatiquement un port libre.
- `--interval` : intervalle de réconciliation complet en millisecondes ; le fichier en train de regarder des flux continus Task Master change immédiatement.
...
| Expo PostToolUtilisez la garde | fusionné avec `~/.kimi/config.toml` |
...
**Comment cela s'articule :** `agents/*.md` déclarer un rôle et, en premier lieu `skills:`, attacher des compétences → les compétences dans `skills/*/SKILL.md` sont chargées automatiquement par description → `commands/<agent>/*.md` fournissent des raccourcis rapides `/agent:command` qui délèguent au sous-agent → `.mcp.json` donne aux agents leurs outils, chacun démarré via `bin/mcp-env.mjs` qui charge les secrets du `.env` partagé. Un crochet `SessionStart` indique à Claude que les agents sont disponibles.
