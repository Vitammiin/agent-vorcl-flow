...
Agent-Vorcl-Flow promění podporovaného kódovacího agenta ve **strukturovaný inženýrský tým**. Namísto jednoho obecného asistenta získáte **24 zaměřených dílčích agentů** (architekt, hlavní architekt, backend, frontend, Expo mobilní inženýr, produktový a vizuální designový inženýr, DB inženýr, kartograf architektury, operátor liveboardu a další), každý s vlastní doménou **dovednosti**, rychlé **příkazy lomítka** a nástroje** ** MCP. Každý netriviální úkol prochází disciplinovanou **Task Master** smyčkou — *cíl → úkoly → implementovat → ověřit → hotovo* — takže práce je plánována, sledována a přečkává přerušení.

- 🧩 **24 podagentů**, 46 dovedností, 150 příkazů lomítka
...
| **Kimi CLI** | Instaluje dovednosti do `~/.kimi/skills`, nativního Expo vlastního agenta do `~/.kimi/agents`, jak Expo architekturu/UI zavěšuje do `~/.kimi/config.toml`, tak slučuje MCP servery. |
...
/audit .
```
`/vorcl` zjistí, který sub-agent by měl dílo vlastnit, a řídí celý Task Master cyklus. `/audit` automaticky detekuje backend, frontend, mobilní zařízení, data a infrastrukturu a sepíše `PROJECT_AUDIT.md` založené na důkazech pomocí všech relevantních rolí.
...
Každý agent má také svůj vlastní `/<agent>:vorcl` vstupní bod, který spouští Task Master smyčku v rozsahu k danému agentovi.
...
| 🢢 **backend** | Backendový vývojář | Uzel/TS, Postgres, Redis; modulární architektura; každá trasa plně pokryta OpenAPI |
...
| 🠠 **analyzátor** | Auditor kódu (pouze pro čtení) | Chyby, typová bezpečnost, DB struktura, frontend zesměšňuje, backend pachy |
...
| 🤤 **vykreslení** | Hosting a nasazení (vykreslení) | Nasazení, diagnostika řízená protokoly, metriky, proměnné prostředí, Render Postgres |
...
| 🖼️ **snímek obrazovky** | Snímek obrazovky UI → kód | Promění snímek obrazovky UI na produkční, responzivní a přístupný kód |
...
| 🎯 **špička** | Snímek obrazovky → umístit do existujícího projektu (pouze pro čtení) | Uzemní snímek obrazovky spuštěné aplikace ve skutečné kódové základně – komponenta, `file:line`, trasa/stránka, přesné ovládání a logika za tím; nevytvoří nic, deleguje úpravy |
...
| 🧜 **mořská panna** | Mermaid diagramy (+ skutečný render) | vývojový diagram, sekvence, třída, stav, ER, Gantt, gitGraph, mapa mysli…; ověřeno přes mcp-mermaid/`mmdc`; předá vám soubor (`.mmd` + SVG/PNG/PDF) |
...
| 📝 **docs** | Dokumentační inženýr | README (vícejazyčná parita), API dokumenty z OpenAPI, ARCHITECTURE, CONTRIBUTING, poznámky k vydání; každý příklad ověřen proti kódu || 🐳 **devops** | Nádoby a CI/CD | Vícefázové soubory Dockerfiles, docker-compose pro místní vývojáře, GitHub kanály akcí, hygiena env/tajemství, monitorování |
| 📡 **živé prkno** | Místní provozní rada | Živé Git pracovní stromy, procesy agentů a Task Master úkoly na efemérním řídicím panelu localhost |
...
- **`principal-architect` je kompletní pracovní postup publikace architektury.** Funguje v jakémkoli úložišti spouští agenta, ignoruje tvrzení Markdown jako důkaz topologie, používá přibalený offline Tree-sitter WASM pro TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin a Swift, nejprve zapíše `ARCHITECTURE.md`HTML, poté vytvoří samostatný model, PDF, nativní draw.io a kopírovatelné Mermaid L0–L4. `update` provede úplné opětovné skenování a zachová anotace a nespravované soubory.
- **`pinpoint` najde, nikdy nevytvoří.** Díky snímku obrazovky spuštěné aplikace namapuje obrazovku na skutečný kód – komponentu, trasu, přesné ovládání a logiku za tím – a úpravu předá `frontend`/`backend`. Funguje na tom, co již existuje (inverzní k `screenshot`).
- **`visual-research` ověřuje místo hádání.** Zachází se snímkem obrazovky jako s důkazem, potvrzuje oficiální doménu a dokumenty, kontroluje aktuální data webu a označuje možné phishingové nebo zastaralé hodnoty.
...
##  Reference příkazuKaždý níže uvedený příkaz je lomítko. `<…>` označí váš vstup.
...
| `/audit [path] [focus]` | Hluboký vícerolový audit pouze pro čtení → detekované systémy, zjištění bezpečnosti/CVE/odolnosti, cílová architektura a fázovaná `PROJECT_AUDIT.md`. |
...
### 🏛️ hlavní architekt – balíček architektury založené na kódu
| Příkaz | Co to dělá |
| --- | --- |
...
### 📱 expo-mobile — React Native / Expo| Příkaz | Co to dělá |
| --- | --- |
...
| `/expo-mobile:create-screen <flow>` | Vytvořte tenkou Expo Router trasu plus obrazovku a stavy vlastněné modulem. |
...
### 🟡 navyšování — OpenAPI/Swagger pokrytí (jakýkoli zásobník)
| Příkaz | Co to dělá |
| --- | --- |
...
| `/firecrawl:agent <goal>` | Spusťte časově omezenou úlohu Firecrawl Agent. |
...
### 🎨 design-studio — produktový a vizuální design
| Příkaz | Co to dělá |
| --- | --- |
...
| `/design-studio:export <project> <format>` | Export do samostatného formátu HTML, PDF, PPTX, MP4 nebo do předávacího formátu. |
...
| Příkaz | Co to dělá |
| --- | --- |
| `/visual-research:vorcl <goal>` | Vícekrokový průzkum snímků obrazovky prostřednictvím Task Master. |
...
| Příkaz | Co to dělá |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Najděte/pochopte/změňte existující UI ze snímku obrazovky pomocí Task Master — mapa → úkoly → delegovat. || `/pinpoint:locate <image>` | Vyhledejte existující součást/soubor(y) ze snímku obrazovky – `file:line`, žádný nový kód. |
...
| `/archmap:html [json]` | Interaktivní samostatná HTML mapa – přepínání vrstev, trasovací paprsky, uzel → `file:line` panel, vyhledávání, tisk CSS. |
| `/archmap:diagram [json] [drawio\|mermaid]` | draw.io (více stránek: Přehled / ERD / API / Agenti) a/nebo Mermaid zobrazení, ověřeno. |

### 🧜 mořská panna — Mermaid diagramy (+ skutečné vykreslení)
| Příkaz | Co to dělá |
| --- | --- |
...
| `/testing:verify <task\|testStrategy>` | Provede úkol `testStrategy` a vrátí verdikt PŘIPRAVEN / NEPŘIPRAVEN se skutečným výstupem. |
...
| `/gitflow:release <version\|auto>` | Semver z potvrzení → synchronizace verzí manifestu → značka → GitHub vydání. Zatlačte až po výslovném potvrzení. |
...
| `/security:secrets [path\|branch]` | Tajemství pracovního stromu A historie git (všechny větve); `${VAR:-}` zástupné symboly nejsou tajemstvím. |
...
| `/devops:compose` | docker-compose.yml pro místní vývojáře (aplikace + DB); změny prostředí potřebují `--force-recreate`, čeká na zdravé. |
...
- `--interval`: úplný interval odsouhlasení v milisekundách; sledování souborů se stále streamuje Task Master se okamžitě změní.
...
> **Proč spouštěč místo `~/.zshrc`?** Rozšíření Env-var se liší podle běhového prostředí (`${VAR:-}` v Claude, `${env:VAR}` v Cursor, literály v Codex/Kimi) a každé běhové prostředí čte pouze prostředí **to** bylo spuštěno. Spouštění GUI / IDE na macOS nepřipojují nic `~/.zshrc`, klíče jsou neviditelné a neexportované k serveru env není nastaveno" selhání. Čtení z jednoho `.env` souboru odstraní oba problémy najednou.**Prednost** (později vyhrává): sdílené `~/.config/agent-vorcl-flow/.env` → a `./.env` v kořenovém adresáři projektu → skutečné `export` ve vašem shellu. Uchovávejte globální klíče ve sdíleném souboru, přepište každý projekt (např. jiný `MONGODB_URI`) projektem `.env` a po CLI spuštění stále vítězí skutečný export shellu. Spouštěč můžete nasměrovat na jiný soubor pomocí `AGENT_VORCL_ENV_FILE=/path/.env`.

Server, jehož požadovaný klíč chybí, se jednoduše **nespustí** — v protokolu MCP běhového prostředí uvidíte jednořádkový `[agent-vorcl-flow] MCP «…» is not configured: …` a každý druhý server funguje dál. Přidejte klíč do `.env` a restartujte. (Názvy `GITHUB_TOKEN`/`MONGODB_URI` si můžete ponechat – spouštěč je namapuje na `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING`, které servery očekávají.)

> ⚠️ **Vyžadováno pro Task Master příkazy poháněné AI:** nakonfigurujte alespoň jednoho vybraného poskytovatele — `ANTHROPIC_API_KEY` pro Claude, `OPENAI_API_KEY` pro GPT nebo Codex CLI OAuth. Bez přihlašovacích údajů pro model vybraný v `.taskmaster/config.json` nemůže `/vorcl` generovat ani rozšiřovat úlohy.
...
Příkaz používá oficiální `task-master models` tok a ukládá pouze výběr modelu do `.taskmaster/config.json`. `PERPLEXITY_API_KEY` je volitelná a je potřeba pouze v případě, že je jako model výzkumu vybrána Zmatenost.

Vzdálené servery **vercel** a **render** používají protokol OAuth (autorizace pomocí `/mcp` v prohlížeči). Pro Render in headless/CI nastavte `RENDER_API_KEY` ve svém prostředí a přidejte na tento server záznam záhlaví Bearer pro vaše běhové prostředí.
...
Instalační program převede definice rolí na Cursor frontmatter, předpony podagentům s `avf-`, aby se zabránilo kolizím názvů dovedností, použije `model: inherit` a označí agenty pouze pro audit jako `readonly: true`. Existující MCP záznamy serveru se stejnými názvy jsou zachovány. Viz [`cursor/README.md`](../cursor/README.md).

---

## Kimi CLI

[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) nativně načítá dovednosti agenta, vlastní soubory agentů a háčky životního cyklu; AVF také spojuje stejné MCP servery používané Claude a Cursor:
...
| Expo Ochrana PostToolUse | sloučeny do `~/.kimi/config.toml` |
...
Kimi CLI nemá žádné rozšíření `${VAR}` v `mcp.json`, takže klíče pocházejí ze sdíleného `.env` přes spouštěč – přesně jako ostatní runtime. Viz [`kimi/README.md`](../kimi/README.md).
