...
Agent-Vorcl-Flow 将受支持的编码代理转变为**结构化工程团队**。您不再需要一名总助理，而是拥有 **24 个专注的子代理**（架构师、以代码为基础的首席架构师、后端、前端、Expo 移动工程师、产品和视觉设计工程师、DB 工程师、架构制图师、现场操作员等），每个代理都有自己的领域 **技能**、快速 **斜线命令** 以及所需的 **MCP 工具**。每项重要任务都经过严格的 **Task Master** 循环 — *目标 → 任务 → 实施 → 验证 → 完成* — 因此，工作会得到计划、跟踪，并且不会受到干扰。

- 🧩 **24 个子代理**，46 种技能，150 个斜线命令
...
/audit .
```
`/vorcl` 确定哪个子代理应该拥有该工作并驱动整个 Task Master 周期。 `/audit` 自动检测后端、前端、移动、数据和基础设施，并使用所有相关角色编写基于证据的 `PROJECT_AUDIT.md`。
...
每个代理还有其自己的 `/<agent>:vorcl` 入口点，该入口点运行作用于该代理的 Task Master 循环。
...
| 🟡 **大摇大摆** | OpenAPI/Swagger 覆盖范围（任意堆栈）|查找未完整记录的路线并覆盖它们，并进行验证 |
| 🔴 **火爬行** |网络研究员|实时 CLI/MCP/REST、应用程序集成和完成的网络数据工作流程 |
| 🟤 **渲染** |托管和部署（渲染）|部署、日志驱动的诊断、指标、环境变量、渲染 Postgres |
| 🟦 **数据库** | DB 工程师/DBA |架构、查询和计划、索引、N+1、安全可逆迁移、缓存 |
...
| 🌿 **gitflow** | Git 工作流程和发布 |常规提交、按名称提交（从不 `git add .`）、PR、保留更改日志、semver 版本；仅在明确确认的情况下推送 || 🛡️ **安全** |安全审核员（只读）|树和 git 历史中的秘密、OWASP Top 10、依赖项 CVE、PII；调查结果变成任务——修复被委托
| 📝 **文档** |文档工程师|自述文件（多语言对等）、来自 OpenAPI 的 API 文档、架构、贡献、发行说明；每个示例都根据代码进行验证 |
| 🐳 **devops** |容器和 CI/CD |多阶段 Dockerfiles、用于本地开发的 docker-compose、GitHub Actions 管道、env/secrets 卫生、监控 |
| 📡 **直播板** |本地运营委员会|临时本地主机仪表板上的实时 Git 工作树、代理进程和 Task Master 任务 |

**一些值得了解的事情：**
- **前端始终与真实的 API 对话。** 后端的 OpenAPI 规范是唯一的事实来源；类型是从它生成的（`openapi-typescript` + `openapi-fetch`）。生产路径中没有模拟。
...
- **`archmap` 永远不会从想象中绘制。** 提取和渲染是严格分开的：零依赖脚本将存储库带入 `architecture.json`（具有真实 FK 基数的数据库、API 路由、AI 代理及其模型/工具/内存、导入图、env），并且每个图表仅从该 JSON 渲染。 LLM 在没有可验证的 `file:line` 的情况下添加的任何内容均强制标记为 `inferred:true` 并用虚线绘制。
...
| `/frontend:create-component <spec>` |按照特征结构生成 UI 组件。 |
...
|命令|它有什么作用 |
| --- | --- |
| `/swagger:vorcl <goal>` |通过Task Master实现全覆盖目标——审计→任务→覆盖→验证。 |
...
| `/firecrawl:deliverable <artifact>` |生成简报、审核、潜在客户列表或其他工作流程工件。 |`/firecrawl:setup`确认后才运行官方`firecrawl-cli init --all`流程。现有的官方 `firecrawl-*` 技能优先，并由 Codex/Cursor 安装程序保留； AVF 为缺失的技能提供兼容的后备方案。实时操作路线为 CLI → MCP → REST/无钥匙。
...
|命令 |它有什么作用 |
| --- | --- |
| `/screenshot:vorcl <goal>` |一组来自 Task Master 屏幕截图的屏幕 — 分解 → 代码。 |
...
### 📊drawio — 图表 (draw.io/diagrams.net)
|命令|它有什么作用 |
| --- | --- |
| `/drawio:vorcl <goal>` |通过 Task Master 绘制的一组图表 — 构建完成。 |
...
> **为什么使用启动器而不是 `~/.zshrc`？** 每个运行时的环境变量扩展都不同（Claude 中的 `${VAR:-}`，Cursor 中的 `${env:VAR}`，Codex/Kimi 中的文字），并且每个运行时仅读取它**启动的环境。macOS 上的 GUI / IDE 启动不来源 `~/.zshrc`，因此导出密钥是不可见的，服务器没有连接到任何东西——典型的“MCP env not set”失败。从一个 `.env` 文件读取可以同时解决这两个问题。
...
> ⚠️ **AI 驱动的 Task Master 命令所需：** 配置至少一个选定的提供程序 — Claude 的 `ANTHROPIC_API_KEY`、GPT 的 `OPENAI_API_KEY` 或 Codex CLI OAuth。如果没有 `.taskmaster/config.json` 中选择的模型的凭据，`/vorcl` 无法生成或扩展任务。
...
该命令使用官方`task-master models`流程，仅存储`.taskmaster/config.json`中的型号选择。 `PERPLEXITY_API_KEY` 是可选的，仅当选择 Perplexity 作为研究模型时才需要。
...
| --- | --- |
|分代理`@agent-vorcl-flow:frontend` |技能角色`$frontend` + `codex --profile frontend` |
|命令 `/analyzer:audit` |任务技能`$analyzer-audit` |
|命令`/vorcl` |任务技能`$vorcl` |
| `.mcp.json` | `config.toml` 中的 `[mcp_servers.*]` |
...
完整映射请参见 [`codex/README.md`](../codex/README.md)。

---

## Cursor

Cursor 使用与 Codex 适配器相同的开放 `SKILL.md` 格式，加上本机自定义子代理和全局 MCP 配置：
...
|通用`/vorcl` |技能`/vorcl` |
| `.mcp.json` | `~/.cursor/mcp.json` 中的合并服务器 |安装程序将角色定义转换为 Cursor frontmatter，为子代理添加 `avf-` 前缀以避免技能名称冲突，使用 `model: inherit`，并将仅审核代理标记为 `readonly: true`。保留具有相同名称的现有 MCP 服务器条目。参见 [`cursor/README.md`](../cursor/README.md)。
...
| Expo PostTool使用卫士|合并到`~/.kimi/config.toml` |
