...
**Un team di sub-agenti IA specializzati per [Claude Code](https://claude.com/claude-code), [GPT Codex](https://developers.openai.com/codex/cli/), [Cursor](https://cursor.com/) e [Kimi CLI](https://github.com/MoonshotAI/kimi-cli), con abilità, comandi e MCP strumenti.**
...
Agent-Vorcl-Flow trasforma un agente di codifica supportato in un **team tecnico strutturato**. Invece di un assistente generale, avrai a disposizione **24 sub-agenti specializzati** (architetto, architetto principale basato sul codice, backend, frontend, Expo ingegnere mobile, ingegnere di progettazione visiva e di prodotto, DB ingegnere, cartografo di architettura, operatore di liveboard e altro), ciascuno con le proprie **competenze** di dominio, rapidi **comandi barra** e gli **MCP strumenti** di cui ha bisogno. Ogni attività non banale viene eseguita attraverso un ciclo disciplinato **Task Master**: *obiettivo → attività → implementazione → verifica → fatto* – in modo che il lavoro venga pianificato, monitorato e sopravviva alle interruzioni.

- 🧩 **24 sub-agenti**, 46 abilità, 150 comandi barra
- ⚡ **Installazione con un solo comando** per Claude Code, Codex, Cursor e/o Kimi CLI — `npx`
- 🔌 **11 server MCP** collegati (GitHub, Postgres, MongoDB, Redis, Docker, Firecrawl, Vercel, Render, filesystem, Task Master, Mermaid)
...
- 🤝 **Funziona su Claude Code, GPT Codex, Cursor e Kimi CLI** dalla stessa fonte
...
> Il programma di installazione non inserisce mai i tuoi segreti: crea solo una `.env` vuota dal modello. Qui si aggiungono le chiavi (vedi [Configuration](#configurazione-mcp--tasti)).
...
### Installazioni alternative (Claude Code)
...
### 1. Punto di ingresso universale: basta indicare un obiettivo
```text
/vorcl add a shopping cart to checkout
/audit .
```
`/vorcl` stabilisce quale sub-agente dovrebbe possedere il lavoro e guida l'intero ciclo Task Master. `/audit` rileva automaticamente backend, frontend, dispositivi mobili, dati e infrastruttura e scrive un `PROJECT_AUDIT.md` basato sull'evidenza utilizzando tutti i ruoli rilevanti.

### 2. Parla con un sub-agente specifico
```text
@agent-vorcl-flow:architect  design billing for a SaaS
@agent-vorcl-flow:backend    add a POST /invoices endpoint
```

### 3. Esegui un comando barra specifico
```text
...
## Gli agenti| Agente | Ruolo | In evidenza |
| --- | --- | --- |
| 🔵 **architetto** | Architetto di sistemi e soluzioni | Analisi dei requisiti, progettazione del sistema/DB/API, revisioni dell'architettura |
...
| 🧜 **sirena** | Mermaid diagrammi (+ rendering reale) | diagramma di flusso, sequenza, classe, stato, ER, gantt, gitGraph, mappa mentale...; convalidato tramite mcp-mermaid/`mmdc`; ti porge il file (`.mmd` + SVG/PNG/PDF) |
...
| 🛡️ **sicurezza** | Revisore della sicurezza (sola lettura) | Segreti nella cronologia degli alberi e di git, OWASP Top 10, CVE delle dipendenze, PII; i risultati diventano compiti: le correzioni vengono delegate |
| 📝 **documenti** | Ingegnere della documentazione | README (parità multilingue), API documenti da OpenAPI, ARCHITETTURA, CONTRIBUTI, note sulla versione; ogni esempio verificato rispetto al codice || 🐳 **devops** | Contenitori & CI/CD | Dockerfile multistadio, docker-compose per sviluppatori locali, GitHub Pipeline di azioni, igiene ambiente/segreti, monitoraggio |
| 📡 **tavola live** | Consiglio operativo locale | Live Git alberi di lavoro, processi agente e Task Master attività su un dashboard effimero localhost |
...
- **`i18n` applica l'"hardcoding in lingua zero".** Gli agenti rilevano innanzitutto se un progetto è multilingue e si adattano: le stringhe rivolte all'utente passano attraverso un livello di traduzione (next-intl / react-i18next / i18next), mai inline.

---

## Riferimento comandoOgni comando seguente è un comando barra. `<…>` contrassegna il tuo input.

### `/vorcl` — router universale
| Comando | Cosa fa |
| --- | --- |
...
| `/audit [path] [focus]` | Audit multiruolo approfondito di sola lettura → sistemi rilevati, risultati di sicurezza/CVE/resilienza, architettura target e `PROJECT_AUDIT.md` graduale. |
...
### 🏛️principal-architect: pacchetto di architettura basato sul codice
| Comando | Cosa fa |
| --- | --- |
| `/principal-architect:vorcl <goal>` | Gestisce un obiettivo architettonico di grandi dimensioni attraverso Task Master e artefatti verificati. |
...
### 🟢 backend — server (Nodo/TS, Postgres, Redis)
| Comando | Cosa fa |
| --- | --- |
...
| `/frontend:create-component <spec>` | Genera un componente UI seguendo la struttura delle funzionalità. |
| `/frontend:refactor <target>` | Refactoring UI / hook senza modificare il comportamento. |
| 🌹 | Ottimizza rendering/raggruppamento/Core Web Vitals. |
| `/frontend:test <target>` | Genera test dei componenti. |

### 📱 expo-mobile — React Native / Expo| Comando | Cosa fa |
| --- | --- |
...
| `/expo-mobile:test <scope>`| Esegui l'unità di dominio, React Native Libreria di test e Maestro controlli. |

### 🟠 analizzatore: verifica del codice (sola lettura)
| Comando | Cosa fa |
| --- | --- |
...
| `/analyzer:db` | Struttura dell'audit DB: schema, indici, FK, N+1, migrazioni. |
| `/analyzer:mocks` | Trova mockup/dati falsi sul frontend. |
| `/analyzer:backend` | Trova codice backend "cattivo": violazioni dell'architettura, logica nei controller. |

### 🟡 spavalderia — OpenAPI/Swagger copertura (qualsiasi stack)
| Comando | Cosa fa |
| --- | --- |
...
| `/firecrawl:agent <goal>` | Eseguire un'attività limitata dell'agente Firecrawl a lunga esecuzione. |
...
| `/firecrawl:integrate <feature>` | Aggiungi Firecrawl al codice dell'applicazione tramite competenze di compilazione a monte. |
| `/firecrawl:deliverable <artifact>`| Produci un brief, un audit, un elenco di lead o altri artefatti del flusso di lavoro. |`/firecrawl:setup` esegue il flusso `firecrawl-cli init --all` ufficiale solo dopo la conferma. Le competenze ufficiali `firecrawl-*` esistenti hanno la precedenza e vengono conservate dal programma di installazione Codex/Cursor; AVF fornisce fallback compatibili per le competenze mancanti. Le operazioni live vengono eseguite tramite CLI → MCP → REST/keyless.

### 🟤 render: hosting/distribuzione (Render)
| Comando | Cosa fa |
| --- | --- |
| `/render:vorcl <goal>` | Obiettivo infra tramite Task Master: distribuzione/diagnosi/configurazione per completare. |
...
### 🟦 database — DB ingegnere / DBA (Postgres / MongoDB / Redis)
| Comando | Cosa fa |
| --- | --- |
...
### ⚪ resilienza: gestione degli errori + registrazione
| Comando | Cosa fa |
| --- | --- |
...
### 🖼️ schermata — schermata UI → codice
| Comando | Cosa fa |
| --- | --- |
...
| `/screenshot:responsive <target>` | Rendi reattivo il UI generato: query su punti di interruzione, fluide, `clamp()` e contenitori. |

### 🎨 design-studio — design del prodotto e visivo
| Comando | Cosa fa |
| --- | --- |
...
| `/design-studio:create <brief>` | Crea un artefatto visivo raffinato e autonomo o un UI hi-fi. |
...
| `/design-studio:deck <brief>` | Costruisci un deck HTML con note del relatore, animazioni e PPTX modificabile opzionale. |
...
### 🔎 ricerca visiva — screenshot → risposta web verificata
| Comando | Cosa fa |
| --- | --- |
...
### 🎯 individua — screenshot → posiziona in un progetto esistente (sola lettura)
| Comando | Cosa fa |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Trova/comprendi/modifica UI esistente da uno screenshot tramite Task Master — mappa → attività → delega. || `/pinpoint:locate <image>` | Individua i componenti/file esistenti da uno screenshot: `file:line`, nessun nuovo codice. |
| `/pinpoint:route <image>` | Identificare il percorso/pagina su cui si trova lo schermo (Next.js App/Pagine Router, React Router). |
| `/pinpoint:control <image>` | Individuare il controllo esatto (pulsante/campo) e il relativo gestore nel codice. |
| `/pinpoint:trace <target>` | Traccia la logica dietro un elemento: gestore → stato → recupero dati → API. |
| `/pinpoint:handoff <change>` | Crea una richiesta di modifica precisa rispetto al codice esistente e delega a `frontend`/`backend`. |

### 📊 drawio — diagrammi (draw.io / diagrams.net)
| Comando | Cosa fa |
| --- | --- |
...
| `/drawio:convert <source> [type]` | Convertire una sorgente in un diagramma — DB schema → ERD, cartelle → albero, codice → UML, sirena/CSV/JSON. |
| `/drawio:refine <file>` | Perfeziona una `.drawio` esistente: layout, tema, aggiungi/rimuovi nodi, allinea alla griglia. |

### 🗺️ archmap — mappa dell'architettura dal codice| Comando | Cosa fa |
| --- | --- |
...
| `/archmap:annotate [json]` | Arricchimento LLM di una `architecture.json` esistente (memoria dell'agente, semantica del flusso di dati); i fatti non dimostrati vengono retrocessi automaticamente a `inferred`. |
...
### 🧜 sirena — Mermaid diagrammi (+ rendering reale)
| Comando | Cosa fa |
| --- | --- |
| `/mermaid:vorcl <goal>` | Una serie di diagrammi tramite Task Master: build to done (render-verificato). |
...
| `/mermaid:convert <source> [type]` | Converti una sorgente in Mermaid — DB schema → ER, codice → classe/sequenza, cartelle → diagramma di flusso, `.drawio`/CSV/JSON. |
...
| `/mermaid:refine <file>` | Perfeziona una `.mmd` esistente: direzione, sottografo, classDef/stili, leggibilità. |

### 🧪 testing: test e verifiche
| Comando | Cosa fa |
| --- | --- |
...
### 🌿 gitflow: flusso di lavoro e versioni git
| Comando | Cosa fa |
| --- | --- |
...
| `/gitflow:commit <files\|scope>` | Un commit con il nome (mai `git add .`) con un messaggio di commit convenzionali; si ferma su WIP sconosciuto. |
...
| `/gitflow:release <version\|auto>` | Semver da commit → sincronizza le versioni manifest → tag → rilascio GitHub. Push solo dopo esplicita conferma. |
...
### 🛡️ sicurezza: controllo di sicurezza (sola lettura)
| Comando | Cosa fa |
| --- | --- |
...
| `/docs:readme [path]` | Crea/aggiorna README: cosa/quickstart/usage/config/troubleshooting; esempi verificati; versioni linguistiche sincronizzate. || `/docs:api [spec]` | API documenti generati dalle specifiche OpenAPI (endpoint, parametri, esempi di curl); suggerisce `/swagger:audit` se nessuna specifica. |
| `/docs:architecture` | ARCHITECTURE.md — moduli, confini, flusso di dati; diagrammi delegati a `mermaid`/`drawio`. |
| `/docs:contributing` | CONTRIBUTING.md: impostazione, struttura, test, convenzioni di commit (allineate con `gitflow`), processo PR. |
...
| `/devops:compose` | docker-compose.yml per sviluppo locale (app + DB); i cambiamenti dell'ambiente necessitano di `--force-recreate`, attendono sani. |
...
## Configurazione (MCP & tasti)

Il pacchetto **non ha backend o database remoto**. La liveboard opzionale è un processo in memoria solo localhost. MCP i server hanno bisogno di token e **ogni utente fornisce i propri**. Per fare in modo che funzioni in modo identico su **Claude Code, Codex, Cursor e Kimi CLI** — e sia che venga avviato da un terminale o da Dock/Spotlight/un IDE — ogni server stdio MCP viene avviato tramite un piccolo launcher (`bin/mcp-env.mjs`) che legge le chiavi da **un file**:
...
> **Perché un launcher invece di `~/.zshrc`?** L'espansione Env-var varia in base al runtime (`${VAR:-}` in Claude, `${env:VAR}` in Cursor, valori letterali in Codex/Kimi) e ogni runtime legge solo l'ambiente **in** è stato avviato. I lanci GUI/IDE su macOS non generano `~/.zshrc`, quindi le chiavi esportate sono invisibili e i server non si connettono a nulla: il classico errore "MCP env non impostato". La lettura di un file `.env` rimuove entrambi i problemi contemporaneamente.**Precedenza** (successivamente vince): la `~/.config/agent-vorcl-flow/.env` condivisa → una `./.env` nella radice del progetto → una `export` reale nella tua shell. Mantieni le chiavi globali nel file condiviso, sovrascrivi per progetto (ad esempio un diverso `MONGODB_URI`) con un progetto `.env` e un'esportazione autentica della shell vince comunque per CLI esecuzioni. È possibile puntare il programma di avvio su un file diverso con `AGENT_VORCL_ENV_FILE=/path/.env`.
...
Il comando utilizza il flusso ufficiale `task-master models` e memorizza solo la selezione del modello in `.taskmaster/config.json`. `PERPLEXITY_API_KEY` è facoltativo e necessario solo quando si seleziona Perplessità come modello di ricerca.
...
Codex non ha "plugin", quindi le stesse funzionalità sono espresse come **competenze**, **profili** e un `AGENTS.md` router:

| Claude Code | Codex equivalente |
| --- | --- |
| sub-agente `@agent-vorcl-flow:frontend` | persona con abilità `$frontend` + `codex --profile frontend` |
...
| `SessionStart` gancio | instradamento dei ruoli in `AGENTS.md` |
...
Cursor utilizza lo stesso formato `SKILL.md` aperto dell'adattatore Codex, oltre ad agenti secondari personalizzati nativi e configurazione MCP globale:

| Agent-Vorcl-Flow concetto | Cursor equivalente |
| --- | --- |
| ruolo `backend` | agente secondario personalizzato `/avf-backend` in `~/.cursor/agents` |
...
Il programma di installazione converte le definizioni dei ruoli in Cursor frontmatter, antepone i subagenti con `avf-` per evitare collisioni tra nomi di abilità, utilizza `model: inherit` e contrassegna gli agenti di solo controllo come `readonly: true`. Le MCP voci del server esistenti con gli stessi nomi vengono conservate. Vedere [`cursor/README.md`](../cursor/README.md).
...
| Expo agente personalizzato | `kimi --agent-file ~/.kimi/agents/avf-expo-mobile.yaml` |
| Expo PostToolUse guardia | confluito in `~/.kimi/config.toml` |
