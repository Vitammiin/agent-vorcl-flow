...
**기술, 명령, MCP 도구를 갖춘 [Claude Code](https://claude.com/claude-code), [GPT Codex](https://developers.openai.com/codex/cli/), [Cursor](https://cursor.com/), [Kimi CLI](https://github.com/MoonshotAI/kimi-cli)용 전문 AI 하위 에이전트로 구성된 팀**
...
Agent-Vorcl-Flow는 지원되는 코딩 에이전트를 **구조화된 엔지니어링 팀**으로 전환합니다. 한 명의 일반 보조자 대신 **24개의 집중 하위 에이전트**(건축가, 코드 기반 수석 건축가, 백엔드, 프런트엔드, Expo 모바일 엔지니어, 제품 및 시각 디자인 엔지니어, DB 엔지니어, 건축 지도 제작자, 라이브보드 운영자 등)가 각각 고유한 도메인 **기술**, 빠른 **슬래시 명령** 및 필요한 **MCP 도구**를 갖습니다. 모든 중요 작업은 엄격한 **Task Master** 루프(*목표 → 작업 → 구현 → 확인 → 완료*)를 통해 실행되므로 작업이 계획되고 추적되며 중단 후에도 살아남습니다.

- 🧩 **24개의 하위 에이전트**, 46개의 스킬, 150개의 슬래시 명령
- ⚡ **1 명령 설치** Claude Code, Codex, Cursor 및/또는 Kimi CLI — `npx`
...
> 설치 프로그램은 비밀 정보를 입력하지 않습니다. 템플릿에서 빈 `.env`만 생성합니다. 여기에 키를 추가합니다([Configuration](#구성mcp-및-키) 참조).
...
/audit .
```
`/vorcl` 어떤 하위 에이전트가 작업을 소유해야 하는지 파악하고 전체 Task Master 주기를 구동합니다. `/audit` 백엔드, 프런트엔드, 모바일, 데이터 및 인프라를 자동 감지하고 모든 관련 역할을 사용하여 증거 기반 `PROJECT_AUDIT.md`을 작성합니다.
...
| 🟤 **렌더링** | 호스팅 및 배포(렌더링) | 배포, 로그 기반 진단, 측정항목, 환경 변수, 렌더링 Postgres |
...
| 🎨 **디자인 스튜디오** | 제품 및 시각 디자인 스튜디오 | 로컬 HTML 가공물, 프로토타입, 와이어프레임, 데크/PPTX, 문서, 애니메이션, 3D, 디자인 시스템 및 Figma/GitHub/HTML 가져오기; MIT에서 각색됨 `JimLiu/baoyu-design` |
...
| 🗺️ **아치맵** | 건축 지도 제작자 | 결정적 코드 → `architecture.json`(`source:{file,line}`가 있는 모든 노드) → 대화형 HTML map, draw.io, Mermaid, ARCHITECTURE.md, PDF; 입증되지 않은 사실은 `inferred` |
...
| 📝 **문서** | 문서 엔지니어 | README(다중 언어 패리티), API OpenAPI 문서, 아키텍처, 기여, 릴리스 노트; 코드에 대해 확인된 모든 예제 || 🐳 **데브옵스** | 컨테이너 & CI/CD | 다단계 Dockerfile, 로컬 개발을 위한 docker-compose, GitHub 작업 파이프라인, 환경/비밀 위생, 모니터링 |
| 📡 **라이브보드** | 지역운영위원회 | 임시 로컬 호스트 대시보드에서 라이브 Git 작업 트리, 에이전트 프로세스 및 Task Master 작업 |

**알아두어야 할 몇 가지 사항:**
- **프런트엔드는 항상 실제 API와 대화합니다.** 백엔드의 OpenAPI 사양은 정보의 단일 소스입니다. 유형이 여기에서 생성됩니다(`openapi-typescript` + `openapi-fetch`). 생산 경로에 모의가 없습니다.
...
- **`archmap`는 결코 상상에서 나오지 않습니다.** 추출과 렌더링은 엄격하게 분리됩니다. 종속성이 없는 스크립트는 저장소를 `architecture.json`(실제 FK 카디널리티가 있는 데이터베이스, API 경로, 모델/도구/메모리가 있는 AI 에이전트, 가져오기 그래프, 환경)로 이동하고 모든 다이어그램은 해당 JSON에서만 렌더링됩니다. 검증 가능한 `file:line` 없이 LLM이 추가하는 모든 항목에는 강제로 `inferred:true`가 표시되고 점선이 그려집니다.
- **`principal-architect`는 전체 아키텍처 게시 워크플로우입니다.** 에이전트를 실행하는 저장소에서 작동하고, 토폴로지 증거로 Markdown 주장을 무시하고, TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin 및 Swift용 번들 오프라인 Tree-sitter WASM을 사용하고, `ARCHITECTURE.md`를 먼저 쓴 다음, 공유 JSON 모델, 자체 포함 HTML, PDF, 기본 draw.io 및 복사 가능 Mermaid를 생성합니다. L0~L4. `update` 전체 재검색을 수행하고 주석과 관리되지 않는 파일을 보존합니다.
- **`pinpoint` 찾기만 하고 절대 만들지 않습니다.** 실행 중인 앱의 스크린샷이 주어지면 화면을 실제 코드(구성 요소, 경로, 정확한 제어 및 그 뒤에 있는 논리)에 매핑하고 편집 내용을 `frontend`/`backend`에 전달합니다. 이는 이미 존재하는 항목에 대해 작동합니다(`screenshot`의 반대).
...
## 명령어 참조아래의 모든 명령은 슬래시 명령입니다. `<…>`는 입력 내용을 표시합니다.
...
### 🏛️ Principal-Architect — 코드 기반 아키텍처 패키지
| 명령 | 그것이 하는 일 |
| --- | --- |
...
| `/firecrawl:setup` | CLI와 공식 빌드 및 워크플로 기술을 설치/확인합니다(확인 포함). |
...
| `/firecrawl:deliverable <artifact>` | 개요, 감사, 리드 목록 또는 기타 워크플로 아티팩트를 생성합니다. |`/firecrawl:setup`는 확인 후에만 공식 `firecrawl-cli init --all` 흐름을 실행합니다. 기존 공식 `firecrawl-*` 기술이 우선권을 가지며 Codex/Cursor 설치 프로그램에 의해 보존됩니다. AVF는 누락된 기술에 대해 호환 가능한 대체 기능을 제공합니다. 실시간 운영 경로는 CLI → MCP → REST/keyless를 통해 이루어집니다.
...
### 🎨 design-studio — 제품 및 시각 디자인
| 명령 | 그것이 하는 일 |
| --- | --- |
| `/design-studio:vorcl <goal>` | Task Master — 컨텍스트 → 변형 → HTML → 미리보기 → 검증 → 내보내기를 통한 전체 디자인 목표. |
...
| 명령 | 그것이 하는 일 |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Task Master — 지도 → 작업 → 위임을 통해 스크린샷에서 기존 UI를 찾아 이해/변경합니다. || `/pinpoint:locate <image>` | 스크린샷에서 기존 구성요소/파일을 찾으세요 — `file:line`, 새 코드는 없습니다. |
...
| `/drawio:refine <file>` | 기존 `.drawio`를 다듬습니다 — 레이아웃, 테마, 노드 추가/제거, 그리드에 정렬. |

### 🗺️ Archmap — 코드의 아키텍처 맵| 명령 | 그것이 하는 일 |
| --- | --- |
| `/archmap:vorcl <goal>` | Task Master를 통한 매핑 목표 - 확인된 아티팩트 세트로 빌드합니다. |
| `/archmap:map [repo]` | 전체 파이프라인: 추출 → `architecture.json` → LLM 주석 → 모든 형식(HTML, draw.io, Mermaid, ARCHITECTURE.md, PDF). |
| `/archmap:extract [repo]` | 추출만 — 모든 노드에 `source:{file,line}`가 포함된 기계 판독 가능 `architecture.json`입니다. |
...
| `/gitflow:commit <files\|scope>` | Conventional Commits 메시지가 포함된 이름별 커밋( `git add .`는 안 됨) 알 수 없는 WIP에서 중지됩니다. |
| `/gitflow:pr <base> <title>` | 무엇을/왜/어떻게 검증했는지 분기 → 커밋 → 풀 요청(gh / GitHub MCP)으로 수행합니다. |
...
> **왜 `~/.zshrc` 대신 런처를 사용하나요?** Env-var 확장은 런타임마다 다르며(`${VAR:-}`는 Claude, `${env:VAR}`는 Cursor, 리터럴은 Codex/Kimi) 각 런타임은 **실행된** 환경만 읽습니다. macOS에서 GUI/IDE 실행은 `~/.zshrc`을 소스하지 않으므로 내보낸 키는 보이지 않으며 서버는 아무데도 연결되지 않습니다. 이는 전형적인 "MCP 환경이 설정되지 않음" 오류입니다. 하나의 `.env` 파일을 읽으면 두 가지 문제가 동시에 제거됩니다.**우선순위**(나중에 우선): 공유 `~/.config/agent-vorcl-flow/.env` → 프로젝트 루트의 `./.env` → 셸의 실제 `export`. 공유 파일에 전역 키를 유지하고 프로젝트별(예: 다른 `MONGODB_URI`)을 프로젝트 `.env`로 재정의하면 CLI 실행에 대해 정품 셸 내보내기가 여전히 승리합니다. `AGENT_VORCL_ENV_FILE=/path/.env`를 사용하여 실행 프로그램이 다른 파일을 가리키도록 할 수 있습니다.
...
원격 **vercel** 및 **렌더링** 서버는 OAuth를 사용합니다(브라우저에서 `/mcp`로 승인). 헤드리스/CI에서 렌더링의 경우 환경에서 `RENDER_API_KEY`를 설정하고 런타임에 대해 해당 서버에 Bearer 헤더 항목을 추가합니다.
...
전체 매핑은 [`codex/README.md`](../codex/README.md)을 참조하세요.

---

## Cursor

Cursor는 Codex 어댑터와 동일한 개방형 `SKILL.md` 형식과 기본 사용자 지정 하위 에이전트 및 전역 MCP 구성을 사용합니다.

| Agent-Vorcl-Flow 컨셉 | Cursor 상당 |
| --- | --- |
| 역할 `backend` | `~/.cursor/agents`의 사용자 정의 하위 에이전트 `/avf-backend` |
| 작업 명령 `/backend:create-api` | 기술 `/backend-create-api` |
| 보편적인 `/vorcl` | 스킬 `/vorcl` |
...
| Expo PostToolUse 가드 | `~/.kimi/config.toml`로 합병 |
...
**함께 맞추는 방법:** `agents/*.md` 역할을 선언하고 머리말 `skills:`에서 스킬 연결 → `skills/*/SKILL.md`의 스킬은 설명에 따라 자동 로드됨 → `commands/<agent>/*.md` 하위 에이전트에 위임하는 빠른 `/agent:command` 단축키 제공 → `.mcp.json`는 에이전트에게 도구를 제공하며 각각은 공유 `.env`에서 비밀을 로드하는 `bin/mcp-env.mjs`를 통해 시작됩니다. `SessionStart` 후크는 Claude 에이전트가 사용 가능함을 알려줍니다.
