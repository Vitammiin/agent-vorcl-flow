...
**O echipă de subagenți AI specializati pentru [Claude Code](https://claude.com/claude-code), [GPT Codex](https://developers.openai.com/codex/cli/), [Cursor](https://cursor.com/) și [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) — cu abilități, comenzi și instrumente MCP.**
...
Agent-Vorcl-Flow transformă un agent de codificare acceptat într-o **echipă de inginerie structurată**. În loc de un asistent general, veți obține **25 de sub-agenți concentrați** (arhitect, arhitect principal bazat pe cod, backend, frontend, Expo inginer mobil, inginer de produs și proiectare vizuală, DB inginer, auditor de integritate în mai multe limbi, cartograf de arhitectură, operator liveboard și alții), fiecare cu propriul domeniu, ** **, slash** și slash** instrumentele** de care are nevoie. Fiecare sarcină netrivială trece printr-o buclă disciplinată **Task Master** — *scop → sarcini → implementare → verificare → finalizată* — astfel încât munca este planificată, urmărită și supraviețuiește întreruperilor.

- 🧩 **25 de sub-agenți**, 73 de abilități, 155 de comenzi oblice
...
| --- | --- |
| **Strat partajat** | Copiază lansatorul în `~/.config/agent-vorcl-flow/bin/mcp-env.mjs` și creează `~/.config/agent-vorcl-flow/.env` din șablon (o dată) — fișierul cheie unică pentru fiecare timp de rulare. |
...
| **Kimi CLI** | Instalează abilitățile în `~/.kimi/skills`, agentul personalizat Expo nativ în `~/.kimi/agents`, ambele arhitectură Expo/UI se conectează în `~/.kimi/config.toml` și îmbină serverele MCP. |

> Programul de instalare nu completează niciodată secretele dvs. - creează doar un `.env` gol din șablon. Adăugați chei acolo (vezi [Configuration](#configuration-mcp--keys)).
...
Actualizarea se suprapune abilități gestionate de Agent-Vorcl-Flow, agenți, cârlige, lansator și blocuri de configurare. Vă păstrează `~/.config/agent-vorcl-flow/.env` existente și secretele sale neschimbate și păstrează abilitățile Firecrawl din amonte. Reporniți ulterior clientul de codare actualizat (sau rulați `/reload-plugins` în Claude Code).
...
Exemplele din această secțiune folosesc sintaxa Claude Code; consultați mapările [Cursor](#cursor) și [GPT Codex](#gpt-codex) de mai jos pentru sintaxa lor nativă. În Claude Code există **trei moduri** de a invoca echipa.
...
/init-code .
```
`/vorcl` dă seama care subagent ar trebui să dețină lucrarea și conduce întregul ciclu Task Master. `/audit` detectează automat backend, front-end, mobil, date și infrastructură și scrie un `PROJECT_AUDIT.md` bazat pe dovezi folosind toate rolurile relevante. `/init-code` citește depozitul static și creează un `PROJECT_DESCRIPTION.md` bazat pe dovezi fără a executa codul de proiect. Odată ce acel fișier există, fiecare rol de modificare trebuie să-și păstreze secțiunile afectate sincronizate; Descrierea dovedită blochează finalizarea sarcinii.
...
## The agents| Agent | Rol | Repere |
| --- | --- | --- |
| 🔵 **arhitect** | Arhitect de sisteme și soluții | Analiza cerințelor, proiectarea sistemului/DB/API, recenzii arhitecturii |
...
| 🟠 **analizator** | Auditor cod (numai citire) | Bug-uri, siguranța tipului, structura DB, batjocuri de front-end, mirosuri de backend |
| 🧭 **integritate** | Auditor de integritate a codului în mai multe limbi (numai citire) | Scurgeri de coduri de producție și simulare/fals/demo/fixare pe front-end/backend/mobil/partajat |
...
| 🟤 **renda** | Gazduire si implementare (Render) | Implementări, diagnosticare bazată pe jurnal, valori, vars env, Render Postgres |
...
| 🖼️ **captură de ecran** | Captură de ecran UI → cod | Transformă o captură de ecran UI în cod gata de producție, receptiv și accesibil |
...
| 🧪 **testare** | Inginer de testare și verificare | Unit (Vitest/Jest), integrare (Supertest), E2E (Playwright), acoperire, flaky-test hunting; execută `testStrategy` fiecărei sarcini — nimic nu este „făcut” fără o rulare verde |
| 🌿 **gitflow** | Git flux de lucru și versiuni | Comite convenționale, comite-uri după nume (niciodată `git add .`), PR-uri, Keep-a-Changelog, lansări semver; push numai cu confirmare explicită |
| 🛡️ **securitate** | Auditor de securitate (numai citire) | Secrete în istoria arborelui și git, OWASP Top 10, CVE-uri de dependență, PII; constatările devin sarcini — corecțiile sunt delegate || 📝 **docs** | Inginer documentare | README (multi-language parity), API documente din OpenAPI, ARHITECTURE, CONTRIBUTING, note de lansare; fiecare exemplu verificat cu codul |
| 🐳 **devops** | Containere și CI/CD | Dockerfiles în mai multe etape, docker-compose pentru dezvoltarea locală, GitHub Conducte de acțiuni, igiena mediului/secrete, monitorizare |
| 📡 **transport** | Consiliul local de operațiuni | Live Git arbori de lucru, procese de agent și Task Master sarcini pe un tablou de bord efemer localhost |

**Câteva lucruri care merită știute:**
- **Frontend-ul vorbește întotdeauna cu un API real.** Specificațiile OpenAPI ale backend-ului sunt singura sursă de adevăr; tipurile sunt generate din acesta (`openapi-typescript` + `openapi-fetch`). Fără batjocuri în calea de producție.
- **`database` mutațiile necesită o confirmare explicită.** Analytics sunt numai pentru citire; Schimbările de schemă/date (DDL/DML/migrații) nu rulează niciodată fără aprobarea dumneavoastră.
...
- **`archmap` nu se bazează niciodată din imaginație.** Extragerea și randarea sunt strict separate: scripturile cu dependență zero conduc repo-ul în `architecture.json` (baze de date cu cardinalitate FK reală, rute API, agenți AI cu modelele/instrumentele/memoria lor, graficul de import, env) și fiecare diagramă este redată numai din acel JSON. Orice adaugă LLM fără un `file:line` verificabil este marcat cu forța `inferred:true` și desenat întrerupt.
- **`principal-architect` este fluxul de lucru complet al publicării arhitecturii.** Funcționează în orice depozit care lansează agentul, ignoră revendicările Markdown ca dovezi de topologie, folosește WASM offline Tree-sitter pentru TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin și Swift, scrie mai întâi `ARCHITECTURE.md` modelul partajat, apoi produce  HTML, PDF, draw.io nativ și copiabil Mermaid L0–L4. `update` efectuează o rescanare completă și păstrează adnotările și fișierele negestionate.
- **`pinpoint` găsește, nu creează niciodată.** Având în vedere o captură de ecran a unei aplicații care rulează, mapează ecranul la codul real - componentă, traseu, controlul exact și logica din spatele acestuia - și dă editarea către `frontend`/`backend`. Funcționează pe ceea ce există deja (inversul lui `screenshot`).
...
## Command referenceFiecare comandă de mai jos este o comandă slash. `<…>` marchează intrarea dvs.
...
| `/audit [path] [focus]` | Audit profund multi-rol, numai citire → sisteme detectate, constatări de securitate/CVE/reziliență, arhitectură țintă și `PROJECT_AUDIT.md` în faze. |
| `/init-code [path] [--update]` | Descoperirea bazei de cod static → bazată pe dovezi `PROJECT_DESCRIPTION.md`; codul de proiect nu este niciodată executat. |
...
| `/frontend:refactor <target>` | Refactorizare UI / cârlige fără a schimba comportamentul. |
...
| Comanda | Ce face |
| --- | --- |
| `/expo-mobile:vorcl <goal>` | Scop → Task Master ciclu pentru Expo munca mobilă. |
...
| `/expo-mobile:test <scope>` | Rulați unitatea de domeniu, React Native Bibliotecă de testare și Maestro verificări. |
...
| `/analyzer:db` | Audit DB structură — schemă, indici, FK-uri, N+1, migrări. |
| `/analyzer:mocks` | Rută de compatibilitate pentru date simulate/false pe frontend și backend; delegă verificări poliglote profunde asupra integrității. |
| `/analyzer:backend` | Găsiți codul backend „prost” - încălcări ale arhitecturii, logica în controlere. |

### 🧭 integrity — hardcode & mock-data audit (read-only, polyglot)| Comanda | Ce face |
| --- | --- |
| `/integrity:vorcl <goal>` | Rulează un obiectiv de integritate non-trivial prin Task Master și transformă constatările în sarcini specifice proprietarului. |
| `/integrity:audit [path]` | Scanează codurile rigide și scurgeri simulate împreună, apoi dovedește accesibilitatea producției. |
| `/integrity:hardcode [path]` | Găsește literalmente utilizator/configurație/afacere care ocolesc localizarea, configurația sau sistemul de înregistrare. |
| `/integrity:mocks [path]` | Găsește cadre simulate, generatoare false, instalații, date demonstrative și răspunsuri statice accesibile din producție. |

Scanerul cu dependență zero la pachet acceptă TS/JS, Python, Go, Java/Kotlin, C#, PHP, Ruby, Rust, Vue/Svelte/HTML și Razor. Pe codul backend, semnalează, de asemenea, valorile de afaceri ascunse în constante, câmpuri statice/finale, parametri impliciti, argumente numite și cataloage statice; auditorul le compară apoi cu scheme/modele/repozitive/interogări/mutații de administrator pentru a demonstra că baza de date – nu codul sau configurația – deține valoarea. Testele, instalațiile, poveștile, exemplele, semințele, codul generat și rădăcinile furnizorului sunt suprimate implicit; candidații lexicali nu sunt defecte până când accesibilitatea și proprietatea nu sunt dovedite.
...
| `/render:query <sql>` | SQL numai pentru citire împotriva Render Postgres. |
...
| `/screenshot:responsive <target>` | Faceți ca UI generat să răspundă — puncte de întrerupere, fluide, `clamp()`, interogări container. |
...
| `/design-studio:import <type> <source>` | Importați Figma `.fig`, GitHub sau HTML/CSS cu proveniența. |
...
| Comanda | Ce face |
| --- | --- |
| `/visual-research:vorcl <goal>` | Cercetarea capturii de ecran în mai mulți pași prin Task Master. |
...
| Comanda | Ce face |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Găsiți/înțelegeți/modificați UI existente dintr-o captură de ecran prin Task Master — hartă → sarcini → delegat. |
| `/pinpoint:locate <image>` | Localizați componenta/fișierele existente dintr-o captură de ecran - `file:line`, fără cod nou. |
...
| `/pinpoint:trace <target>` | Urmăriți logica din spatele unui element — handler → stare → date-fetch → API. || `/pinpoint:handoff <change>` | Creați o solicitare de editare precisă pe baza codului existent și delegați la `frontend`/`backend`. |
...
### 🗺️ archmap — architecture map from code| Comanda | Ce face |
| --- | --- |
...
| `/archmap:html [json]` | Hartă interactivă HTML autonomă — comută layer, grinzi de urmărire, nod → panou `file:line`, căutare, imprimare CSS. |
...
| `/testing:e2e <scenario>` | Playwright E2E pentru o cale critică de utilizator — selectoare de roluri, dispozitive, urmărire la eșec. |
...
| `/security:deps` | CVE-uri de dependență prin npm fișiere de audit / blocare — severitate, steaguri de modificare a rupturii. |
...
| `/docs:readme [path]` | Creați/actualizați README — ce/quickstart/usage/config/troubleshooting; exemple verificate; versiuni de limbă sincronizate. |
| `/docs:api [spec]` | API documente generate din OpenAPI spec (puncte finale, parametri, exemple de curl); sugerează `/swagger:audit` dacă nu există specificații. |
| `/docs:architecture` | ARCHITECTURE.md — module, limite, flux de date; diagrame delegate la `mermaid`/`drawio`. || `/docs:contributing` | CONTRIBUTING.md — configurare, structură, teste, convenții de commit (aliniat cu `gitflow`), proces PR. |
| `/docs:release-notes <version>` | Note de lansare pentru o versiune din CHANGELOG/history. |
| `/docs:audit` | Docs-doar citire↔verificare declinare a codului: linkuri rupte, exemple/contoare învechite, traduceri nesincronizate. |
...
| `/devops:env` | Inventar variabil de mediu: unde se citește, ce este necesar, șablon `.env.example`; secrete niciodată în imagini. |
| `/devops:monitoring` | Jurnalele structurate (pino/JSON), punctul final de sănătate, despre ce să avertizezi; Redați valorile prin agentul `render`. |
...
| `/liveboard:vorcl <goal>` | Dezvoltați sau schimbați liveboard în sine prin fluxul de lucru Task Master necesar. |
...
Pachetul nu are **nu există backend la distanță sau bază de date**. Liveboard-ul opțional este un proces în memorie doar pentru gazdă locală. MCP serverele au nevoie de jetoane și **fiecare utilizator își oferă propriile**. Pentru ca acest lucru să funcționeze identic în **Claude Code, Codex, Cursor și Kimi CLI** — și indiferent dacă lansați de pe un terminal sau de pe Dock / Spotlight / un IDE — fiecare server stdio MCP este pornit printr-un lansator mic (`bin/mcp-env.mjs`) care vă citește cheile dintr-un fișier**:
...
> **De ce un lansator în loc de `~/.zshrc`?** Expansiunea Env-var diferă în funcție de timp de rulare (`${VAR:-}` în Claude, `${env:VAR}` în Cursor, literale în Codex/Kimi) și fiecare runtime citește doar mediul în care a fost lansat. Eroare „MCP env not set”. Citirea dintr-un fișier `.env` elimină ambele probleme simultan.

**Precedență** (câștigă mai târziu): `~/.config/agent-vorcl-flow/.env` partajat → un `./.env` în rădăcina proiectului → un `export` real în shell-ul tău. Păstrați cheile globale în fișierul partajat, suprascrieți pentru fiecare proiect (de exemplu, un alt `MONGODB_URI`) cu un proiect `.env`, iar un export shell autentic câștigă în continuare pentru CLI rulări. Puteți îndrepta lansatorul către un alt fișier cu `AGENT_VORCL_ENV_FILE=/path/.env`.Un server a cărui cheie necesară lipsește pur și simplu **nu pornește** — veți vedea o linie `[agent-vorcl-flow] MCP «…» is not configured: …` în jurnalul MCP al runtime și fiecare alt server continuă să funcționeze. Adăugați cheia la `.env` și reporniți. (Puteți păstra nume `GITHUB_TOKEN`/`MONGODB_URI` — lansatorul le mapează la `GITHUB_PERSONAL_ACCESS_TOKEN`/`MDB_MCP_CONNECTION_STRING` la care se așteaptă serverele.)
...
Alegeți ce furnizor Task Master rulează de fapt generarea; cheile singure nu selectează modelul:
...
Serverele de la distanță **vercel** și **render** folosesc OAuth (autorizare cu `/mcp` într-un browser). Pentru Render în headless/CI, setați `RENDER_API_KEY` în mediul dvs. și adăugați o intrare de antet Bearer la acel server pentru timpul de execuție.
...
Depozitul include acum un manifest plugin nativ Codex la `.codex-plugin/plugin.json`. Programul de instalare npm rămâne disponibil și instalează aceleași capabilități ca **aptitudini**, **profiluri** și un router `AGENTS.md` pentru Codex CLI, Cursor și Kimi:
...
| --- | --- |
| rolul `backend` | subagent personalizat `/avf-backend` în `~/.cursor/agents` |
...
Programul de instalare convertește definițiile rolului în Cursor frontmatter, prefixează subagenții cu `avf-` pentru a evita coliziunile numelor de abilități, folosește `model: inherit` și marchează agenții doar de audit ca `readonly: true`. Sunt păstrate MCP intrările de server existente cu aceleași nume. Vezi [`cursor/README.md`](../cursor/README.md).

---

## Kimi CLI

[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) încarcă nativ abilitățile de agent, fișierele agent personalizate și cârligele ciclului de viață; AVF combină, de asemenea, aceleași MCP servere folosite de Claude și Cursor:
...
| Expo PostToolUse garda | fuzionat în `~/.kimi/config.toml` |
...
Kimi CLI nu are o expansiune `${VAR}` în `mcp.json`, așa că cheile provin din `.env` partajat prin intermediul lansatorului — exact ca și celelalte timpi de execuție. Vezi [`kimi/README.md`](../kimi/README.md).
...
**Cum se potrivește:** `agents/*.md` declarați un rol și, în primul rând, `skills:`, atașați abilitățile → abilitățile din `skills/*/SKILL.md` sunt încărcate automat după descriere → `commands/<agent>/*.md` oferă comenzi rapide `/agent:command` care deleg sub-agentului → `.mcp.json` oferă agenților instrumentele lor, fiecare început prin `bin/mcp-env.mjs` care încarcă secretele `.env`. Un cârlig `SessionStart` spune Claude agenții sunt disponibili.
