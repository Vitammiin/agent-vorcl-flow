...
Agent-Vorcl-Flow verwandelt einen unterstützten Coding-Agenten in ein **strukturiertes Engineering-Team**. Anstelle eines allgemeinen Assistenten erhalten Sie **24 fokussierte Unteragenten** (Architekt, Code-basierter Hauptarchitekt, Backend, Frontend, Expo mobiler Ingenieur, Produkt- und Visual-Design-Ingenieur, DB Ingenieur, Architekturkartograf, Liveboard-Operator und mehr), jeder mit seinen eigenen Domänen-**Fähigkeiten**, schnellen **Slash-Befehlen** und den **MCP Tools**, die er benötigt. Jede nicht triviale Aufgabe durchläuft eine disziplinierte **Task Master**-Schleife – *Ziel → Aufgaben → Implementieren → Überprüfen → Erledigt* – damit die Arbeit geplant und nachverfolgt wird und Unterbrechungen übersteht.

- 🧩 **24 Unteragenten**, 46 Fertigkeiten, 150 Slash-Befehle
...
Das Update überlagert Agent-Vorcl-Flow-verwaltete Skills, Agents, Hooks, Launcher und Konfigurationsblöcke. Es behält Ihr bestehendes `~/.config/agent-vorcl-flow/.env` und seine Geheimnisse bei und bewahrt die vorgelagerten Firecrawl-Fähigkeiten. Starten Sie anschließend den aktualisierten Coding-Client neu (oder führen Sie `/reload-plugins` in Claude Code aus).
...
| 🎨 **Designstudio** | Studio für Produkt- und visuelles Design | Lokale HTML Artefakte, Prototypen, Wireframes, Decks/PPTX, Dokumente, Animation, 3D, Designsysteme und Figma/GitHub/HTML-Import; adaptiert vom MIT `JimLiu/baoyu-design` |
...
| 🧜 **Meerjungfrau** | Mermaid Diagramme (+ reales Rendering) | Flussdiagramm, Sequenz, Klasse, Zustand, ER, Gantt, GitGraph, Mindmap…; validiert über mcp-mermaid/`mmdc`; gibt dir die Datei (`.mmd` + SVG/PNG/PDF) |
...
| 📝 **Dokumente** | Dokumentationsingenieur | README (mehrsprachige Parität), API-Dokumente von OpenAPI, ARCHITEKTUR, BEITRAG, Versionshinweise; jedes Beispiel anhand des Codes überprüft || 🐳 **Entwickler** | Container & CI/CD | Mehrstufige Docker-Dateien, Docker-Compose für lokale Entwicklung, GitHub Aktionspipelines, Env/Secrets-Hygiene, Überwachung |
...
- **`resilience` wird mit einem Sicherheitshaken geliefert.** Ein nicht blockierender `PostToolUse`-Haken (`catch-guard.js`) markiert sanft leere `catch {}`-Blöcke in Dateien, die Sie gerade bearbeitet haben.
- **`archmap` schöpft nie aus der Fantasie.** Extraktion und Rendering sind strikt getrennt: Null-Abhängigkeits-Skripte leiten das Repo in `architecture.json` (Datenbanken mit echter FK-Kardinalität, API-Routen, KI-Agenten mit ihren Modellen/Tools/Speicher, Importdiagramm, Umgebung), und jedes Diagramm wird nur aus diesem JSON gerendert. Alles, was das LLM ohne überprüfbares `file:line` hinzufügt, wird mit `inferred:true` zwangsmarkiert und gestrichelt dargestellt.
- **`principal-architect` ist der vollständige Architektur-Veröffentlichungsworkflow.** Er funktioniert in jedem Repository, in dem der Agent gestartet wird, ignoriert Markdown-Ansprüche als Topologiebeweis, verwendet gebündeltes Offline-Tree-Sitter-WASM für TS/JS, Python, Go, Java, C#, Rust, PHP, Ruby, Kotlin und Swift, schreibt zuerst `ARCHITECTURE.md` und erzeugt dann das gemeinsam genutzte JSON-Modell, eigenständig HTML, PDF, natives draw.io und kopierbar Mermaid L0–L4. `update` führt einen vollständigen Neuscan durch und behält Anmerkungen und nicht verwaltete Dateien bei.
...
## BefehlsreferenzJeder Befehl unten ist ein Schrägstrich-Befehl. `<…>` markiert Ihre Eingabe.

### `/vorcl` – Universalfräse
| Befehl | Was es tut |
| --- | --- |
...
### 🏛️ Principal-Architect – Code-basiertes Architekturpaket
| Befehl | Was es tut |
| --- | --- |
...
| 😉 | Optimieren Sie Rendering/Bundle/Core Web Vitals. |
...
| Befehl | Was es tut |
| --- | --- |
| `/swagger:vorcl <goal>` | Vollständiges Abdeckungsziel durch Task Master – Audit → Aufgaben → Abdeckung → Verifizieren. |
...
| `/firecrawl:setup` | Installieren/verifizieren Sie CLI plus offizielle Build- und Workflow-Kenntnisse (mit Bestätigung). |
...
| `/firecrawl:ask <jobId>` | Diagnostizieren Sie einen fehlgeschlagenen Job. |
...
### 🎨 Design-Studio – Produkt- und visuelles Design
| Befehl | Was es tut |
| --- | --- |
...
| Befehl | Was es tut |
| --- | --- |
| `/pinpoint:vorcl <goal>` | Vorhandenes UI aus einem Screenshot finden/verstehen/ändern über Task Master – Karte → Aufgaben → delegieren. || `/pinpoint:locate <image>` | Suchen Sie die vorhandene(n) Komponente/Datei(en) anhand eines Screenshots – `file:line`, kein neuer Code. |
...
| --- | --- |
| `/archmap:vorcl <goal>` | Ein Mapping-Ziel über Task Master – Aufbau zu einem verifizierten Artefaktsatz. |
...
| `/archmap:annotate [json]` | LLM-Anreicherung eines vorhandenen `architecture.json` (Agentenspeicher, Datenflusssemantik); unbewiesene Tatsachen automatisch auf `inferred` herabgestuft. |
...
| `/security:deps` | Abhängigkeits-CVEs über npm Audit-/Sperrdateien – Schweregrad, Breaking-Change-Flags. |
...
> **Warum ein Launcher anstelle von `~/.zshrc`?** Die Env-Var-Erweiterung unterscheidet sich je nach Laufzeit (`${VAR:-}` in Claude, `${env:VAR}` in Cursor, Literale in Codex/Kimi) und jede Laufzeit liest nur die Umgebung, in der sie gestartet wurde. GUI-/IDE-Starts unter macOS enthalten kein `~/.zshrc`, daher sind exportierte Schlüssel unsichtbar und die Server stellen keine Verbindung her – der klassische Fehler „MCP Umgebung nicht festgelegt“. Das Lesen aus einer `.env`-Datei beseitigt beide Probleme gleichzeitig.**Vorrang** (später gewinnt): das gemeinsame `~/.config/agent-vorcl-flow/.env` → ein `./.env` im Projektstamm → ein echtes `export` in Ihrer Shell. Behalten Sie globale Schlüssel in der freigegebenen Datei, überschreiben Sie jedes Projekt (z. B. ein anderes `MONGODB_URI`) mit einem Projekt `.env`, und ein echter Shell-Export gewinnt immer noch für CLI Läufe. Sie können den Launcher mit `AGENT_VORCL_ENV_FILE=/path/.env` auf eine andere Datei richten.
...
Wählen Sie aus, welcher Anbieter die Generierung tatsächlich durchführt; Tasten allein wählen das Modell nicht aus:
...
Der Befehl verwendet den offiziellen `task-master models`-Ablauf und speichert nur die Modellauswahl in `.taskmaster/config.json`. `PERPLEXITY_API_KEY` ist optional und wird nur benötigt, wenn Perplexity als Forschungsmodell ausgewählt wird.
...
Codex hat keine „Plugins“, daher werden die gleichen Fähigkeiten als **Fähigkeiten**, **Profile** und ein `AGENTS.md` Router ausgedrückt:
...
| Expo PostToolUse Guard | verschmolzen zu `~/.kimi/config.toml` |
...
**Wie es zusammenpasst:** `agents/*.md` Deklarieren Sie eine Rolle und fügen Sie im Vorfeld `skills:` Fertigkeiten hinzu → Fertigkeiten in `skills/*/SKILL.md` werden durch Beschreibung automatisch geladen → `commands/<agent>/*.md` stellen schnelle `/agent:command` Verknüpfungen bereit, die an den Unteragenten delegieren → `.mcp.json` gibt Agenten ihre Werkzeuge, die jeweils durch `bin/mcp-env.mjs` gestartet werden, wodurch Geheimnisse aus dem gemeinsam genutzten `.env` geladen werden. Ein `SessionStart`-Haken zeigt Claude an, dass die Agenten verfügbar sind.
