...
## Без «тихих» падений
Пустой `catch {}` запрещён — в catch: обработать + лог, пробросить (`throw new AppError(msg, { cause })`), либо преобразовать в доменную ошибку. Только `Error`/наследники, сохраняй `cause`/`stack`. В TS `catch (e: unknown)` — сузь тип.
...
## Фронт / бэк
React — error boundary + ошибки данных на уровне запроса (не пустой экран/стек пользователю). Node — `setErrorHandler`, graceful shutdown.
