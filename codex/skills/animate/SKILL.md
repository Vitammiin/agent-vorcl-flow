...
## Agent-Vorcl-Flow integration

Treat `review-animations`, `improve-animations`, `find-animation-opportunities`, and `pick-ui-library` as optional companion skills. Before routing to one, verify that it is installed. If it is unavailable, do not invent the invocation: report the missing companion briefly and route the request through the selected `$frontend` or `$design-studio` role. For a component request, prefer an accessible primitive already used by the project; if none exists and `pick-ui-library` is unavailable, ask before adding a new dependency.
