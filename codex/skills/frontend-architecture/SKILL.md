...
- Словари — централизованно (`messages/<locale>.json`) или на уровне фичи (`features/<feature>/locales/`), единый подход на проект; компоненты берут текст через слой перевода, не хардкодят строки в JSX.
- Определи мультиязычность проекта и адаптируйся; ключи/плюрализация/`Intl` — `$i18n`.
