---
name: resilience-audit
description: Read-only аудит обработки ошибок и логов — «тихие» падения, пустые catch, дыры в логах (роль resilience). Use для ревью надёжности без правок.
---

# Задача: аудит обработки ошибок и логов (resilience)

Проведи **read-only** аудит указанной области (по умолчанию весь репозиторий). Ничего не правь — оформи находки.

Ищи: пустые/глотающие `catch` (`catch (e) {}`, `catch { console.log }` без обработки/проброса), `catch`, возвращающий `null`/`undefined` и скрывающий сбой, `throw` строк/объектов, потерю `cause`, необработанные промис-реджекты, `try` вокруг небросающего кода, незакрытые ресурсы при ошибке; по логам — отсутствие логов на важных границах, двойное логирование одной ошибки, лог секретов/PII, голый `console.log` в прод-пути, неверные уровни. Формат находки: `file:line`, что нашли, первопричина, конкретная починка; severity `critical>high>medium>low`. По значимым находкам — `add_task`, реализацию — через `$resilience-harden`. Опирайся на `$error-handling`, `$typescript`, `$nodejs`, `$backend-architecture`, `$react`.
