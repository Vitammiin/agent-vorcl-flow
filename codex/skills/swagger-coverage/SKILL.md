...
Каждый роут бэкенда должен быть **полностью** описан в OpenAPI-спеке, которую отдаёт `@fastify/swagger`. Спека — единственный источник правды для фронтового клиента (`$data-fetching`). Источник самой спеки — zod-схемы из `schemas.ts` модуля.

Архитектура (дефолт): Fastify; `src/app/plugins/swagger.plugin.ts` регистрирует `@fastify/swagger` + `@fastify/swagger-ui`; модули в `src/modules/<module>/` со слоями `routes.ts` / `schemas.ts` (zod) / `controller.ts` / `service.ts` / `repository.ts`. Типы — `z.infer` из схем (`$backend-architecture`).
...
## Связь с фронтендом
Эта спека — источник истины для фронтового клиента (`$data-fetching`): фронт всегда бьёт в реальные эндпоинты, типы генерируются из OpenAPI, моков в прод-пути нет.
