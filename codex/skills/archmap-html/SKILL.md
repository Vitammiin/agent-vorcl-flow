...
# Задача: интерактивная HTML-карта архитектуры (archmap)

Собери интерактивную HTML-карту архитектуры (см. `$archmap`).
...
Отдай: путь + команду открыть (`open docs/architecture/architecture.html`) + сводку `stats` + подсказку по интерактиву (тумблеры слоёв, клик → панель file:line, Trace ↑/↓, поиск, Cmd+P → PDF). Опирайся на `$archmap`. Веди как роль `$archmap`.
