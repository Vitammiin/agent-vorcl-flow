...
Agent-Vorcl-Flow convierte a un agente de codificación compatible en un **equipo de ingeniería estructurado**. En lugar de un asistente general, obtienes **22 subagentes especializados** (arquitecto, backend, frontend, Expo ingeniero móvil, DB ingeniero, cartógrafo de arquitectura, operador de liveboard y más), cada uno con su propio dominio **habilidades**, **comandos de barra diagonal** rápidos y las **MCP herramientas** que necesita. Cada tarea no trivial se ejecuta a través de un ciclo disciplinado **Task Master** (*objetivo → tareas → implementar → verificar → hecho*) para que el trabajo se planifique, se realice un seguimiento y sobreviva a las interrupciones.

- 🧩 **22 subagentes**, 44 habilidades, 135 comandos de barra
...
| **Kimi CLI** | Instala habilidades en `~/.kimi/skills`, el Expo agente personalizado nativo en `~/.kimi/agents`, ambas Expo arquitectura/UI enlaces en `~/.kimi/config.toml` y fusiona MCP servidores. |
...
| 🧪 **prueba** | Ingeniero de pruebas y verificación | Unidad (Vitest/Jest), integración (Supertest), E2E (Playwright), cobertura, búsqueda de pruebas inestables; ejecuta `testStrategy` de cada tarea: no se "hace" nada sin una ejecución verde |
...
| 📝 **docs** | Ingeniero de documentación | README (paridad multilingüe), API documentos de OpenAPI, ARQUITECTURA, CONTRIBUCIÓN, notas de la versión; cada ejemplo verificado con el código |
...
| 📡 **liveboard** | Junta de operaciones locales | Vive Git árboles de trabajo, procesos de agentes y Task Master tareas en un panel de control localhost efímero |**Algunas cosas que vale la pena saber:**
...
- **`pinpoint` encuentra, nunca crea.** Dada una captura de pantalla de una aplicación en ejecución, asigna la pantalla al código real (componente, ruta, el control exacto y la lógica detrás de él) y entrega la edición a `frontend`/`backend`. Funciona sobre lo que ya existe (lo inverso de `screenshot`).
...
- **`i18n` aplica "codificación de idioma cero".** Los agentes primero detectan si un proyecto es multilingüe y se adaptan: las cadenas de cara al usuario pasan por una capa de traducción (next-intl / react-i18next / i18next), nunca en línea.

---

## Referencia de comando

Cada comando a continuación es un comando de barra diagonal. `<…>` marca tu entrada.
...
| --- | --- |
| `/expo-mobile:vorcl <goal>` | Objetivo → Task Master ciclo para Expo trabajo móvil. |
...
| | Agregue esquema/DTO/mapper/claves de consulta e integración TanStack Query. |
...
| `/analyzer:db` | Auditoría de estructura DB: esquema, índices, FK, N+1, migraciones. |
...
### 🟡 arrogancia — OpenAPI/Swagger cobertura (cualquier pila)
| Comando | Qué hace |
| --- | --- |
...
| `/firecrawl:parse <file>` | Analizar un documento local/privado en rebajas o JSON. |
| `/firecrawl:monitor <action>` | Enumere las comprobaciones o administre los monitores de cambio de página recurrentes. |
| `/firecrawl:agent <goal>` | Ejecute una tarea de agente Firecrawl limitada y de larga duración. |
...
| `/firecrawl:deliverable <artifact>` | Produzca un resumen, una auditoría, una lista de clientes potenciales u otro artefacto del flujo de trabajo. |`/firecrawl:setup` ejecuta el flujo de `firecrawl-cli init --all` oficial solo después de la confirmación. Las habilidades oficiales existentes de `firecrawl-*` tienen prioridad y las conserva el instalador de Codex/Cursor; AVF proporciona alternativas compatibles para las habilidades faltantes. Ruta de operaciones en vivo a través de CLI → MCP → REST/sin llave.

### 🟤 renderizar - alojamiento / implementación (Renderizar)
| Comando | Qué hace |
| --- | --- |
...
| `/pinpoint:handoff <change>` | Cree una solicitud de edición precisa con el código existente y deléguela en `frontend`/`backend`. |

### 📊 drawio — diagramas (draw.io / diagramas.net)
| Comando | Qué hace |
| --- | --- |
| `/drawio:vorcl <goal>` | Un conjunto de diagramas a través de Task Master: compilación lista para terminar. |
...
| `/archmap:extract [repo]` | Solo extracción: `architecture.json` legible por máquina con `source:{file,line}` en cada nodo. |
...
| Comando | Qué hace |
| --- | --- |
| `/mermaid:vorcl <goal>` | Un conjunto de diagramas a través de Task Master: compilación para terminar (verificado por renderizado). |
...
| `/gitflow:commit <files\|scope>` | Una confirmación por nombre (nunca `git add .`) con un mensaje de confirmaciones convencionales; se detiene en WIP desconocido. |
...
| `/devops:compose` | docker-compose.yml para desarrollo local (aplicación + bases de datos); Los cambios de entorno necesitan `--force-recreate`, espera a que esté sano. |
...
Liveboard lee Git árboles de trabajo, procesos locales Claude/Codex/Cursor y el `.taskmaster/tasks/tasks.json` de cada árbol de trabajo. El estado de ejecución permanece en la memoria y desaparece cuando se detiene el proceso en primer plano. UI detecta el idioma del navegador y ofrece 43 idiomas, incluidos inglés, ruso, ucraniano, alemán, francés, español, portugués, italiano, polaco, turco, chino, japonés, árabe, holandés, checo, eslovaco, rumano, húngaro, búlgaro, serbio, croata, esloveno, griego, hebreo, persa, hindi, bengalí, urdu, indonesio, malayo, vietnamita, tailandés, coreano, sueco, noruego, danés, finlandés, estonio, letón, lituano, georgiano. Armenio y azerbaiyano. El árabe, el hebreo, el persa y el urdu utilizan el diseño RTL.
...
> ⚠️ **Requerido para comandos Task Master con tecnología de IA:** configure al menos un proveedor seleccionado: `ANTHROPIC_API_KEY` para Claude, `OPENAI_API_KEY` para GPT o Codex CLI OAuth. Sin credenciales para el modelo seleccionado en `.taskmaster/config.json`, `/vorcl` no puede generar ni expandir tareas.
...
Cursor usa el mismo formato abierto `SKILL.md` que el adaptador Codex, además de subagentes personalizados nativos y configuración MCP global:

| Agent-Vorcl-Flow concepto | Cursor equivalente |
| --- | --- |
| papel `backend` | subagente personalizado `/avf-backend` en `~/.cursor/agents` |
...
El instalador convierte las definiciones de roles a Cursor frontmatter, antepone a los subagentes `avf-` para evitar colisiones de nombres de habilidades, usa `model: inherit` y marca los agentes de solo auditoría como `readonly: true`. Se conservan las MCP entradas de servidor existentes con los mismos nombres. Ver [`cursor/README.md`](./cursor/README.md).
...
| Expo PostToolUsar guardia | fusionado en `~/.kimi/config.toml` |
