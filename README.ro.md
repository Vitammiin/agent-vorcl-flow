...
Agent-Vorcl-Flow transformă un agent de codificare acceptat într-o **echipă de inginerie structurată**. În loc de un asistent general, obțineți **22 de sub-agenți concentrați** (arhitect, backend, frontend, Expo inginer mobil, DB inginer, cartograf arhitectură, operator liveboard și alții), fiecare cu propriul domeniu **aptitudini**, comenzi **slash rapide** și **MCP instrumentele** de care are nevoie. Fiecare sarcină non-trivială trece printr-o buclă disciplinată **Task Master** — *scop → sarcini → implementare → verificare → gata* — astfel încât munca este planificată, urmărită și supraviețuiește întreruperilor.

- 🧩 **22 sub-agenți**, 44 de abilități, 135 de comenzi oblice
...
| **Kimi CLI** | Instalează abilitățile în `~/.kimi/skills`, agentul personalizat nativ Expo în `~/.kimi/agents`, ambele arhitectură Expo/UI se conectează în `~/.kimi/config.toml` și îmbină MCP servere. |
...
Fiecare agent are, de asemenea, propriul `/<agent>:vorcl` punct de intrare care rulează bucla Task Master destinată agentului respectiv.
...
| 🟢 **backend** | Dezvoltator backend | Nod/TS, Postgres, Redis; arhitectura modulara; fiecare rută acoperită în întregime de OpenAPI |
...
| 🟠 **analizator** | Auditor cod (numai citire) | Bug-uri, tip de siguranță, DB structură, batjocuri de front-end, mirosuri de backend |
| 🟡 **tâmpit** | OpenAPI/Swagger acoperire (orice stivă) | Găsește rute nedocumentate complet și le acoperă, cu verificare |
...
| 🖼️ **captură de ecran** | Captură de ecran UI → cod | Transformă o captură de ecran UI într-un cod gata de producție, receptiv și accesibil |
...
| 🗺️ **arhhartă** | Cartograf de arhitectură | Cod determinist → `architecture.json` (fiecare nod cu `source:{file,line}`) → interactiv HTML hartă, draw.io, Mermaid, ARCHITECTURE.md, PDF; faptele nedovedite sunt marcate `inferred` |
...
| 📝 **docs** | Inginer documentare | README (multi-language parity), API documente din OpenAPI, ARHITECTURE, CONTRIBUTING, note de lansare; fiecare exemplu verificat cu codul |
...
| 📡 **liveboard** | Consiliul local de operațiuni | Live Git arbori de lucru, procese de agent și Task Master sarcini pe un tablou de bord localhost efemer |**Câteva lucruri care merită știute:**
...
- **`database` mutațiile necesită confirmare explicită.** Analytics sunt doar pentru citire; Schimbările de schemă/date (DDL/DML/migrații) nu rulează niciodată fără aprobarea dumneavoastră.
...
- **`pinpoint` găsește, nu creează niciodată.** Având în vedere o captură de ecran a unei aplicații care rulează, mapează ecranul la codul real - componentă, traseu, controlul exact și logica din spatele acestuia - și predă editarea către `frontend`/`backend`. Funcționează pe ceea ce există deja (inversul lui `screenshot`).
...
Fiecare comandă de mai jos este o comandă slash. `<…>` marchează intrarea dvs.
...
| `/audit [path] [focus]` | Audit profund multi-rol, doar pentru citire, → sisteme detectate, constatări de securitate/CVE/reziliență, arhitectură țintă și `PROJECT_AUDIT.md` în faze. |
...
| `/frontend:refactor <target>` | Refactor UI / cârlige fără a schimba comportamentul. |
...
| `/expo-mobile:create-screen <flow>` | Creați o rută subțire Expo Router plus un ecran deținut de modul și stări. |
...
| `/expo-mobile:add-api <contract>` | Adăugați chei de schemă/DTO/mapper/interogare și integrarea TanStack Query. |
...
| `/expo-mobile:compatibility [app] [change]` | Audit de compatibilitate live numai pentru citire Expo/RN/Node/pachet/native-runtime față de sursele oficiale cu versiuni. |
...
| `/analyzer:db` | Audit DB structură — schemă, indici, FK, N+1, migrări. |
...
### 🎯 punctare — captură de ecran → plasare într-un proiect existent (numai citire)
| Comanda | Ce face |
| --- | --- |
...
| `/pinpoint:trace <target>` | Urmăriți logica din spatele unui element — handler → stare → data-fetch → API. |
...
| `/archmap:annotate [json]` | Îmbogățirea LLM a unui `architecture.json` existent (memorie agent, semantică flux de date); fapte nedovedite retrogradate automat la `inferred`. |
...
| `/archmap:diagram [json] [drawio\|mermaid]` | draw.io (multi-pagini: Prezentare generală / ERD / API / Agenți) și/sau Mermaid vizualizări, validate. |
...
| `/testing:e2e <scenario>` | Playwright E2E pentru o cale critică de utilizator — selectoare de rol, dispozitive, urmărire la eșec. |
...
| Comanda | Ce face |
| --- | --- |
| `/gitflow:vorcl <goal>` | Un obiectiv git/release prin Task Master (pregătirea unei ediții, curățarea istoricului, ramură caracteristică). |
...
| `/gitflow:release <version\|auto>` | Semver de la comiteri → sincronizare versiuni manifest → etichetă → GitHub lansare. Apăsați numai după confirmarea explicită. |
...
| `/docs:readme [path]` | Creați/actualizați README — ce/quickstart/usage/config/troubleshooting; exemple verificate; versiuni de limbă sincronizate. || `/docs:api [spec]` | API documente generate din OpenAPI spec (puncte finale, parametri, exemple de curl); sugerează `/swagger:audit` dacă nu există specificații. |
...
Pachetul nu are **nu există backend la distanță sau bază de date**. Liveboard-ul opțional este un proces în memorie doar pentru gazdă locală. MCP serverele au nevoie de jetoane și **fiecare utilizator își oferă propriile**. Pentru ca acest lucru să funcționeze identic între **Claude Code, Codex, Cursor și Kimi CLI** — și indiferent dacă lansați de pe un terminal sau de pe Dock / Spotlight / un IDE — fiecare server stdio MCP este pornit printr-un lansator mic (`bin/mcp-env.mjs`) care vă citește cheile dintr-un fișier**:
...
Alegeți ce furnizor Task Master execută de fapt generarea; cheile singure nu selectează modelul:
...
Comanda folosește fluxul oficial `task-master models` și stochează doar selecția de model în `.taskmaster/config.json`. `PERPLEXITY_API_KEY` este opțional și este necesar numai când Perplexity este selectat ca model de cercetare.
...
Codex nu are „plugin-uri”, așa că aceleași capacități sunt exprimate ca **aptitudini**, **profiluri** și un `AGENTS.md` router:
...
Cursor folosește același format `SKILL.md` deschis ca adaptorul Codex, plus subagenți personalizați nativi și configurația globală MCP:
...
Programul de instalare convertește definițiile rolului în Cursor frontmatter, prefixează subagenții cu `avf-` pentru a evita coliziunile numelor de abilități, folosește `model: inherit` și marchează agenții numai de audit ca `readonly: true`. Sunt păstrate MCP intrările de server existente cu aceleași nume. Vezi [`cursor/README.md`](./cursor/README.md).

---

## Kimi CLI

[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) încarcă nativ abilitățile de agent, fișierele agent personalizate și cârligele ciclului de viață; AVF combină, de asemenea, aceleași MCP servere utilizate de Claude și Cursor:
