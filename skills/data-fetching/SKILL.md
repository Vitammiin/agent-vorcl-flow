...
- Единый источник правды контракта — **OpenAPI-спека Fastify Swagger** (`GET /documentation/json`). Полнота спеки — забота бэка (скилл `swagger-coverage`).
...
## Слой api фичи
- `queryFn`/`mutationFn` вызывают `api.GET/POST/...` по эндпоинту из спеки; валидацию ответа обеспечивает бэк (сериализатор Fastify по response-схеме), на фронте достаточно типов из спеки.
