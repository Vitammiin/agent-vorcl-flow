...
- **`/<agent>:vorcl <цель>`** — если область известна заранее, входи сразу в неё:
  `/architect:vorcl` · `/principal-architect:vorcl` · `/backend:vorcl` · `/frontend:vorcl` · `/expo-mobile:vorcl` · `/analyzer:vorcl` · `/swagger:vorcl` · `/firecrawl:vorcl` · `/render:vorcl` · `/database:vorcl` · `/resilience:vorcl` · `/screenshot:vorcl` · `/design-studio:vorcl` · `/drawio:vorcl`
