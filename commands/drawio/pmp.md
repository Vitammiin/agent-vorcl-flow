---
description: Построить проектную диаграмму PMP/PMBOK в draw.io — WBS, PERT/CPM, Gantt, RACI, risk matrix, stakeholder grid (drawio)
...
Требования: валидный `mxGraphModel`, уникальные id, ортогональные рёбра, выравнивание по сетке, семантические цвета по легендам и обязательная **легенда** (цвет → значение). Соблюдай семантику метода (один `A` в RACI; 100% охват в WBS; непрерывная критическая цепочка). Сохрани в `.drawio`, проверь `xmllint --noout`, дай путь + как открыть в diagrams.net.
