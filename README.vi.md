...
Agent-Vorcl-Flow biến tác nhân mã hóa được hỗ trợ thành **nhóm kỹ thuật có cấu trúc**. Thay vì một trợ lý chung, bạn có **22 đại lý phụ tập trung** (kiến trúc sư, phụ trợ, giao diện người dùng, Expo kỹ sư di động, DB kỹ sư, người vẽ bản đồ kiến ​​trúc, nhà điều hành liveboard, v.v.), mỗi đại lý có **kỹ năng** miền riêng, **lệnh gạch chéo** nhanh và **MCP công cụ** cần thiết. Mọi nhiệm vụ không hề tầm thường đều chạy qua một vòng lặp **Task Master** có kỷ luật — *mục tiêu → nhiệm vụ → thực hiện → xác minh → hoàn thành* — vì vậy công việc được lên kế hoạch, theo dõi và không bị gián đoạn.

- 🧩 **22 đặc vụ phụ**, 44 kỹ năng, 135 lệnh gạch chéo
...
| **Kimi CLI** | Cài đặt các kỹ năng vào `~/.kimi/skills`, tác nhân tùy chỉnh Expo gốc vào `~/.kimi/agents`, cả kiến ​​trúc/UI đều nối vào `~/.kimi/config.toml` và hợp nhất các máy chủ. |
...
Bản cập nhật bao gồm các kỹ năng, tác nhân, hook, trình khởi chạy và khối cấu hình do Agent-Vorcl-Flow quản lý. Nó giữ cho `~/.config/agent-vorcl-flow/.env` hiện tại của bạn và các bí mật của nó không thay đổi, đồng thời duy trì các kỹ năng Firecrawl ngược lại. Sau đó khởi động lại máy khách mã hóa đã cập nhật (hoặc chạy `/reload-plugins` trong Claude Code).
...
Mọi nhiệm vụ không hề nhỏ đều được thực hiện thông qua **Task Master** (`task-master-ai`):
...
| 🧪 **thử nghiệm** | Kỹ sư kiểm tra và xác minh | Đơn vị (Vitest/Jest), tích hợp (Supertest), E2E (Playwright), phạm vi bao phủ, săn tìm thử nghiệm không ổn định; thực hiện `testStrategy` của mỗi nhiệm vụ — không có gì được "hoàn thành" nếu không có lệnh chạy màu xanh lá cây |
...
| 📝 **tài liệu** | Kỹ sư tài liệu | README (tương đương đa ngôn ngữ), API tài liệu từ OpenAPI, KIẾN TRÚC, ĐÓNG GÓP, ghi chú phát hành; mọi ví dụ được xác minh theo mã |
| 🐳 **devops** | Thùng chứa & CI/CD | Dockerfiles nhiều tầng, docker-compose dành cho nhà phát triển cục bộ, GitHub Quy trình hành động, vệ sinh môi trường/bí mật, giám sát |
| 📡 **bảng trực tiếp** | Ban điều hành địa phương | Sơ đồ công việc Git trực tiếp, quy trình tác nhân và nhiệm vụ Task Master trên bảng điều khiển localhost tạm thời |**Một số điều đáng biết:**
- **Frontend luôn nói chuyện với một API thực sự.** Thông số OpenAPI của backend là nguồn thông tin chính xác duy nhất; các loại được tạo ra từ nó (`openapi-typescript` + `openapi-fetch`). Không có mô hình nào trong quá trình sản xuất.
...
- **`resilience` gửi một móc an toàn.** Một móc `PostToolUse` (`catch-guard.js`) không chặn nhẹ nhàng gắn cờ các khối `catch {}` trống trong các tập tin bạn vừa chỉnh sửa.
- **`archmap` không bao giờ dựa trên trí tưởng tượng.** Quá trình trích xuất và kết xuất được tách biệt hoàn toàn: các tập lệnh không phụ thuộc đưa kho lưu trữ vào `architecture.json` (cơ sở dữ liệu với số lượng FK thực, API tuyến đường, tác nhân AI với mô hình/công cụ/bộ nhớ của chúng, biểu đồ nhập, env) và mọi sơ đồ chỉ được kết xuất từ ​​JSON đó. Bất cứ điều gì LLM thêm vào mà không có dấu `file:line` có thể kiểm chứng đều được đánh dấu bắt buộc và được vẽ nét đứt.
- **`pinpoint` tìm thấy, không bao giờ tạo.** Đưa ra một ảnh chụp màn hình của một ứng dụng đang chạy, nó ánh xạ màn hình tới mã thực — thành phần, tuyến đường, điều khiển chính xác và logic đằng sau nó — và giao phần chỉnh sửa cho `frontend`/`backend`. Nó hoạt động trên những gì đã tồn tại (nghịch đảo của `screenshot`).
...
Mọi lệnh dưới đây đều là lệnh gạch chéo. `<…>` đánh dấu thông tin đầu vào của bạn.
...
| `/expo-mobile:create-screen <flow>` | Tạo một tuyến đường mỏng cùng với màn hình và các trạng thái do mô-đun sở hữu. |
...
| `/expo-mobile:add-api <contract>` | Thêm các khóa lược đồ/DTO/mapper/truy vấn và tích hợp TanStack Query. |
...
| `/analyzer:db` | Kiểm tra cấu trúc — lược đồ, chỉ mục, FK, N+1, di chuyển. |
...
| Lệnh | Nó làm gì |
| --- | --- |
| `/firecrawl:vorcl <goal>` | Mục tiêu nghiên cứu thông qua Task Master — thu thập dữ liệu web để có kết quả cuối cùng. |
...
| `/firecrawl:research <query>` | Tìm kiếm tài liệu và bối cảnh nghiên cứu. |
| `/firecrawl:ask <jobId>` | Chẩn đoán một công việc Firecrawl thất bại. |
| `/firecrawl:docs-search <question>` | Tìm kiếm tài liệu Firecrawl chính thức hiện tại. |
| `/firecrawl:integrate <feature>` | Thêm Firecrawl vào mã ứng dụng thông qua kỹ năng xây dựng ngược dòng. |
...
| `/screenshot:responsive <target>` | Làm cho UI được tạo ra có tính đáp ứng — các truy vấn điểm ngắt, linh hoạt, `clamp()`, vùng chứa. |
...
| `/pinpoint:locate <image>` | Xác định vị trí (các) thành phần/tệp hiện có từ ảnh chụp màn hình — `file:line`, không có mã mới. |
...
| Lệnh | Nó làm gì |
| --- | --- |
| `/mermaid:vorcl <goal>` | Một bộ sơ đồ thông qua Task Master — xây dựng xong (kết xuất đã được xác minh). |
...
Gói này không có phần phụ trợ hoặc cơ sở dữ liệu từ xa**. Liveboard tùy chọn là một quy trình trong bộ nhớ chỉ dành cho localhost. MCP máy chủ cần mã thông báo và **mỗi người dùng tự cung cấp mã thông báo**. Để làm cho tính năng này hoạt động giống hệt nhau trên **Claude Code, Codex, Cursor và Kimi CLI** — và cho dù bạn khởi chạy từ thiết bị đầu cuối hay từ Dock / Spotlight / IDE — mọi máy chủ stdio MCP đều được khởi động thông qua một trình khởi chạy nhỏ (`bin/mcp-env.mjs`) đọc khóa của bạn từ **một tệp**:
...
[Kimi CLI](https://github.com/MoonshotAI/kimi-cli) (MoonshotAI) tải các Kỹ năng đặc vụ, các tệp tác nhân tùy chỉnh và móc vòng đời; AVF cũng hợp nhất các máy chủ MCP được sử dụng bởi Claude và Cursor:
...
| Expo PostToolUse bảo vệ | sáp nhập vào `~/.kimi/config.toml` |
...
**Làm thế nào nó phù hợp với nhau:** `agents/*.md` khai báo một vai trò và, trong vấn đề chính `skills:`, đính kèm các kỹ năng → kỹ năng trong `skills/*/SKILL.md` được tải tự động theo mô tả → `commands/<agent>/*.md` cung cấp các phím tắt `/agent:command` nhanh chóng ủy quyền cho đại lý phụ → `.mcp.json` cung cấp cho các đại lý các công cụ của họ, mỗi công cụ bắt đầu thông qua `bin/mcp-env.mjs` để tải các bí mật từ `.env` được chia sẻ. Một `SessionStart` hook cho Claude biết các tác nhân đang sẵn sàng.
